 ...
 * <p>
 * 应用程序通过在子类中覆盖 {@link
 * #getPasswordAuthentication()} 方法来使用此类。此方法通常会使用各种 getXXX() 访问器方法来获取请求身份验证的实体的信息。然后，它必须通过与用户交互或其他非交互方式获取用户名和密码。凭据随后作为 {@link PasswordAuthentication} 返回值返回。
 * <p>
 * 该具体子类的实例通过调用 {@link #setDefault(Authenticator)} 注册到系统中。当需要身份验证时，系统将调用其中一个 requestPasswordAuthentication() 方法，这些方法又会调用已注册对象的 getPasswordAuthentication() 方法。
 * <p>
...
class Authenticator {
    ...
    /**
     * 设置当代理或 HTTP 服务器请求身份验证时将使用的身份验证器。
     * <p>
     * 首先，如果有安全经理，其 {@code checkPermission}
     * 方法将被调用，权限为
     * {@code NetPermission("setDefaultAuthenticator")}。这可能导致 java.lang.SecurityException。
     *
     * @param   a       要设置的身份验证器。如果 a 为 {@code null}，则移除任何先前设置的身份验证器。
     *
     ...
     * 首先，如果有安全经理，其 {@code checkPermission}
     * 方法将被调用，权限为
     * {@code NetPermission("requestPasswordAuthentication")}。这可能导致 java.lang.SecurityException。
     *
    ...
    /**
     * 请求系统中注册的身份验证器提供密码。这是请求密码的首选方法，因为在无法提供 InetAddress 的情况下可以提供主机名。
     * <p>
     * 首先，如果有安全经理，其 {@code checkPermission}
     * 方法将被调用，权限为
     * {@code NetPermission("requestPasswordAuthentication")}。这可能导致 java.lang.SecurityException。
     ...
     * @param protocol 请求连接的协议
     *          ({@link java.net.Authenticator#getRequestingProtocol()})
     * @param prompt 用户的提示字符串，标识身份验证领域。
     * @param scheme 身份验证方案
     *
     * @return 用户名/密码，如果无法获取则为 null。
     *
    ...
    /**
     * 获取请求身份验证的站点或代理的 {@code hostname}，如果不可用则为 {@code null}。
     *
     * @return 需要身份验证的连接的主机名，如果不可用则为 null。
     * @since 1.4
     */
    ...
    /**
     * 获取请求授权的站点的 {@code InetAddress}，如果不可用则为 {@code null}。
     *
     * @return 请求授权的站点的 InetAddress，如果不可用则为 null。
     */
    ...
    /**
     * 获取请求连接的协议。通常这将基于 URL，但在未来的 JDK 中，例如，它可以是受密码保护的 SOCKS5 防火墙的 "SOCKS"。
     *
     * @return 协议，可选后跟 "/version"，其中 version 是版本号。
     *
     ...
     * 获取请求者提供的提示字符串。
     *
     * @return 请求者提供的提示字符串（对于 HTTP 请求为领域）。
     */
     ...
     * @return 从用户收集的 PasswordAuthentication，如果未提供则为 null。
     */
