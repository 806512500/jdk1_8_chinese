...
class InMemoryCookieStore implements CookieStore {
    ...
    // cookie 按其域和关联的 URI（如果存在）索引
    // 注意：当 cookie 从主数据结构（即 cookieJar）中移除时，
    //       它不会在 domainIndex 和 uriIndex 中被清除。从索引存储中检索 cookie 时，需要双重检查 cookie 的存在。
    ...
    /*
     * 这几乎与 HttpCookie.domainMatches 相同，只有一个不同点：
     * 它不会因为域的 'H' 部分包含点 ('.') 而拒绝 cookie。
     * 例如：RFC 2965 第 3.3.2 节指出，如果主机是 x.y.domain.com
     * 而 cookie 域是 .domain.com，则应拒绝该 cookie。
     * 但实际情况并非如此。浏览器不会拒绝，有些网站（如 yahoo.com）实际上期望这些 cookie 被传递。
    ...
    // @param comparator        用于决定索引中的 cookie 是否应返回的预测
    ...
    // 将 'cookie' 通过 'index' 索引添加到 'indexStore'
    ...
    //
    // 对于 cookie 的目的，有效的 URI 应仅为 http://host
    // 路径将在应用路径匹配算法时考虑
    //
