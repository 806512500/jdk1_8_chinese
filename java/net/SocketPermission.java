...
/**
 * 该类表示通过套接字访问网络。
 * SocketPermission 由一个主机规范和一组指定连接到该主机方式的“操作”组成。主机指定为
 * <pre>
 *    host = (hostname | IPv4address | iPv6reference) [:portrange]
 *    portrange = portnumber | -portnumber | portnumber-[portnumber]
 * </pre>
 * 主机表示为 DNS 名称、数值 IP 地址或“localhost”（本地机器）。
 * 通配符“*”可以在 DNS 名称主机规范中包含一次。如果包含，它必须位于最左侧，例如“*.sun.com”。
 * <p>
 * IPv6reference 的格式应遵循 <a
 * href="http://www.ietf.org/rfc/rfc2732.txt"><i>RFC&nbsp;2732: Format
 * for Literal IPv6 Addresses in URLs</i></a> 中指定的格式：
 * <pre>
 *    ipv6reference = "[" IPv6address "]"
 *</pre>
 * 例如，可以构造一个 SocketPermission 实例如下：
 * <pre>
 ...
 * </pre>
 * <p>
 * <A HREF="Inet6Address.html#lform">完整的未压缩形式</A>
 * 的 IPv6 文字地址也是有效的。
 * <p>
 * 端口或端口范围是可选的。形式为 "N-" 的端口规范，其中 <i>N</i> 是端口号，表示所有编号为 <i>N</i> 及以上的端口，而形式为 "-N" 的规范表示所有编号为 <i>N</i> 及以下的端口。
 * 特殊端口值 {@code 0} 指的是整个 <i>临时</i> 端口范围。这是一个系统可能用于分配动态端口的固定范围的端口。实际范围可能因系统而异。
 * <p>
 * 连接到主机的可能方式为
 * <pre>
 * accept
 ...
 * “resolve” 操作在其他任何操作存在时隐含。操作“resolve”指的是主机/IP 名称服务查找。
 * <P>
 * 操作字符串在处理前转换为小写。
 ...
 * </pre>
 *
 * 则允许该代码连接到 {@code puffin.eng.sun.com} 的 7777 端口，并在该端口上接受连接。
 *
 * <p>同样，如果授予以下权限：
 *
 * <pre>
 ...
 * </pre>
 *
 * 则允许该代码在本地主机的 1024 到 65535 之间的任何端口上接受连接、连接或监听。
 *
 * <p>注意：授予代码接受或连接到远程主机的权限可能是危险的，因为恶意代码可以更容易地在可能没有访问数据权限的各方之间传输和共享机密数据。
 *
    ...
    implements java.io.Serializable
{
    ...
    // 如果使用单个数值 IP 地址初始化，则为 true
    private transient boolean init_with_ip;

    // 如果此 SocketPermission 表示无效/未知主机，则为 true
    // 用于 implies，当延迟查找已失败时
    ...
    // 未能通过我们的反向映射启发式测试，则为 true
    private transient boolean untrusted;
    private transient boolean trusted;

    // 如果设置了 sun.net.trustNameService 系统属性，则为 true
    ...
    /**
     * 使用指定的操作创建新的 SocketPermission 对象。
     * 主机表示为 DNS 名称或数值 IP 地址。
     * 可选地，可以提供端口或端口范围（用冒号分隔）。
     * <p>
     * 要指定本地机器，使用“localhost”作为 <i>host</i>。
     * 请注意：空的 <i>host</i> 字符串 (“”) 等同于“localhost”。
     * <p>
     * <i>actions</i> 参数包含一个逗号分隔的列表，列出了授予指定主机（和端口）的操作。可能的操作是
     * “connect”，“listen”，“accept”，“resolve”，或这些操作的任何组合。“resolve” 在指定了其他三个操作中的任何一个时自动添加。
     * <p>
     * 以下是一些 SocketPermission 实例化的例子：
     * <pre>
     ...
     * </pre>
     *
     * @param host 计算机的主机名或 IP 地址，可选地包括冒号后跟端口或端口范围。
     * @param action 操作字符串。
     */
    public SocketPermission(String host, String action) {
        super(getHost(host));
        // name 初始化为 getHost(host); 在 getHost() 中检测 NPE
    ...
    private void init(String host, int mask) {
        ...
        // 解析主机名。名称最多有三个部分，主机名、端口号，或两个表示端口范围的数字。
    ...
    private static int getMask(String action) {
        ...
        // 使用对象身份比较已知的内部字符串以提高性能（这些值在JDK中频繁使用）。
        ...
        throws UnknownHostException
    {
        ...
        try {
            // 如果我们还没有 IP 地址，则首先获取 IP 地址
            // 这是因为我们需要 IP 地址来获取 FQDN。
            if (addresses == null) {
                getIP();
            }

            // 我们必须进行此检查，否则可能无法获取完全限定域名
            if (init_with_ip) {
    ...
    /**
     * 获取 IP 地址。如果无法获取 IP 地址，则将 invalid 设置为 true。
     *
     */
    void getIP()
        throws UnknownHostException
     ...
     * </ul>
     *
     * 然后 {@code implies} 按以下顺序检查每个条件，并在每个条件为真时返回 true：
     * <ul>
     * <li> 如果此对象使用单个 IP 地址初始化，并且 <i>p</i> 的一个 IP 地址等于此对象的 IP 地址。
     * <li> 如果此对象是通配符域（如 *.sun.com），并且 <i>p</i> 的规范名称（不带任何前导 *）以该对象的规范主机名结尾。例如，*.sun.com 隐含 *.eng.sun.com。
     * <li> 如果此对象未使用单个 IP 地址初始化，并且此对象的一个 IP 地址等于 <i>p</i> 的一个 IP 地址。
     * <li> 如果此规范名称等于 <i>p</i> 的规范名称。
     * </ul>
     *
     * 如果以上任何条件都不为真，则 {@code implies} 返回 false。
     * @param p 要检查的权限。
     *
     ...
     * <li> 如果此对象使用 IP 地址初始化，检查 "p" 的一个 IP 地址是否等于此对象的 IP 地址。
     * <li> 如果任一对象是通配符域（例如，"*.sun.com"），尝试基于通配符进行匹配。
     * <li> 如果此对象未使用 IP 地址初始化，尝试基于两个对象的 IP 地址进行匹配。
    ...
    boolean impliesIgnoreMask(SocketPermission that) {
        ...
        try {
            ...
            if (this.wildcard || that.wildcard) {
                // 如果它们都是通配符，当且仅当
                // that 的 cname 以 this 的 cname 结尾时返回 true（即，*.sun.com
                // 意味着 *.eng.sun.com）
                if (this.wildcard && that.wildcard)
                ...
                // 这是一个通配符，让我们看看 that 的 cname 是否以
                // 它结尾...
                if (that.cname == null) {
                    that.getCanonName();
                }
     ...
     *  SocketPermission 对象相同的主机名、端口范围和操作，则返回 true。但是，如果 <i>obj</i> 只包含 'resolve' 操作，则在比较中忽略端口范围。
     */
    ...
    /**
     * 返回一个用于存储 SocketPermission
     * 对象的新 PermissionCollection 对象。
     * <p>
     * SocketPermission 对象必须以允许它们
     * 以任何顺序插入集合的方式存储，但同时也要使
    ...
    /**
     * WriteObject 被调用来将 SocketPermission
     * 的状态保存到流中。操作被序列化，超类
     * 负责名称。
     */
    ...
    /**
     * readObject 被调用来从流中恢复 SocketPermission 的状态。
     */
    ...
    implements Serializable
{
    ...
    /**
     * 向 SocketPermissions 中添加一个权限。哈希的键是
     * 通配符情况下的名称，或所有 IP 地址。
     *
     ...
     *                                       SocketPermission
     *
     * @exception SecurityException - 如果此 SocketPermissionCollection 对象
     *                                已被标记为只读
     */
     ...
     *
     * @return 如果 "permission" 是权限集合中权限的适当子集，则返回 true，否则返回 false。
     */
     ...
     */
    /*
     * 将 perms 字段的内容写入 Vector，以实现与早期版本的序列化兼容性。
     */
