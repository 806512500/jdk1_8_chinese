...
public class Proxy {
     ...
     * 基本上告诉协议处理程序不使用任何代理。
     * 例如，用于创建绕过任何其他全局代理设置（如 SOCKS）的套接字：
     * <P>
    ...
    /**
     * 创建一个表示代理连接的条目。
     * 某些组合是非法的。例如，对于类型 Http 和 Socks，
     * 必须提供一个 SocketAddress。
     * <P>
     * 使用 {@code Proxy.NO_PROXY} 常量
     * 表示直接连接。
     *
     * @param type 代理的 {@code Type}
     * @param sa 该代理的 {@code SocketAddress}
    ...
    /**
     * 构造此 Proxy 的字符串表示形式。
     * 该字符串通过调用其类型的 toString() 方法并连接 " @ " 和其地址的 toString() 结果（如果类型不是 {@code DIRECT}）来构造。
     *
     * @return 此对象的字符串表示形式。
     */
    public String toString() {
        ...
        /**
     * 将此对象与指定的对象进行比较。
     * 如果且仅当参数不为 {@code null} 并且表示与
     * 此对象相同的代理时，结果为 {@code true}。
     * <p>
