...
/*
 * 该类 PlainSocketImpl 简单地委托给适当的实际 SocketImpl。我们这样做是因为 PlainSocketImpl 已经被 SocksSocketImpl 扩展。
 * <p>
 * 实际的 SocketImpl 有两种可能性，TwoStacksPlainSocketImpl 或 DualStackPlainSocketImpl。我们在具有双栈
 * TCP 实现的系统上使用 DualStackPlainSocketImpl。否则，我们创建一个 TwoStacksPlainSocketImpl 的实例并委托给它。
 *
 * @作者 Chris Hegarty
 */
...
class PlainSocketImpl extends AbstractPlainSocketImpl
{
    ...
    static {
        ...
        if (exclBindProp != null) {
            // sun.net.useExclusiveBind 为 true
    ...
    // 覆盖访问 impl 字段的 AbstractPlainSocketImpl 方法。

    protected synchronized void create(boolean stream) throws IOException {
        impl.create(stream);

        // 设置 fd 为委托的 fd 以与旧版本兼容
        this.fd = impl.fd;
    }
...
// 覆盖 AbstractPlainSocketImpl 中需要实现的方法。
