...
/**
 * 抽象的 datagram 和 multicast socket 实现基类。
...
public abstract class DatagramSocketImpl implements SocketOptions {
    ...
    /**
     * 发送一个 datagram 包。包中包含数据和要发送包的目标地址。
     * @param p 要发送的包。
     * @exception IOException 如果在发送 datagram 包时发生 I/O 异常。
     * @exception  PortUnreachableException 如果 socket 连接到当前无法到达的目标，则可能会抛出此异常。请注意，不保证会抛出此异常。
     */
    protected abstract void send(DatagramPacket p) throws IOException;

    /**
     * 将 datagram socket 连接到远程目标。这将远程地址与本地 socket 关联起来，以便 datagram 只能发送到此目标并从该目标接收。这可以被重写以调用本机系统的 connect。
     *
     * <p>如果 socket 连接到的远程目标不存在或无法到达，并且已收到该地址的 ICMP 目标不可达包，则后续的发送或接收调用可能会抛出 PortUnreachableException。
     * 请注意，不保证会抛出此异常。
     * @param address 要连接到的远程 InetAddress
     * @param port 远程端口号
     * @exception   SocketException 如果 socket 无法连接到远程目标，则可能会抛出此异常
     * @since 1.4
     */
    protected void connect(InetAddress address, int port) throws SocketException {}

    /**
     * 从其远程目标断开 datagram socket。
     * @since 1.4
     */
    ...
    /**
     * 查看包的来源。数据被复制到指定的 {@code DatagramPacket} 中。数据被返回，
     * 但未被消耗，因此后续的 peekData/receive 操作
     * 将看到相同的数据。
     * @param p 接收到的包。
     * @return 包的来源端口号。
     * @exception IOException 如果发生 I/O 异常
     ...
     * 接收 datagram 包。
     * @param p 接收到的包。
     * @exception IOException 如果在接收 datagram 包时发生 I/O 异常。
     * @exception  PortUnreachableException 如果 socket 连接到当前无法到达的目标，则可能会抛出此异常。请注意，不保证会抛出此异常。
     */
    ...
    protected abstract void setTimeToLive(int ttl) throws IOException;
                ...
                /**
     * 获取 TTL (生存时间) 选项。
     * @exception IOException 如果在获取生存时间选项时发生 I/O 异常
     ...
     * @exception IOException 如果在退出多播组时发生 I/O 异常。
     */
     ...
     * @param netIf 指定退出多播组的本地接口
     * @throws IOException 如果在退出多播组时发生 I/O 异常
     * @since 1.4
     */
    ...
    /**
     * 获取数据报套接字文件描述符。
     * @return 一个 {@code FileDescriptor} 对象表示数据报套接字文件描述符
     */
