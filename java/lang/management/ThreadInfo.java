 ...
 * <ul>
 *   <li>线程因同步或等待通知而阻塞的次数。</li>
 *   <li>线程因同步或等待通知而阻塞的累积时间
 *       自从 {@link ThreadMXBean#setThreadContentionMonitoringEnabled
 *       线程竞争监控}
 *       启用以来。某些 Java 虚拟机实现可能不支持此功能。可以使用
 *       {@link ThreadMXBean#isThreadContentionMonitoringSupported()}
 *       方法来确定 Java 虚拟机是否支持此功能。</li>
 * </ul>
 *
 * <p>此类设计用于系统监控，而不是用于同步控制。
 *
 * <h4>MXBean 映射</h4>
 * <tt>ThreadInfo</tt> 映射到一个 {@link CompositeData CompositeData}
 * 属性如 {@link #from from} 方法中指定。
 *
...
public class ThreadInfo {
     ...
     * @param t             线程
     * @param state         线程状态
     * @param lockObj       线程阻塞的对象
     * @param lockOwner     持有锁的线程
     * @param blockedCount  进入锁的阻塞次数
     * @param blockedTime   进入锁的阻塞时间
     * @param waitedCount   等待锁的次数
     * @param waitedTime    等待锁的时间
     ...
     * @param waitedTime    等待锁的时间
     * @param stackTrace    线程堆栈跟踪
     * @param lockedMonitors 锁定的监视器列表
     * @param lockedSynchronizers 锁定的同步器列表
     */
    ...
    /*
     * 从 {@link CompositeData CompositeData} 构造 <tt>ThreadInfo</tt> 对象。
     */
    private ThreadInfo(CompositeData cd) {
        ...
        if (ticd.isCurrentVersion()) {
            ...
            // lockInfo 是 1.6 ThreadInfo 中新增的属性
    ...
    /**
     * 返回与此 <tt>ThreadInfo</tt> 关联的线程自线程竞争监控启用以来
     * 进入或重新进入监视器的累积时间（以毫秒为单位）。
     * 即自线程竞争监控上次启用以来，线程处于
     * {@link java.lang.Thread.State#BLOCKED BLOCKED} 状态的总累积时间。
     * 如果线程竞争监控已禁用，此方法返回 <tt>-1</tt>。
     *
     * <p>Java 虚拟机可能使用高分辨率计时器测量时间。此统计信息在
     * 线程竞争监控重新启用时重置。
     *
     * @return 线程进入 <tt>BLOCKED</tt> 状态的累积时间（以毫秒为单位）；
     * <tt>-1</tt> 如果线程竞争监控已禁用。
     *
     ...
     * <tt>TIMED_WAITING</tt> 状态的累积时间（以毫秒为单位）；
     * <tt>-1</tt> 如果线程竞争监控已禁用。
     *
     * @throws java.lang.UnsupportedOperationException 如果 Java
     * 虚拟机不支持此操作。
    ...
    public long getWaitedTime() {
        return waitedTime;
                ...
                /**
     * 返回与此 <tt>ThreadInfo</tt> 关联的线程等待通知的总次数。
     * 即线程处于 {@link java.lang.Thread.State#WAITING WAITING}
    ...
    /**
     * 返回与此 <tt>ThreadInfo</tt> 关联的线程正在等待的对象的 <tt>LockInfo</tt>。
     * 线程可能因以下原因之一而被阻塞等待：
     * <ul>
     ...
     *     <p></li>
     * <li>等待其他线程通知对象监视器。
     *     <br>线程处于 {@link java.lang.Thread.State#WAITING WAITING}
     *     或 {@link java.lang.Thread.State#TIMED_WAITING TIMED_WAITING} 状态
     *     由于调用了 {@link Object#wait Object.wait} 方法。
     *     <p></li>
     * <li>由于线程被挂起而负责的同步对象。
     *     <br>线程处于 {@link java.lang.Thread.State#WAITING WAITING}
     *     或 {@link java.lang.Thread.State#TIMED_WAITING TIMED_WAITING} 状态
     ...
     *     LockSupport.park} 方法。同步对象是从
     *     {@link java.util.concurrent.locks.LockSupport#getBlocker
     *     LockSupport.getBlocker} 方法返回的。通常它是一个
     ...
     *
     * @return 线程正在等待的对象的 <tt>LockInfo</tt>，如果线程没有被阻塞等待任何对象，则返回 <tt>null</tt>。
     * @since 1.6
     */
    ...
    /**
     * 返回与此 <tt>ThreadInfo</tt> 关联的线程正在等待的对象的
     * {@link LockInfo#toString 字符串表示}。
     * 此方法等同于调用：
     * <blockquote>
     * <pre>
     ...
     * </pre></blockquote>
     *
     * <p>如果此线程没有被阻塞等待任何对象或对象未被任何线程拥有，此方法将返回 <tt>null</tt>。
     *
     * @return 线程正在等待的对象的字符串表示，如果线程没有被阻塞等待任何对象或对象未被任何线程拥有，则返回 <tt>null</tt>。
     *
     * @see #getLockInfo
     */
    ...
    /**
     * 返回与此 <tt>ThreadInfo</tt> 关联的线程正在等待的对象的拥有线程的 ID。
     * 如果此线程没有被阻塞等待任何对象或对象未被任何线程拥有，此方法将返回 <tt>-1</tt>。
     *
     * @return 线程正在等待的对象的拥有线程的 ID；
     *         如果此线程没有被阻塞或对象未被任何线程拥有，则返回 <tt>-1</tt>。
     *
     * @see #getLockInfo
     */
    ...
    /**
     * 返回与此 <tt>ThreadInfo</tt> 关联的线程正在等待的对象的拥有线程的名称。
     * 如果此线程没有被阻塞等待任何对象或对象未被任何线程拥有，此方法将返回 <tt>null</tt>。
     *
     * @return 线程正在等待的对象的拥有线程的名称；
     *         如果此线程没有被阻塞或对象未被任何线程拥有，则返回 <tt>null</tt>。
     *
     * @see #getLockInfo
     */
     ...
     * 如果返回的数组长度不为零，则数组的第一个元素表示堆栈的顶部，即序列中最新的方法调用。
     * 数组的最后一个元素表示堆栈的底部，即序列中最不新的方法调用。
     *
    ...
    /**
     * 测试与此 <tt>ThreadInfo</tt> 关联的线程是否被挂起。如果调用了
     * {@link Thread#suspend}，此方法返回 <tt>true</tt>。
     *
     ...
     * 此字符串的格式取决于实现。
     * 返回的字符串通常包括
     * {@linkplain #getThreadName 线程名称}，
     * {@linkplain #getThreadId 线程 ID}，
     * 其 {@linkplain #getThreadState 状态}，
     * 以及一个 {@linkplain #getStackTrace 堆栈跟踪}（如果有）。
     *
     * @return 此线程信息的字符串表示。
     */
    ...
    /**
     * 返回由给定的 <tt>CompositeData</tt> 表示的 <tt>ThreadInfo</tt> 对象。
     ...
     *       <p>
     *       如果 <tt>cd</tt> 不包含此属性，
     *       则 <tt>LockInfo</tt> 对象将从 <tt>lockName</tt> 属性的值构造。 </td>
     * </tr>
     * <tr>
     *   <td>lockName</td>
     *   <td><tt>java.lang.String</tt></td>
     ...
     *   <td><tt>javax.management.openmbean.CompositeData[]</tt>
     *       <p>
     *       每个元素是一个 <tt>CompositeData</tt>，表示包含以下属性的 StackTraceElement：
     *       <blockquote>
     ...
     *   <td><tt>javax.management.openmbean.CompositeData[]</tt>
     *       - 由 {@link MonitorInfo#from Monitor.from} 方法指定的 {@link MonitorInfo} 的映射类型。
     *       <p>
     *       如果 <tt>cd</tt> 不包含此属性，
     *       则此属性将被设置为一个空数组。 </td>
     * </tr>
     ...
     *
     * @return 由 <tt>cd</tt> 表示的 <tt>ThreadInfo</tt> 对象，如果 <tt>cd</tt> 为 <tt>null</tt>，则返回 <tt>null</tt>。
     */
    ...
    /**
     * 返回一个 {@link MonitorInfo} 对象数组，每个对象表示与此 <tt>ThreadInfo</tt> 关联的线程当前锁定的对象监视器。
     * 如果没有为此线程信息请求锁定的监视器或线程没有锁定任何监视器，此方法将返回一个零长度的数组。
     *
    ...
    /**
     * 返回一个 {@link LockInfo} 对象数组，每个对象表示与此 <tt>ThreadInfo</tt> 关联的线程当前锁定的
     * <a href="LockInfo.html#OwnableSynchronizer">可拥有同步器</a>。
     * 如果没有为此线程信息请求锁定的同步器或线程没有锁定任何同步器，此方法将返回一个零长度的数组。
     *
