...
public final class Parameter implements AnnotatedElement {
    ...
    /**
     * 包私有的 {@code Parameter} 构造器。
     *
     * 如果类文件中存在方法参数数据，则 JVM 直接创建 {@code Parameter} 对象。如果不存在，则 {@code Executable} 使用此构造器来合成它们。
     *
     * @param name 参数的名称。
    ...
    /**
     * 如果参数在类文件中有名称，则返回 true；否则返回 false。参数是否有名称由声明参数的方法的 {@literal MethodParameters} 属性决定。
     *
     * @return 如果参数在类文件中有名称，则返回 true。
     */
    ...
    /**
     * 返回参数的名称。如果参数的名称 {@linkplain #isNamePresent() 存在}，则此方法返回类文件提供的名称。否则，此方法合成一个形式为 argN 的名称，其中 N 是参数在声明参数的方法描述符中的索引。
     *
     * @return 参数的名称，由类文件提供或合成（如果类文件未提供名称）。
     */
    public String getName() {
        // 注意：参数名称为空字符串现在是非法的。
        // .equals("") 是为了与当前 JVM 行为兼容。将来可能会移除。
    ...
    public Class<?> getType() {
        ...
        return tmp;
                ...
                /**
     * 返回一个 AnnotatedType 对象，该对象表示用于指定此 Parameter 表示的形式参数类型的类型的使用。
     *
     * @return 一个 {@code AnnotatedType} 对象，表示用于指定此 Parameter 表示的形式参数类型的类型的使用
     */
     ...
     *
     * @return 如果且仅如果此参数如《Java&trade; 语言规范》所定义的那样隐式声明，则返回 true
     */
     ...
     *
     * @jls 13.1 二进制的形式
     * @return 如果且仅如果此参数如《Java&trade; 语言规范》所定义的那样是一个合成构造，则返回 true
     */
     ...
     *
     * @return 如果且仅如果此参数表示一个可变参数列表，则返回 {@code true}
     */
