...
/*
 * 版权所有 (c) 1994, 2013, Oracle 和/或其子公司。保留所有权利。
 ...
 * <p>
 * 此外，此类提供了许多方法，用于将 {@code boolean} 转换为 {@code String} 和将
 * {@code String} 转换为 {@code boolean}，以及其他在处理 {@code boolean} 时有用的常量和方法。
 *
...
{
    /**
     * 与基本值 {@code true} 对应的 {@code Boolean} 对象。
     */
    public static final Boolean TRUE = new Boolean(true);

    /**
     * 与基本值 {@code false} 对应的 {@code Boolean} 对象。
     */
     ...
     *
     * <p><b>注意：很少适合使用此构造函数。除非需要一个 <i>新的</i> 实例，否则静态工厂
     * {@link #valueOf(boolean)} 通常是一个更好的选择。它可能会显著提高空间和时间性能。</b>
     *
    ...
    /**
     * 如果字符串参数不为 {@code null} 且忽略大小写等于字符串 {@code "true"}，则分配一个表示
     * 值 {@code true} 的 {@code Boolean} 对象。否则，分配一个表示值 {@code false} 的
    ...
    /**
     * 将字符串参数解析为布尔值。如果字符串参数不为 {@code null} 且忽略大小写等于字符串
     * {@code "true"}，则返回的 {@code boolean} 表示值 {@code true}。 <p>
     ...
     * 为 {@code true}，此方法返回 {@code Boolean.TRUE}；如果为 {@code false}，此方法返回
    ...
    /**
     * 返回一个表示指定字符串值的 {@code Boolean}。如果字符串参数不为 {@code null}
     * 且忽略大小写等于字符串 {@code "true"}，则返回的 {@code Boolean} 表示一个
     * 真值。
     *
     * @param   s   一个字符串。
     * @return  字符串表示的 {@code Boolean} 值。
     */
    ...
    /**
     * 返回表示指定布尔值的 {@code String} 对象。如果指定的布尔值为 {@code true}，则
     * 返回字符串 {@code "true"}，否则返回字符串 {@code "false"}。
     *
     * @param b 要转换的布尔值
    ...
    /**
     * 返回表示此 Boolean 值的 {@code String} 对象。如果此对象表示值 {@code true}，
     ...
     *
     * @return  如果此对象表示 {@code true}，则返回整数 {@code 1231}；如果此
     * 对象表示 {@code false}，则返回整数 {@code 1237}。
     */
    @Override
    public int hashCode() {
   ...
   /**
     * 如果且仅如果参数不是 {@code null} 并且是一个表示与该对象相同的 {@code boolean} 值的 {@code Boolean} 对象，则返回 {@code true}。
     *
     * @param   obj   要比较的对象。
    ...
    /**
     * 如果且仅如果名为参数的系统属性存在并且等于字符串 {@code "true"}，则返回 {@code true}。（从 Java<small><sup>TM</sup></small> 平台 1.0.2 版开始，此字符串的测试不区分大小写。）系统属性可以通过 {@code System} 类定义的 {@code getProperty} 方法访问。
     * <p>
     * 如果没有指定名称的属性，或者指定的名称为空或 null，则返回 {@code false}。
     *
     * @param   name   系统属性名称。
     ...
     * @return  如果此对象表示的布尔值与参数相同，则返回零；如果此对象表示 true 而参数表示 false，则返回正值；如果此对象表示 false 而参数表示 true，则返回负值
    ...
    /**
     * 返回将逻辑 AND 操作符应用于指定的 {@code boolean} 操作数的结果。
     *
     * @param a 第一个操作数
     * @param b 第二个操作数
    ...
    /**
     * 返回将逻辑 OR 操作符应用于指定的 {@code boolean} 操作数的结果。
     *
     * @param a 第一个操作数
     * @param b 第二个操作数
    ...
    /**
     * 返回将逻辑 XOR 操作符应用于指定的 {@code boolean} 操作数的结果。
     *
     * @param a 第一个操作数
     * @param b 第二个操作数
