...
class InvokerBytecodeGenerator {
    ...
    /*
     * 注意：这些 load/store 方法使用 localsMap 来找到正确的索引！
     */
    ...
    /**
     * 发射一个隐式转换，该参数必须是给定的 pclass 类型。
     * 通常这是一个空操作，除非 pclass 是一个子字类型或不是 Object 或接口的引用。
     *
     * @param ptype 当前栈上的值类型
     * @param pclass 栈上需要的值类型
    ...
    /** 更新 localClasses 类型映射。如果信息已经存在，则返回 true。 */
    ...
    private void emitReferenceCast(Class<?> cls, Object arg) {
        Name writeBack = null;  // 用于写回结果的局部变量
    ...
    private void emitReturnInsn(BasicType type) {
        ...
        default:
            throw new InternalError("未知返回类型: " + type);
        }
        mv.visitInsn(opcode);
    }
    ...
    private boolean checkActualReceiver() {
        // 期望栈顶有一个 MethodHandle，实际接收者 MethodHandle 在槽 #0
        mv.visitInsn(Opcodes.DUP);
    ...
    private byte[] generateCustomizedCodeBytes() {
        ...
        if (lambdaForm.customized != null) {
            // 由于 LambdaForm 是为特定的 MethodHandle 定制的，因此可以将接收者 MethodHandle（在槽 #0）替换为嵌入的常量并使用它。
        ...
        for (int i = lambdaForm.arity; i < lambdaForm.names.length; i++) {
            ...
            switch (intr) {
                ...
                case NONE:
                    ...
                    break;
                default:
                    throw newInternalError("未知内在函数: "+intr);
            }
    ...
    void emitInvoke(Name name) {
        assert(!isLinkerMethodInvoke(name));  // 应该使用静态路径
        if (true) {
            // 推送接收者
    ...
    static boolean isStaticallyInvocable(Name name) {
        ...
        if (cls.getClassLoader() != MethodHandle.class.getClassLoader())
            return false;  // 不在引导类路径上
        if (ReflectUtil.isVMAnonymousClass(cls)) // 修复：一旦添加支持的API，切换到支持的API
            return false;
        ...
        if (!member.isPrivate() && VerifyAccess.isSamePackage(MethodHandle.class, cls))
            return true;   // 在 java.lang.invoke 包中
    ...
    static boolean isStaticallyNameable(Class<?> cls) {
        ...
        if (ReflectUtil.isVMAnonymousClass(cls)) // 修复：一旦添加支持的API，切换到支持的API
            return false;
        // 可以使用 VerifyAccess.isClassAccessible，但以下是一个安全的近似值
    ...
    void emitStaticInvoke(MemberName member, Name name) {
        ...
        if (refKind == REF_invokeSpecial) {
            // 为了通过验证器，我们需要在所有情况下将此转换为 invokevirtual
            assert(member.canBeStaticallyBound()) : member;
            refKind = REF_invokeVirtual;
        }

        if (member.getDeclaringClass().isInterface() && refKind == REF_invokeVirtual) {
            // 在接口中声明的 Object 方法可以由 JVM 解析为 invokevirtual 类型。
            // 需要将其转换回 invokeinterface 以通过验证并使调用按预期工作。
    ...
    void emitNewArray(Name name) throws InternalError {
        ...
        if (name.arguments.length == 0) {
            ...
            assert(emptyArray.getClass() == rtype);  // 精确类型
    ...
    /**
     * 检查 MemberName 是否是对类 {@code declaringClass} 中名为 {@code name} 的方法的调用。
     */
    ...
    /**
     * 检查 MemberName 是否是对 MethodHandle.invokeBasic 的调用。
     */
    private boolean isInvokeBasic(Name name) {
        if (name.function == null)
            return false;
        if (name.arguments.length < 1)
            return false;  // 必须有 MethodHandle 参数
        MemberName member = name.function.member();
    ...
    /**
     * 检查 MemberName 是否是对 MethodHandle.linkToStatic 等方法的调用。
     */
    ...
    /**
     * 检查第 i 个名称是否是对 MethodHandleImpl.selectAlternative 的调用。
     */
    private boolean isSelectAlternative(int pos) {
        // selectAlternative 用法：
        ...
        return nameRefersTo(name0, MethodHandleImpl.class, "selectAlternative") &&
               ...
               lambdaForm.lastUseIndex(name0) == pos+1; // t_{n} 是局部的：仅在 t_{n+1} 中使用
    }

    /**
     * 检查第 i 个名称是否是 GuardWithCatch 用法的开始。
     */
    private boolean isGuardWithCatch(int pos) {
        ...
        return nameRefersTo(name1, MethodHandleImpl.class, "guardWithCatch") &&
               ...
               lambdaForm.lastUseIndex(name1) == pos+2;   // t_{n+1} 是局部的：仅在 t_{n+2} 中使用
    }

    /**
     * 为 selectAlternative 用法生成字节码。
     *
     * 该模式如下（参见 MethodHandleImpl.makeGuardWithTest）：
     * <blockquote><pre>{@code
     *   Lambda(a0:L,a1:I)=>{
     *     t2:I=foo.test(a1:I);
    ...
    private Name emitSelectAlternative(Name selectAlternativeName, Name invokeBasicName) {
        ...
        emitPushArgument(selectAlternativeName, 1);  // 获取 selectAlternative 的第 2 个参数
        ...
        emitPushArgument(selectAlternativeName, 2);  // 获取 selectAlternative 的第 3 个参数
        emitAstoreInsn(receiver.index());  // 将 MH 存储在接收者槽中
        emitStaticInvoke(invokeBasicName);
    ...
    /**
      * 为 guardWithCatch 用法生成字节码。
      *
      * 该模式如下（参见 MethodHandleImpl.makeGuardWithCatch）：
      * <blockquote><pre>{@code
    ...
    private Name emitGuardWithCatch(int pos) {
        ...
        emitPushArguments(args, 1); // 跳过第 1 个参数：方法句柄
        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, MH, "invokeBasic", type.basicType().toMethodDescriptorString(), false);
    ...
    /**
     * 生成一个从 LF 调用者的返回语句。如果需要，结果类型将被转换为正确的返回类型。
     */
    ...
    private void emitPrimCast(Wrapper from, Wrapper to) {
        ...
        if (from.isSubwordOrInt()) {
            // 从 {byte,short,char,int} 转换为任何类型
            emitI2X(to);
        } else {
            // 从 {long,float,double} 转换为任何类型
            if (to.isSubwordOrInt()) {
                // 转换为 {byte,short,char,int}
                emitX2I(from);
                if (to.bitWidth() < 32) {
                    // 除了 int 以外的目标类型需要另一次转换
                    emitI2X(to);
                }
            } else {
                // 转换为 {long,float,double} - 这个过程比较繁琐
                boolean error = false;
                switch (from) {
                case LONG:
                    switch (to) {
    ...
    /**
     * 生成一个调用 interpretWithArguments 的 LambdaForm.vmentry 的字节码。
     */
    ...
    private byte[] generateLambdaFormInterpreterEntryPointBytes() {
        ...
        mv.visitInsn(Opcodes.SWAP);  // 交换 form 和数组；避免使用局部变量
    ...
    private byte[] generateNamedFunctionInvokerImpl(MethodTypeForm typeForm) {
        ...
        for (int i = 0; i < dstType.parameterCount(); i++) {
            ...
            if (dptype.isPrimitive()) {
                ...
                Wrapper srcWrapper = dstWrapper.isSubwordOrInt() ? Wrapper.INT : dstWrapper;  // 将子字节类型从 int 窄化
        ...
        if (rtype != void.class && rtype.isPrimitive()) {
            ...
            Wrapper dstWrapper = srcWrapper.isSubwordOrInt() ? Wrapper.INT : srcWrapper;  // 将子字节类型扩展到 int
            // 不允许布尔类型转换
            emitPrimCast(srcWrapper, dstWrapper);
    ...
    /**
     * 生成一个虚假的方法，该方法仅加载一些字符串常量。这是为了将常量放入常量池中，以便于调试。
     */
