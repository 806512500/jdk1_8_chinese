...
/*
 * 版权所有 (c) 2011, 2012, Oracle 和/或其附属公司。保留所有权利。
...
/**
 * 本类仅包含方法句柄实现的内部静态名称。
 * 用法:  {@code import static java.lang.invoke.MethodHandleStatics.*}
 * @author John Rose, JSR 292 EG
 */
/*non-public*/ class MethodHandleStatics {

    private MethodHandleStatics() { }  // 不允许实例化
    ...
    static {
        ...
        if (CUSTOMIZE_THRESHOLD < -1 || CUSTOMIZE_THRESHOLD > 127) {
            throw newInternalError("CUSTOMIZE_THRESHOLD 应在 [-1...127] 范围内");
        }
    }

    /** 检查是否有任何调试开关已打开。
     *  如果是这种情况，执行额外的检查或保存额外的信息是合理的。
     */
    ...
    /** 传播未检查的异常和错误，但包装任何已检查的异常并抛出。 */
    /*non-public*/ static Error uncaughtException(Throwable ex) {
        if (ex instanceof Error)  throw (Error) ex;
        if (ex instanceof RuntimeException)  throw (RuntimeException) ex;
        throw newInternalError("未捕获的异常", ex);
    }
    static Error NYI() {
