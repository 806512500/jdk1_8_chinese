...
class DirectMethodHandle extends MethodHandle {
    ...
    MethodHandle copyWith(MethodType mt, LambdaForm lf) {
        assert(this.getClass() == DirectMethodHandle.class);  // 子类必须重写
    ...
    private static LambdaForm makePreparedLambdaForm(MethodType mtype, int which) {
        ...
        assert(outArgs[outArgs.length-1] == names[GET_MEMBER]);  // 查看，移动参数！
    ...
    /*non-public*/ static long fieldOffset(Object accessorObj) {
        // 注意：我们返回一个 long，因为这是 Unsafe.getObject 喜欢的。
    ...
    /*non-public*/ static Object checkBase(Object obj) {
        ...
        // 这在 DirectMethodHandle.make 中进行了验证。
        // 因此，唯一剩下的检查是 null。
        // 由于这个检查 *不* 被 Unsafe.getInt
        // 和它的兄弟方法保证，我们需要在这里进行显式的检查。
    ...
    private static byte
            ...
            AF_PUTSTATIC_INIT  = 5,
            AF_LIMIT           = 6;
    // 使用Wrapper枚举不同的字段类型，并为检查引用添加一个额外的案例。
    private static int
    ...
    /**
     * 创建一个可以访问给定字段的LambdaForm。
     * 在所有具有相同basicType和refKind的字段之间缓存并共享此结构。
     */
    ...
    private static LambdaForm preparedFieldLambdaForm(byte formOp, boolean isVolatile, Class<?> ftype) {
        ...
        ACCESSOR_FORMS[afIndex] = lform;  // 不使用CAS
        return lform;
    }
    ...
    private static LambdaForm makePreparedFieldLambdaForm(byte formOp, boolean isVolatile, int ftypeKind) {
        ...
        mtype = mtype.basicType();  // 擦除short到int等
        if (!isStatic)
        ...
        final int F_OFFSET  = nameCursor++;  // 静态偏移或字段偏移。
        final int OBJ_CHECK = (OBJ_BASE >= 0 ? nameCursor++ : -1);
        final int INIT_BAR  = (needsInit ? nameCursor++ : -1);
        final int PRE_CAST  = (needsCast && !isGetter ? nameCursor++ : -1);
        final int LINKER_CALL = nameCursor++;
        final int POST_CAST = (needsCast && isGetter ? nameCursor++ : -1);
        final int RESULT    = nameCursor-1;  // 要么是调用，要么是转换
        Name[] names = arguments(nameCursor - ARG_LIMIT, mtype.invokerType());
        if (needsInit)
