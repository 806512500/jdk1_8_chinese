 ...
 * 参数列表中的每个名称引用必须引用同一 lambda 中先前定义的名称。
 * Lambda 的结果为 void 当且仅当其结果索引为 -1。
 * 如果一个临时值的类型为 "V"，则它不能成为 NameRef 的主题，即使它拥有一个编号。
 * 请注意，所有引用类型都被擦除为 "L"，表示 {@code Object}。
 * 所有子字节类型（boolean, byte, short, char）都被擦除为 "I"，表示 {@code int}。
 * 其他类型表示通常的原始类型。
 * <p>
 ...
 * 只有在不改变擦除类型的情况下才允许转换。
 * <ul>
 * <li>L = Object: 可以自由地使用转换来转换进入和离开引用类型
 * <li>I = int: 子字节类型在传递为参数时会被强制缩小（参见 {@code explicitCastArguments}）
 * <li>J = long: 没有隐式转换
 * <li>F = float: 没有隐式转换
 * <li>D = double: 没有隐式转换
 * <li>V = void: 函数结果可以为 void 当且仅当其 Name 类型为 "V"
 * </ul>
 ...
 *     == 通用的 unary insertArgument 组合调用器
 ...
 *     == 通用的 unary filterArgument 组合调用器
 * (a0:L, a1:L)=>{ ...(与上一个示例相同)...
 *                 t5:L = MethodHandle#invoke(t4, t3, a1); t5 }
 *     == 通用的 unary/unary foldArgument 组合调用器
 * (a0:L, a1:I)=>{ t2:I = identity(long).asType((int)->long)(a1); t2 }
 *     == 执行 i2l 的 identity 方法句柄调用器
 * (a0:L, a1:L)=>{ t2:L = BoundMethodHandle#argument(a0);
 *                 t3:L = Class#cast(t2,a1); t3 }
 *     == 执行 cast 的 identity 方法句柄调用器
 * }</pre></blockquote>
 * <p>
...
class LambdaForm {
    ...
    // 要么是 LambdaForm 缓存（由 LambdaFormEditor 管理），要么是未定制版本的链接（对于定制的 LF）
    ...
    private LambdaForm(String sig) {
        // 创建一个空白的 lambda 形式，返回一个常量零或 null。
        // 它用作管理类似形式的调用的模板，这些形式是非空的。
        // 仅从 getPreparedForm 调用。
    ...
    LambdaForm customize(MethodHandle mh) {
        ...
        if (COMPILE_THRESHOLD > 0 && isCompiled) {
            // 如果共享的 LambdaForm 已经被编译，那么也编译定制的版本。
    ...
    LambdaForm uncustomize() {
        ...
        if (COMPILE_THRESHOLD > 0 && isCompiled) {
            // 如果定制的 LambdaForm 已经被编译，那么也编译未定制的版本。
            uncustomizedForm.compileToBytecode();
        }
        return uncustomizedForm;
    }

    /** 重新编号和/或替换参数，使它们被内部化并按规范编号。
    ...
    /**
     * 检查所有嵌入的 Name 引用是否可以定位到此 lambda，
     * 并且在它们对应的定义之后正确排序。
     * <p>
    ...
    static boolean isValidSignature(String sig) {
        ...
        if (arity < 0)  return false;  // 必须是 *_* 的形式
     ...
     *
     * 编译后的 LFs 应该是可重用的。
     * 最大的问题是决定何时将名称拉入字节码，而不是从 MH 数据中加载重新化形式。
     *
     * 例如，asType 包装器可能需要在调用 MH 后执行类型转换。
     * 目标类型的转换可以作为常量放在 LF 本身中。这将强制转换类型
     * 被编译到 MH 的字节码和本机代码中。或者，目标类型的转换可以被擦除在 LF 中，
     * 并从 MH 数据中加载。（稍后，如果整个 MH 被内联，数据将流入内联实例的 LF，
     * 作为常量，最终结果将是最佳的转换。）
     *
     * 这种类型转换的擦除可以用于任何引用类型的使用。它也可以用于整个方法句柄。
     * 擦除方法句柄可能会留下一个 LF，该 LF 对于给定类型的任何 MH 都能正确执行，
     * 并从包含的 MH 数据中加载所需的 MH。（或者，擦除甚至可以擦除预期的 MT。）
     *
     * 对于直接 MH，目标的 MemberName 可以被擦除，并从包含的直接 MH 中加载。
     * 作为一个简单的例子，所有 int 值的非静态字段获取器的 LF 将对其输入参数执行类型转换
     * （到从 MemberName 导出的非常量基类型）并从输入对象中加载整数值
     * （在从 MemberName 导出的非常量偏移处）。这样的 MN 擦除的 LFs 可以在任何时候内联回优化代码，
     * 只要有一个常量的包含 DMH 可以从其数据中提供一个常量 MN。
     *
     * 这里的主要问题是保持 LFs 的通用性，同时确保热点将内联良好的实例。
     * “合理通用”意味着我们不会生成重复的字节码或机器代码版本，这些版本在优化形式上没有差异。
     * 重复的机器代码版本会有不必要的开销，包括（a）冗余的编译工作和（b）额外的 I$ 压力。
     * 为了控制重复的版本，我们需要准备好从 LFs 中擦除细节并将它们移动到 MH 数据中，
     * 只要这些细节与重要的优化无关。“重要”意味着实际热点的代码优化。
     *
     * 实现这一点可能需要通过用更专业的版本替换通用 LF 来动态拆分 MH，
     * 在同一 MH 上，如果（a）MH 频繁执行且（b）MH 不能
     * 被内联到包含的调用者中，例如 invokedynamic。
     *
     * 不再使用的编译后的 LFs 应该可以被 GC 回收。
     * 如果它们包含非 BCP 引用，它们应该与它们依赖的类加载器正确地互连。
     * 这可能意味着可重用的编译后的 LFs 将在相关的类加载器上进行索引（表化），
     * 或者缓存它们的表将具有弱链接。
     */

    /**
     * 使此 LF 可以直接作为 MethodHandle 的一部分执行。
    ...
    private static void computeInitialPreparedForms() {
        // 查找所有预定义的调用者并将它们与规范的空 lambda 表单关联。
    ...
    // 将此设置为 false 以禁用此文件中定义的 interpret_L 方法的使用。
    private static final boolean USE_PREDEFINED_INTERPRET_METHODS = true;

    // 以下是一些预定义的精确调用者。系统必须为每个不同的签名构建
    // 一个单独的调用者。
    ...
    private void checkInvocationCounter() {
        if (COMPILE_THRESHOLD != 0 &&
            ...
            invocationCounter++;  // 无害的竞争
            if (invocationCounter >= COMPILE_THRESHOLD) {
                // 用此 LF 的字节码版本替换 vmentry。
                compileToBytecode();
            }
        }
    }
    Object interpretWithArgumentsTracing(Object... argumentValues) throws Throwable {
        ...
        if (invocationCounter < COMPILE_THRESHOLD) {
            int ctr = invocationCounter++;  // 无害的竞争
    ...
    static class NamedFunction {
        ...
        // 以下是一些预定义的 NamedFunction 调用者。系统必须为每个不同的签名构建一个单独的调用者。
        /** void 返回类型调用者。 */
        @Hidden
        ...
        private static MethodHandle computeInvoker(MethodTypeForm typeForm) {
            ...
            MemberName invoker = InvokerBytecodeGenerator.generateNamedFunctionInvoker(typeForm);  // 这可能需要一段时间
            mh = DirectMethodHandle.make(invoker);
            MethodHandle mh2 = typeForm.cachedMethodHandle(MethodTypeForm.MH_NF_INV);
            if (mh2 != null)  return mh2;  // 无害的竞争
    ...
    static final class Name {
        ...
        final Object constraint;  // 如果不为 null，则为额外的类型信息
        ...
        Name replaceName(Name oldName, Name newName) {  // FIXME: 统一使用 replaceNames
        ...
        /** 在此 Name 的参数中，成对地替换 oldNames[i] 为 newNames[i]。
         *  仅限于 {@code start<=i<end}。返回可能已更改的自身。
         */
        ...
        eachArg:
            for (int j = 0; j < arguments.length; j++) {
                if (arguments[j] instanceof Name) {
                    ...
                    int check = n.index;
                    // 无害检查以查看该对象是否已在 newNames 中：
                    if (check >= 0 && check < newNames.length && n == newNames[check])
                        continue eachArg;
                    // n 可能没有正确的索引: n != oldNames[n.index]。
    ...
    private static void createIdentityForms() {
        ...
        // 在单独的传递中执行此操作，以便 SimpleMethodHandle.make 可以看到这些表。
        for (BasicType type : BasicType.ALL_TYPES) {
            int ord = type.ordinal();
    ...
    // 以下 hack 是为了在执行此类的静态初始化时抑制 TRACE_INTERPRETER。
    // 过早地打开 TRACE_INTERPRETER 将导致在尝试跟踪 LambdaForm.<clinit> 期间发生的事件时发生堆栈溢出和其他不良行为。
