...
/** 对 LF 的转换。
 *  一个 lambda 形式编辑器可以从其基础 LF 派生新的 LF。
 *  编辑器可以缓存派生的 LF，这简化了它们底层字节码的重用。
 *  为了支持这种缓存，LF 有一个可选的指向其编辑器的指针。
 */
class LambdaFormEditor {
    ...
    static LambdaFormEditor lambdaFormEditor(LambdaForm lambdaForm) {
        // TO DO:  考虑在这里放置 intern 逻辑，以减少重复。
        // lambdaForm = findPreexistingEquivalent(lambdaForm)

        // 始终使用未定制的版本进行编辑。
        // 这有助于缓存，定制的 LambdaForms 重用 transformCache 字段来保持与未定制版本的链接。
        return new LambdaFormEditor(lambdaForm.uncustomize());
    }

    /** 缓存转换的描述，可能与转换的结果相关联。
     *  逻辑内容是一系列字节值，以 Kind.ordinal 值开头。
     *  该序列未终止，以不定数量的零字节结束。
    ...
    private static final class Transform extends SoftReference<LambdaForm> {
        ...
        private enum Kind {
            ...
            PERMUTE_ARGS
            // 可能会添加更多，例如 guard with test, catch exception, pointwise type conversions
        }
    ...
    private LambdaForm getInCache(Transform key) {
        assert(key.get() == null);
        // transformCache 可能是 null, Transform, Transform[], 或 ConcurrentHashMap。
    ...
    LambdaForm permuteArgumentsForm(int skip, int[] reorder) {
        assert(skip == 1);  // 仅跳过前导 MH 参数，即 names[0]
        ...
        if (nullPerm)  return lambdaForm;  // 不需要缓存
