 ...
 * <p>
 * 关于方法类型结构的实证讨论，参见 <a href="http://groups.google.com/group/jvm-languages/browse_thread/thread/ac9308ae74da9b7e/">
 * jvm-languages 上的“避免装箱”讨论线程</a>。
 * JDK 中大约有 2000 种不同的擦除方法类型。
 * 未擦除的类型数量大约是这个数字的 10 倍多。
 * 同时加载的类型不会超过这些类型的一半。
 * @author John Rose
 */
final class MethodTypeForm {
    ...
    final long primCounts;              // 打包的原生类型和双精度类型计数
    final MethodType erasedType;        // 规范的擦除类型
    final MethodType basicType;         // 规范的擦除类型，原生类型简化
    ...
    /** 返回唯一对应于此 MT 形式的类型。
     *  它可能具有任何原生返回值或参数，但除了 Object 之外不会有任何引用。
     */
     ...
     *  基本类型是擦除的（所有引用都是 Object），并且所有原生类型（除了 int, long, float, double, void）都被规范化为 int。
    ...
    public int parameterSlotCount() {                  // # 出参解释器槽位数量
        return unpack(argCounts, 2);
    }
    public int returnCount() {                         // = 0 (V), 或 1
        return unpack(argCounts, 1);
    }
    public int returnSlotCount() {                     // = 0 (V), 2 (J/D), 或 1
    ...
    public int primitiveReturnCount() {                // = 0 (obj), 或 1
        return unpack(primCounts, 1);
    }
    public int longPrimitiveReturnCount() {            // = 1 (J/D), 或 0
    ...
    /** 用于 {@link #canonicalize(java.lang.Class, int)} 的代码。
     * ERASE 表示将每个引用更改为 {@code Object}。
     * WRAP 表示将原始类型（包括 {@code void}）转换为相应的包装类型。UNWRAP 表示 WRAP 的逆操作。
     * INTS 表示将所有非 void 原始类型转换为 int 或 long，根据大小。LONGS 表示将所有非 void 原始类型转换为 long，无论大小。
     * RAW_RETURN 表示将类型（假设为返回类型）转换为 int，如果它小于 int 或为 void。
     */
    public static final int NO_CHANGE = 0, ERASE = 1, WRAP = 2, UNWRAP = 3, INTS = 4, LONGS = 5, RAW_RETURN = 6;

    /** 规范化给定方法类型中的类型。
     * 如果任何类型发生变化，内部化新类型并返回它。
     * 否则返回 null。
     */
