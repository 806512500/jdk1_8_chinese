...
/**
 * 该方法句柄的变体模拟了对预定义参数的调用指令。当句柄被创建时，JVM 会调度到正确的方法，而不是在调用时。
 *
 * 所有绑定的参数都封装在专用的物种中。
 */
/*非公开*/ abstract class BoundMethodHandle extends MethodHandle {

    /*非公开*/ BoundMethodHandle(MethodType type, LambdaForm form) {
        super(type, form);
    ...
    static BoundMethodHandle bindSingle(MethodType type, LambdaForm form, BasicType xtype, Object x) {
        ...
        try {
            ...
            default : throw newInternalError("意外的 xtype: " + xtype);
            }
        } catch (Throwable t) {
    ...
    @Override // 在超类中有一个默认的绑定器，仅适用于 'L' 类型
    /*非公开*/
     ...
     */
    /*非公开*/ abstract SpeciesData speciesData();

    /*非公开*/ static SpeciesData speciesData(LambdaForm form) {
    ...
    /*非公开*/ final Object arg(int i) {
        ...
        throw new InternalError("意外的类型: " + speciesData().typeChars+"."+i);
    }
    ...
    /*非公开*/ abstract BoundMethodHandle copyWith(MethodType mt, LambdaForm lf);
    /*非公开*/ abstract BoundMethodHandle copyWithExtendL(MethodType mt, LambdaForm lf, Object narg);
    /*非公开*/ abstract BoundMethodHandle copyWithExtendI(MethodType mt, LambdaForm lf, int    narg);
    /*非公开*/ abstract BoundMethodHandle copyWithExtendJ(MethodType mt, LambdaForm lf, long   narg);
    /*非公开*/ abstract BoundMethodHandle copyWithExtendF(MethodType mt, LambdaForm lf, float  narg);
    /*非公开*/ abstract BoundMethodHandle copyWithExtendD(MethodType mt, LambdaForm lf, double narg);

    //
    // 为关闭引导循环所需的具体 BMH 类
    //

    private  // 使其私有以强制用户首先访问外部类
    static final class Species_L extends BoundMethodHandle {
        ...
        /*非公开*/ static final SpeciesData SPECIES_DATA = new SpeciesData("L", Species_L.class);
        /*非公开*/ static BoundMethodHandle make(MethodType mt, LambdaForm lf, Object argL0) {
            return new Species_L(mt, lf, argL0);
        }
        @Override
        /*非公开*/ final BoundMethodHandle copyWith(MethodType mt, LambdaForm lf) {
            return new Species_L(mt, lf, argL0);
        }
        @Override
        /*非公开*/ final BoundMethodHandle copyWithExtendL(MethodType mt, LambdaForm lf, Object narg) {
            try {
        ...
        @Override
        /*非公开*/ final BoundMethodHandle copyWithExtendI(MethodType mt, LambdaForm lf, int narg) {
            try {
        ...
        @Override
        /*非公开*/ final BoundMethodHandle copyWithExtendJ(MethodType mt, LambdaForm lf, long narg) {
            try {
        ...
        @Override
        /*非公开*/ final BoundMethodHandle copyWithExtendF(MethodType mt, LambdaForm lf, float narg) {
            try {
        ...
        @Override
        /*非公开*/ final BoundMethodHandle copyWithExtendD(MethodType mt, LambdaForm lf, double narg) {
            try {
    ...
    /**
     * 具体 BMH 类型的元数据包装器。
     * 每个 BMH 类型对应给定的基本字段类型序列（LIJFD）。
     * 字段是不可变的；它们的值在对象构造时完全指定。
     * 每个 BMH 类型提供一个 getter 函数数组，这些函数可以在 lambda 形式中使用。
     * BMH 通过克隆较短的 BMH 并添加一个或多个新字段值来构造。
     * 最短的 BMH 没有字段；其类为 SimpleMethodHandle。
     * 即使看起来较短的 BMH 可以作为扩展它的较长 BMH 的超类型，BMH 物种之间也没有子类型关系。
     */
    static class SpeciesData {
        ...
        /*非公开*/ BasicType fieldType(int i) {
            return typeCodes[i];
        }
        /*非公开*/ char fieldTypeChar(int i) {
        ...
        /**
         * 返回一个包含 {@link LambdaForm.NamedFunction} 的 {@link LambdaForm.Name}，该函数表示绑定到通用调用者的 MH，该调用者反过来又转发到相应的 getter。
         */
    ...
    static class Factory {
        ...
        static Class<? extends BoundMethodHandle> getConcreteBMHClass(String types) {
            // CHM.computeIfAbsent 确保 generateConcreteBMHClass 仅针对每个键调用一次。
         ...
         *
         * {@code [[fields]]} 部分由类型签名中的每个字符对应的一个字段定义组成，遵循 {@link #makeFieldName} 中描述的命名模式。
         *
         * 例如，对于两个引用类型和一个整数类型的绑定值，具体的 BMH 子类将具有以下形状：
         *
         * <pre>
        ...
        static Class<? extends BoundMethodHandle> generateConcreteBMHClass(String types) {
            ...
            mv.visitInsn(DUP);
            // 加载 mt, lf
            mv.visitVarInsn(ALOAD, 0);  // 类型
            mv.visitVarInsn(ALOAD, 1);  // 形式
            // 加载工厂方法参数
            for (int i = 0, j = 0; i < types.length(); ++i, ++j) {
                // i 计数参数，j 计数对应的参数槽
                char t = types.charAt(i);
        ...
        static MethodHandle[] makeCtors(Class<? extends BoundMethodHandle> cbmh, String types, MethodHandle mhs[]) {
            ...
            if (types.equals(""))  return mhs;  // 为空的 BMH 类型的 hack
