...
/* package */ final class InnerClassLambdaMetafactory extends AbstractValidatingLambdaMetafactory {
    ...
    private final String[] argNames;                 // 生成的构造函数参数的名称
     ...
     * @param invokedType 由 VM 自动堆叠；调用方法的签名，包括预期的返回 lambda 对象的静态类型，以及 lambda 的捕获参数的静态类型。如果实现方法是实例方法，则调用签名中的第一个参数将对应于接收者。
     * @param samMethodName 转换为函数接口的方法的名称，表示为字符串。
     * @param samMethodType 转换为函数接口的方法的类型，表示为 MethodType。
     * @param implMethod 应在调用结果函数接口实例的方法时调用的实现方法（适当调整参数类型、返回类型和捕获参数）。
     * @param instantiatedMethodType 类型变量被从捕获站点实例化后的主函数接口方法的签名
     * @param isSerializable 是否使 lambda 可序列化？如果设置，则目标类型或附加 SAM 类型之一必须扩展 {@code Serializable}。
     * @param markerInterfaces lambda 对象应实现的附加接口。
     * @param additionalBridges 需要桥接到实现方法的附加签名的方法类型
     * @throws LambdaConversionException 如果元工厂协议的任何不变量被违反
     */
    public InnerClassLambdaMetafactory(MethodHandles.Lookup caller,
        ...
        if (parameterCount > 0) {
            ...
            argNames = argDescs = EMPTY_STRING_ARRAY;
                ...
                /**
     * 构建 CallSite。生成一个实现功能接口的类文件，定义该类，如果没有任何参数，则创建该类的一个实例，该实例是 CallSite 将返回的实例，否则，生成将调用该类构造函数的句柄。
     *
     * @return 一个 CallSite，当被调用时，将返回一个功能接口的实例
     * @throws ReflectiveOperationException
     * @throws LambdaConversionException 如果找不到正确形成的功能接口
     */
    @Override
        ...
        if (invokedType.parameterCount() == 0) {
            final Constructor<?>[] ctrs = AccessController.doPrivileged(
                ...
                public Constructor<?>[] run() {
                    ...
                    if (ctrs.length == 1) {
                        // Lambda 实现内部类的构造函数是私有的，在创建常量唯一实例之前，设置其可访问（仅对我们）
            ...
            if (ctrs.length != 1) {
                throw new LambdaConversionException("期望 " + innerClass.getCanonicalName() + " 有一个 lambda 构造函数，但得到了 " + ctrs.length);
            }
            ...
            try {
                ...
                throw new LambdaConversionException("实例化 lambda 对象时出现异常", e);
            }
        } else {
            try {
                ...
                throw new LambdaConversionException("查找构造函数时出现异常", e);
            }
        }
    }

    /**
     * 生成一个实现功能接口的类文件，定义并返回该类。
     *
     * @implNote 生成的类不包含 SAM 方法上可能存在的异常签名信息。
     * 这是为了减少类文件的大小，而且这样做是无害的，因为检查异常会被擦除，
     * 没有人会用这个类文件进行编译，我们不对 lambda 对象的反射属性做出任何保证。
     *
     * @return 一个实现功能接口的类
     * @throws LambdaConversionException 如果找不到正确形成的功能接口
     */
    ...
    /**
     * 该类生成一个方法体，该方法体调用 lambda 实现方法，并根据需要转换参数类型。
     */
