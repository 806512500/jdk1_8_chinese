...
/**
 * 抽象实现了一个提供参数展开和输入验证的 lambda 元工厂。
 *
...
/* package */ abstract class AbstractValidatingLambdaMetafactory {
    ...
    final MethodType instantiatedMethodType;  // 实例化后擦除的功能接口方法类型 "(Integer)Object"
    final boolean isSerializable;             // 返回的实例是否应可序列化
    final Class<?>[] markerInterfaces;        // 需要实现的额外标记接口
    final MethodType[] additionalBridges;     // 需要桥接到实现方法的额外方法签名
     ...
     * @param invokedType 由 VM 自动堆栈；调用方法的签名，包括返回的 lambda 对象的预期静态类型，
     *                    以及 lambda 的捕获参数的静态类型。如果实现方法是实例方法，则调用签名中的
     *                    第一个参数将对应于接收者。
     * @param samMethodName 功能接口中 lambda 或方法引用要转换的方法的名称，表示为字符串。
     * @param samMethodType 功能接口中 lambda 或方法引用要转换的方法的类型，表示为 MethodType。
     * @param implMethod 当调用结果功能接口实例的方法时应调用的实现方法
     *                   （根据需要适配参数类型、返回类型，并调整捕获参数）。
     * @param instantiatedMethodType 类型变量被从捕获站点实例化后的主功能接口方法的签名
     * @param isSerializable lambda 是否应可序列化？如果设置，目标类型或额外的 SAM 类型之一
     *                       必须扩展 {@code Serializable}。
    ...
    /**
     * 检查元工厂参数是否有错误
     * @throws LambdaConversionException 如果存在不正确的转换
     */
    void validateMetafactoryArgs() throws LambdaConversionException {
        ...
        // 如果是实例方法：第一个捕获参数（接收者）必须是实现方法所在类的子类型
        final int capturedStart;
        final int samStart;
        if (implIsInstanceMethod) {
            final Class<?> receiverClass;

            // 实现是一个实例方法，调整捕获变量和 SAM 参数中的接收者
            if (capturedArity == 0) {
                // 接收者是函数参数
                capturedStart = 0;
                samStart = 1;
     ...
     * @param strict 如果为 true，进行严格检查，否则允许 fromType 可能是参数化类型
