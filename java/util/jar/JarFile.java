...
/**
 * <code>JarFile</code> 类用于从任何可以用 <code>java.io.RandomAccessFile</code> 打开的文件中读取 jar 文件的内容。
 * 它扩展了 <code>java.util.zip.ZipFile</code> 类，支持读取可选的 <code>Manifest</code> 条目。该 <code>Manifest</code> 可用于指定关于 jar 文件及其条目的元信息。
 *
 * <p>除非另有说明，向此类的构造函数或方法传递 <tt>null</tt> 参数将导致抛出 {@link NullPointerException}。
 *
 * 如果在打开已签名的 jar 文件时启用了验证标志，则文件内容将根据文件中嵌入的签名进行验证。请注意，验证过程不包括验证签名者的证书。调用者应检查 {@link JarEntry#getCodeSigners()} 的返回值，以进一步确定签名是否可信。
 *
...
class JarFile extends ZipFile {
    ...
    /**
     * 创建一个新的 <code>JarFile</code> 以从指定的文件 <code>name</code> 读取。如果该文件已签名，则将验证 <code>JarFile</code>。
     * @param name 要打开以读取的 jar 文件的名称
     ...
     * @param name 要打开以读取的 jar 文件的名称
     * @param verify 如果文件已签名，是否验证 jar 文件。
     * @throws IOException 如果发生 I/O 错误
     * @throws SecurityException 如果 SecurityManager 拒绝访问文件
     */
    ...
    /**
     * 创建一个新的 <code>JarFile</code> 以从指定的 <code>File</code> 对象读取。如果该文件已签名，则将验证 <code>JarFile</code>。
     * @param file 要打开以读取的 jar 文件
     * @throws IOException 如果发生 I/O 错误
     * @throws SecurityException 如果 SecurityManager 拒绝访问文件
     */
    ...
    /**
     * 创建一个新的 <code>JarFile</code> 以在指定模式下从指定的 <code>File</code> 对象读取。模式参数必须是 <tt>OPEN_READ</tt> 或 <tt>OPEN_READ | OPEN_DELETE</tt>。
     *
     * @param file 要打开以读取的 jar 文件
     * @param verify 如果文件已签名，是否验证 jar 文件。
     * @param mode 打开文件的模式
                    ...
                    if (!jvInitialized) {
                        if (getManifestCount() == 1) {
                            ...
                            if (JarVerifier.debug != null) {
                                JarVerifier.debug.println("找到多个 MANIFEST.MF。将 JAR 文件视为未签名");
                            }
                        }
                    }
    ...
    /**
     * 返回给定条目名称的 <code>JarEntry</code> 或
     * 如果未找到则返回 <code>null</code>。
     *
     * @param name JAR 文件条目名称
     * @return 给定条目名称的 <code>JarEntry</code> 或
     *         如果未找到则返回 <code>null</code>。
     *
     * @throws IllegalStateException
     *         如果 JAR 文件已关闭，可能会抛出此异常
    ...
    /**
     * 返回给定条目名称的 <code>ZipEntry</code> 或
     * 如果未找到则返回 <code>null</code>。
     *
     * @param name JAR 文件条目名称
     * @return 给定条目名称的 <code>ZipEntry</code> 或
     *         如果未找到则返回 <code>null</code>
     *
     * @throws IllegalStateException
     *         如果 JAR 文件已关闭，可能会抛出此异常
    ...
    /*
     * 确保如果需要（即 JAR 看起来已签名）则创建 JarVerifier。这是为了快速检查，避免对未签名的 JAR 进行清单处理。
     */
            ...
            if (names != null) {
                for (int i = 0; i < names.length; i++) {
                    ...
                    if (name.endsWith(".DSA") ||
                        ...
                        name.endsWith(".SF")) {
                        // 假设由于找到了与签名相关的文件
                        // JAR 是已签名的，因此我们需要一个 JarVerifier 和 Manifest
                        getManifest();
                        return;
                    }
                }
    ...
    private void initializeVerifier() {
        ...
        try {
            ...
            if (names != null) {
                for (int i = 0; i < names.length; i++) {
                    ...
                    if (MANIFEST_NAME.equals(uname)
                        ...
                        if (e == null) {
                            throw new JarException("损坏的 JAR 文件");
                        }
                        if (mev == null) {
            ...
            // 如果解析任何块时出现错误，仅将 JAR 文件视为未签名
            jv = null;
            verify = false;
            if (JarVerifier.debug != null) {
                JarVerifier.debug.println("JAR 文件解析错误！");
                ex.printStackTrace();
            }
        }

        // 如果初始化验证器后没有任何签名内容，我们将其置为 null。

        if (jv != null) {

            jv.doneWithMeta();
            if (JarVerifier.debug != null) {
                JarVerifier.debug.println("完成元数据处理！");
            }

            if (jv.nothingToVerify()) {
                if (JarVerifier.debug != null) {
                    JarVerifier.debug.println("没有需要验证的内容！");
                }
                jv = null;
                verify = false;
            }
    ...
    private byte[] getBytes(ZipEntry ze) throws IOException {
        try (InputStream is = super.getInputStream(ze)) {
            ...
            if (uncompressedSize > MAX_ARRAY_SIZE) {
                throw new IOException("不支持的大小: " + uncompressedSize);
            }
        ...
        throws IOException
    {
        ...
        if (!jvInitialized) {
            initializeVerifier();
            jvInitialized = true;
            // 在调用 initializeVerifier 后，如果没有需要验证的内容，可能会将其置为 null
            if (jv == null)
                return super.getInputStream(ze);
        }
    ...
    */
     ...
     * {@code lastOcc} 和 {@code optoSft} 数组是预计算的
     * 不良字符和良好后缀移位。
     */
    ...
    /**
     * 在首次调用时，检查 JAR 文件是否有 Class-Path
     * 属性。在后续调用中是一个空操作。
     */
    ...
    private boolean isKnownNotToHaveSpecialAttributes() {
        // 优化掉我们提供的不包含 class-path 属性的 JAR 文件的清单扫描。如果这些
        // JAR 文件中的一个被更改以包含此类属性，则必须更改此代码。
        if (javaHome == null) {
