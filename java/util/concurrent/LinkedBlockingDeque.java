 ...
 *
 *
 * 由 Doug Lea 在 JCP JSR-166 专家组成员的帮助下编写，并根据以下网址解释的内容发布到公共领域：
...
/**
 * 基于链节点的可选有界 {@linkplain BlockingDeque 阻塞双端队列}。
 *
 * <p>可选的容量边界构造参数用于防止过度扩展。如果未指定容量，则默认为 {@link Integer#MAX_VALUE}。除非这会使双端队列超出容量，否则在每次插入时都会动态创建链节点。
 *
 * <p>大多数操作在常数时间内运行（忽略阻塞时间）。例外包括 {@link #remove(Object) remove}、
...
public class LinkedBlockingDeque<E>
    ...
    /*
     * 实现为由单个锁保护并使用条件管理阻塞的简单双向链表。
     *
     * 为了实现弱一致性迭代器，似乎我们需要保持从已出队的节点到所有节点的 GC 可达性。
     * 这会导致两个问题：
     * - 允许恶意迭代器导致无限制的内存保留
     * - 如果一个节点在存活时被提升到老年代，这会导致老节点与新节点之间的跨代链接，这对分代垃圾收集器来说很难处理，导致重复的全量收集。
     * 然而，只有未删除的节点需要从已出队的节点可达，且可达性不一定必须是 GC 理解的那种。我们使用将刚刚出队的节点链接到自身的技巧。
     * 这种自链接隐含地意味着跳转到“第一个”（对于 next 链接）或“最后一个”（对于 prev 链接）。
     */

    /*
     * 我们在这里有“菱形”多重接口/抽象类继承，这引入了歧义。我们经常希望结合 BlockingDeque 的 Javadoc 和 AbstractQueue 的实现，
     * 因此许多方法规范在这里被重复。
     */
    ...
    /**
     * 在不违反容量限制的情况下，将指定的元素插入到此双端队列的末尾。当使用容量受限的双端队列时，通常建议使用 {@link #offer(Object) offer} 方法。
     *
    ...
    /**
     * 检索并移除此双端队列表示的队列的头部。此方法与 {@link #poll poll} 的区别在于，如果此双端队列为空，它会抛出异常。
     *
    ...
    /**
     * 获取但不移除此双端队列表示的队列的头部。此方法与 {@link #peek peek} 的唯一区别是，
     * 如果此双端队列为空，则此方法会抛出异常。
     *
    ...
    /**
     * 返回此双端队列在不阻塞的情况下（在没有内存或资源限制的情况下）可以理想地接受的额外元素的数量。
    ...
    public int drainTo(Collection<? super E> c, int maxElements) {
        ...
        try {
            ...
            for (int i = 0; i < n; i++) {
                c.add(first.item);   // 按此顺序，以防 add() 抛出异常。
                unlinkFirst();
            }
            return n;
        } finally {
    ...
    /**
     * 从此双端队列中移除指定元素的第一个出现。如果双端队列不包含该元素，则不改变。
     ...
     * 更正式地说，如果且仅当此双端队列包含至少一个满足 {@code o.equals(e)} 的元素时，返回 {@code true}。
     *
     ...
     * TODO: 添加对更高效批量操作的支持。
     *
     * 我们不希望在每次迭代时都获取锁，但我们也要给其他线程机会与集合进行交互，
     * 尤其是在 count 接近容量时。
     */
...
//     }
    ...
    /**
     * 返回一个包含此双端队列中所有元素的数组，按正确顺序（从第一个元素到最后一个元素）。
     *
     * <p>返回的数组是“安全的”，即此双端队列中没有任何对此数组的引用。（换句话说，此方法必须分配一个新数组）。因此，调用者可以自由地修改返回的数组。
     *
     * <p>此方法作为基于数组和基于集合的API之间的桥梁。
     *
    ...
    /**
     * 返回一个包含此双端队列中所有元素的数组，按正确顺序；返回数组的运行时类型是指定数组的类型。如果双端队列适合指定的数组，则返回该数组。否则，将分配一个具有指定数组的运行时类型和此双端队列大小的新数组。
     *
     * <p>如果此双端队列适合指定的数组并且有剩余空间（即，数组的元素多于此双端队列），则紧跟在双端队列末尾的数组元素将被设置为 {@code null}。
     *
     * <p>像 {@link #toArray()} 方法一样，此方法作为基于数组和基于集合的API之间的桥梁。此外，此方法允许精确控制输出数组的运行时类型，并且在某些情况下，可以用来节省分配成本。
     *
     * <p>假设 {@code x} 是一个已知仅包含字符串的双端队列。以下代码可以用来将双端队列转储到一个新分配的 {@code String} 数组中：
     *
     *  <pre> {@code String[] y = x.toArray(new String[0]);}</pre>
     *
     * 注意，{@code toArray(new Object[0])} 在功能上与 {@code toArray()} 相同。
     *
     * @param a 如果足够大，则将双端队列的元素存储到此数组中；否则，为此目的分配一个相同运行时类型的新数组
     * @return 包含此双端队列中所有元素的数组
    ...
    /**
     * 从双端队列中原子地移除所有元素。调用此方法后，双端队列将为空。
     */
    public void clear() {
        final ReentrantLock lock = this.lock;
        lock.lock();
    ...
    /**
     * 返回一个按正确顺序遍历此双端队列元素的迭代器。元素将按从第一个（头部）到最后一个（尾部）的顺序返回。
     *
     * <p>返回的迭代器是
     * <a href="package-summary.html#Weakly"><i>弱一致性</i></a>。
     *
     * @return 按正确顺序遍历此双端队列元素的迭代器
     */
    ...
    /**
     * 返回一个按逆序遍历此双端队列元素的迭代器。元素将按从最后一个（尾部）到第一个（头部）的顺序返回。
     *
     * <p>返回的迭代器是
     * <a href="package-summary.html#Weakly"><i>弱一致性</i></a>。
     *
     * @return 按逆序遍历此双端队列元素的迭代器
     */
    ...
    private abstract class AbstractItr implements Iterator<E> {
        ...
        /**
         * nextItem 保存 item 字段，因为在 hasNext() 中声明一个元素存在后，即使该元素在调用 hasNext() 时正在被移除，也必须返回在锁下读取的 item（在 advance() 中）。
         */
        E nextItem;

        /**
         * 最近一次调用 next() 返回的节点。需要由 remove 使用。如果此元素被 remove 调用删除，则重置为 null。
         */
        ...
        private Node<E> succ(Node<E> n) {
            // 如果多个内部节点被移除，可能会出现以 null 或自链接结尾的已删除节点链。
            for (;;) {
                Node<E> s = nextNode(n);
                if (s == null)
                    return null;
     ...
     * @serialData 容量（int），后跟元素（每个都是 {@code Object}）的正确顺序，最后是一个 null
     */
