 ...
 *
 * <P>
 * 名称是属性的名称（如 "java.home"、"os.name" 等）。命名
 * 约定遵循分层属性命名约定。
 * 另外，名称末尾可以出现星号，星号前可以有 "."，或者单独出现，表示通配符匹配。例如："java.*" 和 "*" 表示通配符
 * 匹配，而 "*java" 和 "a*b" 则不表示。
 * <P>
 * 要授予的权限操作在构造函数中以包含一个或多个逗号分隔的关键字的字符串形式传递。可能的关键字是
 ...
 * 操作字符串在处理前转换为小写。
 * <P>
 * 在授予代码访问某些系统属性的权限时应谨慎。例如，授予访问 "java.home" 系统属性的权限会向潜在的恶意代码提供系统环境的敏感信息（Java 安装目录）。同样，授予访问
 * "user.name" 和 "user.home" 系统属性的权限会向潜在的恶意代码提供用户环境的敏感信息（用户的账户名和主目录）。
 *
...
public final class PropertyPermission extends BasicPermission {
    ...
    /**
     * 使用指定的名称创建一个新的 PropertyPermission 对象。
     * 名称是系统属性的名称，<i>actions</i> 包含一个逗号分隔的列表，列出了
     * 要授予的属性操作。可能的操作是 "read" 和 "write"。
     *
    ...
    /**
     * 检查此 PropertyPermission 对象是否 "隐含" 指定的权限。
     * <P>
     * 更具体地说，如果满足以下条件，此方法返回 true：
     * <ul>
     * <li> <i>p</i> 是 PropertyPermission 的实例，
     * <li> <i>p</i> 的操作是此对象操作的子集，且
     * <li> <i>p</i> 的名称由此对象的名称隐含。例如，"java.*" 隐含 "java.home"。
     * </ul>
     * @param p 要检查的权限。
     *
     * @return 如果指定的权限被此对象隐含，则返回 true，否则返回 false。
     */
    ...
    /**
     * 检查两个 PropertyPermission 对象是否相等。检查 <i>obj</i> 是否是
     * PropertyPermission，并且具有与此对象相同的名称和操作。
     * <P>
     * @param obj 要测试与此对象相等的对象。
     * @return 如果 obj 是 PropertyPermission，并且具有与此 PropertyPermission 对象相同的名称和操作，则返回 true。
     */
     ...
     * 返回此对象的哈希码值。
     * 使用的哈希码是此权限名称的哈希码，即，
     * <code>getName().hashCode()</code>，其中 <code>getName</code> 来自 Permission 超类。
     *
     * @return 此对象的哈希码值。
     */
    public int hashCode() {
    ...
    private static int getMask(String actions) {
        ...
        // 使用对象身份比较已知的内部字符串以提高性能（这些值在 JDK 中大量使用）。
     ...
     * 允许读取和写入操作，调用 <code>getActions</code>
     * 将返回字符串 "读取,写入"。
     *
     * @return 操作的规范字符串表示。
     */
    ...
    /**
     * WriteObject 被调用来将 PropertyPermission 的状态保存到流中。动作被序列化，超类负责名称。
     */
    private synchronized void writeObject(java.io.ObjectOutputStream s)
        throws IOException
    {
        // 写出动作。超类负责名称
        // 调用 getActions 以确保动作字段已初始化
        if (actions == null)
            getActions();
    ...
    /**
     * PropertyPermissionCollection 存储一组 PropertyPermission 权限。
     *
     * @see java.security.Permission
     * @see java.security.Permissions
     * @see java.security.PermissionCollection
        ...
        implements Serializable
    {
        ...
        /**
         * 向 PropertyPermissions 添加权限。哈希的键是名称。
         *
        ...
        public boolean implies(Permission permission) {
            ...
            // 策略：
            // 首先检查完全匹配。然后逐步向上查找名称，查找 a.b.* 的匹配项
            ...
            // 我们不必检查 "*"，因为已经在顶部检查过（all_allowed），所以直接返回 false
            return false;
        }
        ...
        public Enumeration<Permission> elements() {
            // 将 Map 值的 Iterator 转换为 Enumeration
            synchronized (this) {
                /**
         ...
         */
        /*
        * 将 perms 字段的内容作为 Hashtable 写出，以保持与早期版本的序列化兼容性。all_allowed 保持不变。
        */
        private void writeObject(ObjectOutputStream out) throws IOException {
            // 不调用 out.defaultWriteObject()

            // 将 perms 复制到一个 Hashtable 中
        ...
        /*
        * 读取一个 PropertyPermissions 的 Hashtable 并将其保存在 perms 字段中。读取 all_allowed。
        */
