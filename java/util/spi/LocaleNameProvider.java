...
/**
 * 一个抽象类，用于提供 {@link java.util.Locale Locale} 类的本地化名称。
 *
 * @since        1.6
 */
public abstract class LocaleNameProvider extends LocaleServiceProvider {
     ...
     * 如果返回的名称无法根据 <code>locale</code> 进行本地化（例如，提供者没有克罗地亚语的日语名称），
     * 则此方法返回 null。
     * @param languageCode 形式为两到八个介于 'a' (U+0061) 和 'z' (U+007A) 之间的小写字母的字符串
     * @param locale 所需的区域设置
     * @return 给定语言代码的指定区域设置的名称，如果不可用则返回 null。
     * @exception NullPointerException 如果 <code>languageCode</code> 或 <code>locale</code> 为 null
     * @exception IllegalArgumentException 如果 <code>languageCode</code> 不是两个或三个小写字母的形式，或者 <code>locale</code> 不是
     ...
     * 如果返回的名称无法根据 <code>locale</code> 进行本地化（例如，提供者没有西里尔文的日语名称），
     * 则此方法返回 null。默认实现返回 null。
     * @param scriptCode 形式为四个标题大小写字母（第一个字母是介于 'A' (U+0041) 和 'Z' (U+005A) 之间的大写字母，后跟三个介于 'a' (U+0061) 和 'z' (U+007A) 之间的字母）的字符串
     * @param locale 所需的区域设置
     * @return 给定脚本代码的指定区域设置的名称，如果不可用则返回 null。
     * @exception NullPointerException 如果 <code>scriptCode</code> 或 <code>locale</code> 为 null
     * @exception IllegalArgumentException 如果 <code>scriptCode</code> 不是四个标题大小写字母的形式，或者 <code>locale</code> 不是
     *     {@link java.util.spi.LocaleServiceProvider#getAvailableLocales()
     *     getAvailableLocales()} 返回的区域设置之一。
     ...
     * 如果返回的名称无法根据 <code>locale</code> 进行本地化（例如，提供者没有克罗地亚的日语名称），
     * 则此方法返回 null。
     * @param countryCode 形式为两个介于 'A' (U+0041) 和 'Z' (U+005A) 之间的大写字母或三个介于 '0' (U+0030) 和 '9' (U+0039) 之间的数字的字符串
     * @param locale 所需的区域设置
     * @return 给定国家代码的指定区域设置的名称，如果不可用则返回 null。
     * @exception NullPointerException 如果 <code>countryCode</code> 或 <code>locale</code> 为 null
     * @exception IllegalArgumentException 如果 <code>countryCode</code> 不是两个大写字母或三个数字的形式，或者 <code>locale</code> 不是
     *     {@link java.util.spi.LocaleServiceProvider#getAvailableLocales()
     *     getAvailableLocales()} 返回的区域设置之一。
     ...
     * 如果返回的名称无法根据 <code>locale</code> 进行本地化，则此方法返回 null。
     * @param variant 变体字符串
     * @param locale 所需的区域设置
     * @return 给定变体字符串的指定区域设置的名称，如果不可用则返回 null。
