...
class GZIPInputStream extends InflaterInputStream {
    ...
    /**
     * 读取未压缩的数据到字节数组中。如果 <code>len</code> 不为零，该方法将阻塞直到可以解压缩一些输入；否则，
     ...
     * <code>len</code> 为负数，或者 <code>len</code> 大于 <code>buf.length - off</code>
    ...
    /*
     * 读取 GZIP 成员尾部并返回 true 如果到达 eos，false 如果有更多（连接的 gzip 数据集）
     */
