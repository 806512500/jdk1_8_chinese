...
/**
 * 该类提供了使用流行的 ZLIB 压缩库进行通用解压缩的支持。ZLIB 压缩库最初是作为 PNG 图形标准的一部分开发的，不受专利保护。它在
...
class Inflater {
    ...
    /**
     * 创建一个新的解压缩器。如果参数 'nowrap' 为 true，则不会使用 ZLIB 头和校验字段。这提供了与 GZIP 和 PKZIP 使用的压缩格式的兼容性。
     * <p>
     * 注意：当使用 'nowrap' 选项时，还需要提供一个额外的“虚拟”字节作为输入。这是 ZLIB 本地库为了支持某些优化所必需的。
     *
    ...
    /**
     * 将预设字典设置为给定的字节数组。当 inflate() 返回 0 且 needsDictionary() 返回 true 表示需要预设字典时，应调用此方法。
     * 可以使用 getAdler() 方法获取所需字典的 Adler-32 值。
     * @param b 字典数据字节
     * @param off 数据的起始偏移量
     * @param len 数据的长度
    ...
    /**
     * 返回输入缓冲区中剩余的字节总数。这可以用于确定解压缩完成后输入缓冲区中仍剩余哪些字节。
    ...
    /**
     * 如果输入缓冲区中没有剩余数据，则返回 true。这可以用于确定是否应调用 #setInput 以提供更多输入。
     * @return 如果输入缓冲区中没有剩余数据，则返回 true
     */
    ...
    /**
     * 将字节解压缩到指定的缓冲区。返回实际解压缩的字节数。返回值为 0 表示应调用 needsInput() 或 needsDictionary() 以确定是否需要更多输入数据或预设字典。
     * 在后一种情况下，可以使用 getAdler() 获取所需的字典的 Adler-32 值。
     * @param b 用于解压缩数据的缓冲区
     * @param off 数据的起始偏移量
     * @param len 最大解压缩字节数
     * @return 实际解压缩的字节数
    ...
    /**
     * 关闭解压缩器并丢弃任何未处理的输入。
     * 当解压缩器不再使用时应调用此方法，但也会在 finalize() 方法中自动调用。一旦调用此方法，解压缩器对象的行为将变得不确定。
     */
    public void end() {
        synchronized (zsRef) {
