...
/**
 * 表示一个接受一个 {@code long} 类型参数的布尔值函数。这是 {@link Predicate} 的 {@code long} 类型特化版本。
 *
 * <p>这是一个 <a href="package-summary.html">函数式接口</a>，其函数方法是 {@link #test(long)}。
 *
 * @see Predicate
 * @since 1.8
 */
...
public interface LongPredicate {
     ...
     *
     * <p>在评估任一谓词过程中抛出的任何异常都会传递给调用者；如果评估此谓词时抛出异常，则不会评估 {@code other} 谓词。
     *
     * @param other 将与本谓词进行逻辑 AND 操作的谓词
     ...
     * <p>在评估任一谓词过程中抛出的任何异常都会传递给调用者；如果评估此谓词时抛出异常，则不会评估 {@code other} 谓词。
     *
     * @param other 将与本谓词进行逻辑 OR 操作的谓词
