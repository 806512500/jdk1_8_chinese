...
/**
 * 在内存中使用循环缓冲区存储请求的 <tt>Handler</tt>。
 * <p>
 * 通常，此 <tt>Handler</tt> 仅将传入的 <tt>LogRecords</tt> 存储在其内存缓冲区中，并丢弃较早的记录。这种缓冲非常便宜，避免了格式化成本。在某些触发条件下，<tt>MemoryHandler</tt> 会将其当前缓冲区内容推送到目标 <tt>Handler</tt>，后者通常会将这些内容发布到外部世界。
 * <p>
 * 触发缓冲区推送主要有三种模型：
 * <ul>
 * <li>
 * 传入的 <tt>LogRecord</tt> 的类型大于预定义的级别，即 <tt>pushLevel</tt>。 </li>
 * <li>
 * 外部类显式调用 <tt>push</tt> 方法。 </li>
 * <li>
 * 子类覆盖 <tt>log</tt> 方法并扫描每个传入的 <tt>LogRecord</tt>，如果记录符合某些标准，则调用 <tt>push</tt>。 </li>
 * </ul>
 * <p>
 * <b>配置：</b>
 * 默认情况下，每个 <tt>MemoryHandler</tt> 使用以下 <tt>LogManager</tt> 配置属性进行初始化，其中 <tt>&lt;handler-name&gt;</tt> 是处理程序的完全限定类名。
 * 如果属性未定义（或具有无效值），则使用指定的默认值。如果没有定义默认值，则抛出 <tt>RuntimeException</tt>。
 * <ul>
...
public class MemoryHandler extends Handler {
    ...
    // 从 LogManager 属性和/或类 javadoc 中指定的默认值配置 MemoryHandler 的私有方法。
    ...
    /**
     * 创建一个 <tt>MemoryHandler</tt> 并基于 <tt>LogManager</tt> 配置属性进行配置。
     */
     ...
     * <p>
     * <tt>MemoryHandler</tt> 基于 <tt>LogManager</tt> 属性（或其默认值）进行配置，但使用给定的 <tt>pushLevel</tt> 参数和缓冲区大小参数。
     *
     ...
     * <p>
     * 如果存在 <tt>Filter</tt>，则调用其 <tt>isLoggable</tt> 方法检查给定的日志记录是否可记录。如果不是，则返回。否则，将给定的记录复制到内部循环缓冲区中。然后将记录的级别属性与 <tt>pushLevel</tt> 进行比较。如果给定的级别大于或等于 <tt>pushLevel</tt>，则调用 <tt>push</tt> 将所有缓冲记录写入目标输出 <tt>Handler</tt>。
     *
    ...
    /**
     * 在目标 <tt>Handler</tt> 上执行刷新操作。
     * <p>
     * 注意，当前 <tt>MemoryHandler</tt>
     * 缓冲区的内容 <b>不会</b> 被写出。这需要一个 "push" 操作。
     */
    @Override
    public void flush() {
        target.flush();
     ...
     *
     * @exception  SecurityException  如果存在安全管理器，并且调用者没有 <tt>LoggingPermission("control")</tt> 权限。
     */
    @Override
    ...
    /**
     * 设置 <tt>pushLevel</tt>。在 <tt>LogRecord</tt> 被复制到内部缓冲区后，
     * 如果其级别大于或等于 <tt>pushLevel</tt>，则将调用 <tt>push</tt>。
     *
    ...
    /**
     * 检查此 <tt>Handler</tt> 是否实际上会将给定的
     * <tt>LogRecord</tt> 记录到其内部缓冲区。
     * <p>
     * 此方法检查 <tt>LogRecord</tt> 是否具有适当的级别以及
     * 是否满足任何 <tt>Filter</tt>。但是它 <b>不会</b>
     * 检查 <tt>LogRecord</tt> 是否会导致缓冲区内容的 "push"。如果 <tt>LogRecord</tt> 为 null，它将返回 false。
     * <p>
