 ...
 * 如果存在值，{@code isPresent()} 将返回 {@code true} 并且
 * {@code getAsDouble()} 将返回该值。
 *
 * <p>提供了依赖于包含值存在或不存在的其他方法，例如 {@link #orElse(double) orElse()}
 * （如果值不存在则返回默认值）和
...
public final class OptionalDouble {
    ...
    /**
     * 返回一个空的 {@code OptionalDouble} 实例。此 OptionalDouble 没有值存在。
     *
     * @apiNote 尽管可能很诱人，但避免通过与 {@code Option.empty()} 返回的实例
     * 进行 {@code ==} 比较来测试对象是否为空。没有保证它是单例的。
     ...
     *
     * @param value 要存在的值
     * @return 一个包含值的 {@code OptionalDouble}
     */
    ...
    /**
     * 如果存在值返回 {@code true}，否则返回 {@code false}。
     *
     * @return 如果存在值返回 {@code true}，否则返回 {@code false}
     */
     ...
     * @throws NullPointerException 如果值存在且 {@code consumer} 为
     * null
     */
    ...
    /**
     * 如果存在值返回该值，否则调用 {@code other} 并返回该调用的结果。
     *
     * @param other 一个 {@code DoubleSupplier}，如果不存在值则返回其结果
     * @return 如果存在值则返回该值，否则返回 {@code other.getAsDouble()} 的结果
     * @throws NullPointerException 如果值不存在且 {@code other} 为
     * null
     */
    ...
    /**
     * 指示另一个对象是否“等于”此 OptionalDouble。如果满足以下条件，则认为另一个对象相等：
     * <ul>
     * <li>它也是一个 {@code OptionalDouble} 并且；
     * <li>两个实例都没有值存在，或者；
     * <li>存在的值通过 {@code Double.compare() == 0} 认为“相等”。
     * </ul>
     *
     * @param obj 要测试相等性的对象
     * @return 如果另一个对象“等于”此对象则返回 {code true}，否则返回 {@code false}
     */
    @Override
