...
/**
 * 一个支持顺序和并行聚合操作的原始 long 类型元素序列。这是 {@link Stream} 的 long 原始类型特化。
 *
 * <p>以下示例说明了使用 {@link Stream} 和 {@link LongStream} 进行的聚合操作，计算红色小部件的重量总和：
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * 有关流、流操作、流管道和并行性的更多规范，请参阅 {@link Stream} 的类文档和 <a href="package-summary.html">java.util.stream</a> 的包文档。
 *
 * @since 1.8
 * @see Stream
...
public interface LongStream extends BaseStream<Long, LongStream> {
    ...
    /**
     * 返回一个由应用给定函数到此流元素的结果组成的对象值 {@code Stream}。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">中间操作</a>。
     *
     * @param <U> 新流的元素类型
    ...
    /**
     * 返回一个由应用给定函数到此流元素的结果组成的 {@code IntStream}。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">中间操作</a>。
     *
     * @param mapper 应用于每个元素的 <a href="package-summary.html#NonInterference">非干扰的</a>、
    ...
    /**
     * 返回一个由应用给定函数到此流元素的结果组成的 {@code DoubleStream}。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">中间操作</a>。
     *
     * @param mapper 应用于每个元素的 <a href="package-summary.html#NonInterference">非干扰的</a>、
    ...
    /**
     * 返回一个由将提供的映射函数应用于每个元素生成的映射流的内容替换每个元素的结果组成的新流。每个映射流在将其内容放入此流后将被 {@link java.util.stream.BaseStream#close() 关闭}。（如果映射流为 {@code null}，则使用空流代替。）
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">中间操作</a>。
     *
     * @param mapper 应用于每个元素的 <a href="package-summary.html#NonInterference">非干扰的</a>、
     *               <a href="package-summary.html#Statelessness">无状态的</a> 函数，生成包含新值的 {@code LongStream}
     * @return 新的流
     ...
     * 操作</a>。
     *
     * <p>对于并行流管道，操作可能在元素由上游操作提供时的任何时间和任何线程中调用。如果操作修改了共享状态，
     * 它负责提供所需的同步。
     *
     * @apiNote 此方法主要用于支持调试，您希望在管道中的某个点看到元素的流动情况：
     * <pre>{@code
     ...
     *
     * @apiNote
     * 虽然在顺序流管道中 {@code limit()} 通常是一个廉价的操作，但在有序并行管道中可能会非常昂贵，
     * 特别是对于较大的 {@code maxSize} 值，因为 {@code limit(n)} 必须返回不仅仅是任意的 <em>n</em> 个元素，
     * 而是按遇到顺序的 <em>前 n</em> 个元素。使用无序的流源（如 {@link #generate(LongSupplier)}) 或
     * 通过 {@link #unordered()} 移除排序约束，可能会在并行管道中显著提高 {@code limit()} 的速度，
     * 如果您的情况允许的话。如果需要与遇到顺序一致，并且在并行管道中使用 {@code limit()} 时性能较差或内存使用率不佳，
     ...
     *
     * @apiNote
     * 虽然在顺序流管道中 {@code skip()} 通常是一个廉价的操作，但在有序并行管道中可能会非常昂贵，
     * 特别是对于较大的 {@code n} 值，因为 {@code skip(n)} 必须跳过不仅仅是任意的 <em>n</em> 个元素，
     * 而是按遇到顺序的 <em>前 n</em> 个元素。使用无序的流源（如 {@link #generate(LongSupplier)}) 或
     * 通过 {@link #unordered()} 移除排序约束，可能会在并行管道中显著提高 {@code skip()} 的速度，
     * 如果您的情况允许的话。如果需要与遇到顺序一致，并且在并行管道中使用 {@code skip()} 时性能较差或内存使用率不佳，
     * 使用 {@link #sequential()} 切换到顺序执行可能会提高性能。
     *
     * @param n 要跳过的前导元素数量
     * @return 新的流
     ...
     * 操作可能在库选择的任何时间和任何线程中执行。如果操作访问了共享状态，它负责提供所需的同步。
     *
     * @param action 一个 <a href="package-summary.html#NonInterference">
     *               不干扰</a> 的操作，对元素执行
     */
     ...
     * <a href="package-summary.html#Associativity">结合性</a>的累加函数，并返回归约值。这等价于：
     * <pre>{@code
     ...
     * 但不受限于顺序执行。
     *
     * <p>初始值{@code identity}必须是累加函数的恒等值。这意味着对于所有{@code x}，
     * {@code accumulator.apply(identity, x)}等于{@code x}。累加函数必须是
     * <a href="package-summary.html#Associativity">结合性</a>的函数。
     *
     ...
     * }</pre>
     *
     * <p>虽然这可能看起来比在循环中简单地修改运行总计更迂回，但归约操作可以更优雅地并行化，而不需要额外的同步，并大大减少了数据竞争的风险。
     *
     ...
     * <a href="package-summary.html#Associativity">结合性</a>的累加函数，并返回一个描述归约值的
     * {@code OptionalLong}，如果有的话。这等价于：
     * <pre>{@code
     ...
     * 但不受限于顺序执行。
     *
     * <p>累加函数必须是<a href="package-summary.html#Associativity">结合性</a>的函数。
     *
     * <p>这是一个<a href="package-summary.html#StreamOps">终端操作</a>。
     *
     * @param op 一个<a href="package-summary.html#Associativity">结合性</a>、
    ...
    /**
     * 对此流的元素执行<a href="package-summary.html#MutableReduction">可变归约</a>操作。可变归约是指归约值是一个可变的结果容器，
     * 例如一个{@code ArrayList}，元素是通过更新结果的状态而不是替换结果来合并的。这产生的结果等价于：
     * <pre>{@code
     ...
     * }</pre>
     *
     * <p>像{@link #reduce(long, LongBinaryOperator)}一样，{@code collect}操作可以在并行执行时不需要额外的同步。
     *
     * <p>这是一个<a href="package-summary.html#StreamOps">终端操作</a>。
     *
     * @param <R> 结果的类型
     * @param supplier 一个创建新结果容器的函数。对于并行执行，此函数可能被调用多次，且每次必须返回一个新值。
    ...
    long sum();
                ...
                /**
     * 返回一个描述此流中最小元素的 {@code OptionalLong}，如果此流为空，则返回一个空的可选值。这是一个
     * <a href="package-summary.html#Reduction">归约</a> 的特例，等价于：
     * <pre>{@code
    ...
    /**
     * 返回一个描述此流中最大元素的 {@code OptionalLong}，如果此流为空，则返回一个空的可选值。这是一个
     * <a href="package-summary.html#Reduction">归约</a> 的特例，等价于：
     * <pre>{@code
    ...
    /**
     * 返回此流中的元素数量。这是一个 <a href="package-summary.html#Reduction">归约</a> 的特例，等价于：
     * <pre>{@code
     *     return map(e -> 1L).sum();
     * }</pre>
     *
     ...
     * <a href="package-summary.html#Reduction">归约</a> 的特例。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">终结操作</a>。
     *
    ...
    /**
     * 返回一个描述此流中元素各种汇总数据的 {@code LongSummaryStatistics}。这是一个
     * <a href="package-summary.html#Reduction">归约</a> 的特例。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">终结操作</a>。
     *
    ...
    /**
     * 返回此流中是否有任何元素匹配所提供的谓词。如果确定结果不需要评估所有元素，则可能不会对所有元素评估谓词。如果流为空，则返回
     * {@code false} 并且不评估谓词。
     *
     ...
     *                  谓词，应用于此流的元素
     * @return 如果此流中的任何元素匹配所提供的谓词，则返回 {@code true}，否则返回 {@code false}
     */
    boolean anyMatch(LongPredicate predicate);

    /**
     * 返回此流中的所有元素是否匹配所提供的谓词。如果确定结果不需要评估所有元素，则可能不会对所有元素评估谓词。如果流为空，则返回
     * {@code true} 并且不评估谓词。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">短路终结操作</a>。
     *
     * @apiNote
     * 此方法评估谓词在流元素上的 <em>全称量化</em>（对所有 x，P(x)）。如果流为空，则量化被认为是 <em>空满足</em>，总是
     * {@code true}（无论 P(x) 如何）。
     *
     * @param predicate 一个 <a href="package-summary.html#NonInterference">非干扰的</a>、
     *                  <a href="package-summary.html#Statelessness">无状态的</a>
     *                  谓词，应用于此流的元素
     * @return 如果此流中的所有元素匹配所提供的谓词或流为空，则返回 {@code true}，否则返回 {@code false}
     */
    boolean allMatch(LongPredicate predicate);

    /**
     * 返回此流中是否有任何元素不匹配所提供的谓词。如果确定结果不需要评估所有元素，则可能不会对所有元素评估谓词。如果流为空，则返回
     * {@code true} 并且不评估谓词。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">短路终结操作</a>。
     *
     * @apiNote
     * 此方法评估否定谓词在流元素上的 <em>全称量化</em>（对所有 x，非 P(x)）。如果流为空，则量化被认为是空满足，总是
     * {@code true}，无论 P(x) 如何。
     *
     * @param predicate 一个 <a href="package-summary.html#NonInterference">非干扰的</a>、
     *                  <a href="package-summary.html#Statelessness">无状态的</a>
     *                  谓词，应用于此流的元素
     * @return 如果此流中的任何元素不匹配所提供的谓词或流为空，则返回 {@code true}，否则返回 {@code false}
     */
    boolean noneMatch(LongPredicate predicate);

    /**
     * 返回一个描述此流中第一个元素的 {@link OptionalLong}，如果流为空，则返回一个空的 {@code OptionalLong}。如果
     * 流没有遇到顺序，则可以返回任何元素。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">短路终结操作</a>。
     *
    ...
    OptionalLong findFirst();
                ...
                /**
     * 返回描述流中某个元素的 {@link OptionalLong}，或者如果流为空，则返回空的 {@code OptionalLong}。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">短路终端操作</a>。
     *
     * <p>此操作的行为明确是非确定性的；它可以自由选择流中的任何元素。这是为了在并行操作中实现最大性能；代价是多次调用可能不会返回相同的结果。 （如果需要稳定的结果，请使用 {@link #findFirst()}。）
     *
     * @return 描述此流中某个元素的 {@code OptionalLong}，或者如果流为空，则返回空的 {@code OptionalLong}
     * @see #findFirst()
     */
     ...
     * <p>这是一个 <a href="package-summary.html#StreamOps">中间操作</a>。
     *
     * @return 一个由此流的元素转换为 {@code double} 组成的 {@code DoubleStream}
     */
    DoubleStream asDoubleStream();

    /**
     * 返回一个由此流的元素转换为 {@code Long} 组成的 {@code Stream}。
     *
     * <p>这是一个 <a href="package-summary.html#StreamOps">中间操作</a>。
     *
     * @return 一个由此流的元素转换为 {@code Long} 组成的 {@code Stream}
     */
    Stream<Long> boxed();
    ...
    /**
     * 返回一个由迭代应用函数 {@code f} 到初始元素 {@code seed} 生成的无限顺序有序 {@code LongStream}，
     * 生成的流包含 {@code seed}，{@code f(seed)}，{@code f(f(seed))} 等。
     *
     * <p>流中的第一个元素（位置 {@code 0}）将是提供的 {@code seed}。对于 {@code n > 0}，位置
     * {@code n} 的元素将是将函数 {@code f} 应用到位置 {@code n - 1} 的元素的结果。
     *
     * @param seed 初始元素
     * @param f 用于将前一个元素转换为新元素的函数
     * @return 一个新的顺序 {@code LongStream}
     */
    ...
    /**
     * 返回一个由提供的 {@code LongSupplier} 生成的无限顺序无序流。适用于生成常量流、随机元素流等。
     *
     * @param s 生成元素的 {@code LongSupplier}
    ...
    /**
     * 返回一个从 {@code startInclusive}（包含）到 {@code endExclusive}（不包含）的顺序有序 {@code LongStream}，
     * 步长为 {@code 1}。
     *
     * @apiNote
     * <p>使用以下 {@code for} 循环可以顺序生成相同的递增值序列：
     * <pre>{@code
     *     for (long i = startInclusive; i < endExclusive ; i++) { ... }
     * }</pre>
     *
     * @param startInclusive 初始值（包含）
     * @param endExclusive 上限值（不包含）
     * @return 一个顺序 {@code LongStream}，包含指定范围的 {@code long} 元素
     */
    public static LongStream range(long startInclusive, final long endExclusive) {
        if (startInclusive >= endExclusive) {
            ...
            // 较低范围 [Long.MIN_VALUE, 0) 将进一步分为两部分
     ...
     *
     * @param startInclusive 初始值（包含）
     * @param endInclusive 包含的上限
     * @return 一个包含范围内的 {@code long} 元素的顺序 {@code LongStream}
     */
     ...
     *
     * <p>流构建器有一个生命周期，从构建阶段开始，在此期间可以添加元素，然后转换为已构建阶段，在此期间不能添加元素。
     * 当调用 {@link #build()} 方法时，已构建阶段开始，该方法创建一个顺序流，其元素是添加到流构建器中的元素，按添加的顺序排列。
     *
