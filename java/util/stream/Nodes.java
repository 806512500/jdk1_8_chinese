...
/**
 * 用于构建 {@link Node} 和 {@link Node.Builder} 及其原始类型特化的工厂方法。Fork/Join 任务用于从 {@link PipelineHelper} 收集输出到 {@link Node} 并展平 {@link Node}。
 *
 * @since 1.8
 */
final class Nodes {
    ...
    static final String BAD_SIZE = "流大小超过最大数组大小";
    ...
    /**
     * 生成一个具有两个或多个子节点的连接 {@link Node}。
     * <p>连接节点的计数等于每个子节点的计数之和。遍历连接节点会按子节点列表的顺序遍历每个子节点的内容。从连接节点获取的拆分器进行拆分时会保留子节点列表的顺序。
     *
     * <p>结果可能是连接节点，如果列表的大小为 1，则可能是输入的单个节点，或者是一个空节点。
     *
     * @param <T> 连接节点的元素类型
    ...
    /**
     * 生成一个描述 {@link Collection} 的 {@link Node}。
     * <p>
     * 节点将引用集合，不会复制集合。
     *
     * @param <T> 节点持有的元素类型
     ...
     *
     * @param exactSizeIfKnown 如果请求可变大小的构建器则为 -1，否则为所需的精确容量。固定容量的构建器如果添加的元素数量不正确将失败。
    ...
    /**
     * 生成一个描述 int[] 数组的 {@link Node.OfInt}。
     *
     * <p>节点将引用数组，不会复制数组。
     *
     * @param array 数组
    ...
    /**
     * 并行收集从管道输出的元素，并用 {@link Node} 描述这些元素。
     *
     * @implSpec
     * 如果管道输出的精确大小已知且源 {@link Spliterator} 具有 {@link Spliterator#SUBSIZED} 特性，
     * 则将返回一个内容为数组的扁平 {@link Node}，因为大小已知，可以预先构造数组，并由叶任务并发地将输出元素放置到数组的正确偏移位置。
     * 如果精确大小未知，输出元素将被收集到一个形状与计算相匹配的 conc-node 中。如果需要，可以并行展平此 conc-node 以生成一个描述数组的扁平 {@code Node}。
     *
     * @param helper 描述管道的管道助手
     * @param flattenTree 是否应在返回前将 conc 节点展平为描述数组的节点
    ...
    /**
     * 并行收集从 int 值管道输出的元素，并用 {@link Node.OfInt} 描述这些元素。
     *
     * @implSpec
     * 如果管道输出的精确大小已知且源 {@link Spliterator} 具有 {@link Spliterator#SUBSIZED} 特性，
     * 则将返回一个内容为数组的扁平 {@link Node}，因为大小已知，可以预先构造数组，并由叶任务并发地将输出元素放置到数组的正确偏移位置。
     * 如果精确大小未知，输出元素将被收集到一个形状与计算相匹配的 conc-node 中。如果需要，可以并行展平此 conc-node 以生成一个描述数组的扁平 {@code Node.OfInt}。
     *
    ...
    /**
     * 并行收集从 long 值管道输出的元素，并用 {@link Node.OfLong} 描述这些元素。
     *
     * @implSpec
     * 如果管道输出的精确大小已知且源 {@link Spliterator} 具有 {@link Spliterator#SUBSIZED} 特性，
     * 则将返回一个内容为数组的扁平 {@link Node}，因为大小已知，可以预先构造数组，并由叶任务并发地将输出元素放置到数组的正确偏移位置。
     * 如果精确大小未知，输出元素将被收集到一个形状与计算相匹配的 conc-node 中。如果需要，可以并行展平此 conc-node 以生成一个描述数组的扁平 {@code Node.OfLong}。
     *
     * @param <P_IN> 源 Spliterator 的元素类型
     * @param helper 描述管道的管道助手
     * @param flattenTree 是否应在返回前将 conc 节点展平为描述数组的节点
    ...
    public static <P_IN> Node.OfLong collectLong(PipelineHelper<Long> helper,
        ...
        if (size >= 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
            ...
            return flattenTree ? flattenLong(node) : node;
                ...
                /**
     * 并行收集来自 n 个双精度值管道的元素，并使用 {@link Node.OfDouble} 描述这些元素。
     *
     * @implSpec
     * 如果管道输出的确切大小已知，并且源 {@link Spliterator} 具有 {@link Spliterator#SUBSIZED} 特性，
     * 则将返回一个扁平的 {@link Node}，其内容是一个数组，因为大小已知，数组可以提前构建，
     * 输出元素可以由叶任务并发地放置在数组中。如果确切大小未知，输出元素将被收集到一个 conc-node 中，
     * 其形状反映了计算的形状。如果需要，可以并行地展平此 conc-node 以生成一个描述数组的扁平 {@code Node.OfDouble}。
     *
    ...
    /**
     * 并行展平一个 {@link Node.OfLong}。展平的节点是没有子节点的节点。如果节点已经是扁平的，则直接返回。
     *
     * @implSpec
     * 如果需要创建新节点，将创建一个长度为 {@link Node#count()} 的新 long[] 数组。然后遍历节点树，
     * 叶任务将叶节点元素并发地放置在数组中。
     *
     * @param node 要展平的节点
     * @return 扁平的 {@code Node.OfLong}
     */
    ...
    /**
     * 并行展平一个 {@link Node.OfDouble}。展平的节点是没有子节点的节点。如果节点已经是扁平的，则直接返回。
     *
     * @implSpec
     * 如果需要创建新节点，将创建一个长度为 {@link Node#count()} 的新 double[] 数组。然后遍历节点树，
     * 叶任务将叶节点元素并发地放置在数组中。
     *
     * @param node 要展平的节点
     * @return 扁平的 {@code Node.OfDouble}
     */
    ...
    private static abstract class AbstractConcNode<T, T_NODE extends Node<T>> implements Node<T> {
        ...
        AbstractConcNode(T_NODE left, T_NODE right) {
            this.left = left;
            this.right = right;
            // 当获取节点的拆分器时需要节点计数，构建树时自底向上积极计算比稍后从顶向下遍历树更便宜
    ...
    static final class ConcNode<T>
        ...
        public void copyInto(T[] array, int offset) {
            ...
            // 转换为 int 是安全的，因为调用者有责任确保数组中有足够的空间
    ...
    private static abstract class InternalNodeSpliterator<T,
        ...
        // 如果当前节点是最后一个且没有子节点，则为该节点的遍历器。
        // 这个遍历器将用于拆分和遍历。
        // 如果当前节点有子节点，则为 null
        S lastNodeSpliterator;

        // 在使用 tryAdvance 遍历时使用的遍历器
        // 如果没有部分遍历发生，则为 null
        ...
        /**
         * 使用显式栈进行从左到右的深度优先搜索，以找到下一个非空叶节点。
         */
        @SuppressWarnings("unchecked")
    ...
    private static final class FixedNodeBuilder<T>
        ...
        public void accept(T t) {
            if (curSize < array.length) {
                ...
                throw new IllegalStateException(String.format("接受的元素超过固定大小 %d",
                                                              array.length));
            }
        }
    ...
    private static class IntArrayNode implements Node.OfInt {
        ...
        IntArrayNode(long size) {
            if (size >= MAX_ARRAY_SIZE)
                throw new IllegalArgumentException("大小无效");
    ...
    private static final class IntFixedNodeBuilder
        ...
        public void accept(int i) {
            if (curSize < array.length) {
                ...
                throw new IllegalStateException(String.format("接受超过固定大小 %d",
                                                              array.length));
            }
        }
    ...
    private static final class LongSpinedNodeBuilder
        ...
        LongSpinedNodeBuilder() {} // 避免创建特殊访问器
    ...
    private static final class DoubleSpinedNodeBuilder
        ...
        DoubleSpinedNodeBuilder() {} // 避免创建特殊访问器
    ...
    private static abstract class SizedCollectorTask<P_IN, P_OUT, T_SINK extends Sink<P_OUT>,
        ...
        public void begin(long size) {
            ...
            // 转换为 int 是安全的，因为绝对大小在构建根具体的 SizedCollectorTask 时已经验证过
            // 与共享数组一起
            index = (int) offset;
