     ...
     * 构造一个新的格式化器。
     *
     * <p> 格式化输出的目标是一个 {@link StringBuilder}，可以通过调用 {@link #out out()} 获取，
     * 并且可以通过调用 {@link #toString toString()} 将其当前内容转换为字符串。使用的区域设置是此 Java
     * 虚拟机实例的 {@linkplain Locale#getDefault(Locale.Category) 默认格式化区域设置}。
     */
    public Formatter() {
     ...
     * Locale#getDefault(Locale.Category) 默认格式化区域设置}。
     *
     * @param  a
     *         格式化输出的目标。如果 {@code a} 为
     *         {@code null}，则会创建一个 {@link StringBuilder}。
     */
     ...
     *
     * <p> 格式化输出的目标是一个 {@link StringBuilder}，可以通过调用 {@link #out out()} 获取，
     * 并且可以通过调用 {@link #toString toString()} 将其当前内容转换为字符串。
     *
     * @param  l
     *         在格式化过程中应用的 {@linkplain java.util.Locale 区域设置}。如果 {@code l} 为
     *         {@code null}，则不应用本地化。
     */
     ...
     *
     * @param  fileName
     *         用作此格式化器目标的文件名。如果文件存在，则会被截断为零大小；否则，将创建一个新文件。输出将被写入文件并缓冲。
     *
     ...
     *
     * @throws  FileNotFoundException
     *          如果给定的文件名不表示一个现有的、可写的普通文件，并且无法创建一个新文件，或者在打开或创建文件时发生其他错误
     */
     ...
     *
     * @param  csn
     *         支持的 {@linkplain java.nio.charset.Charset 字符集} 名称
     *
     * @throws  FileNotFoundException
     *          如果给定的文件名不表示一个现有的、可写的普通文件，并且无法创建一个新文件，或者在打开或创建文件时发生其他错误
     *
...
/**
 * 构造一个具有指定文件名、字符集和区域设置的新格式化器。
 *
 * @param  fileName
 *         用作此格式化器目标的文件名。如果文件存在，则将被截断为零大小；否则，将创建一个新文件。输出将写入文件并进行缓冲。
 *
 * @param  csn
 ...
 *
 * @throws  FileNotFoundException
 *          如果给定的文件名不表示一个现有的、可写的常规文件，并且无法创建同名的新常规文件，或者在打开或创建文件时发生其他错误
 *
 * @throws  SecurityException
 *          如果存在安全经理，并且 {@link
 *          SecurityManager#checkWrite checkWrite(fileName)} 拒绝写入文件的权限
 ...
 *
 * <p> 使用的区域设置是此 Java 虚拟机实例的 {@linkplain
 * Locale#getDefault(Locale.Category) 默认区域设置}，用于
 * {@linkplain Locale.Category#FORMAT 格式化}。
 *
 * @param  file
 *         用作此格式化器目标的文件。如果文件存在，则将被截断为零大小；否则，将创建一个新文件。输出将写入文件并进行缓冲。
 *
 * @throws  SecurityException
 *          如果存在安全经理，并且 {@link
 ...
 *
 * @throws  FileNotFoundException
 *          如果给定的文件对象不表示一个现有的、可写的常规文件，并且无法创建同名的新常规文件，或者在打开或创建文件时发生其他错误
 */
    ...
    throws FileNotFoundException, UnsupportedEncodingException
{
     ...
     *
     * <p> 使用的区域设置是此 Java 虚拟机实例的 {@linkplain
     * Locale#getDefault(Locale.Category) 默认区域设置}
     * {@linkplain Locale.Category#FORMAT 用于格式化}。
     *
     * <p> 字符将写入给定的 {@link java.io.PrintStream
     * PrintStream} 对象，并因此使用该对象的字符集进行编码。
     *
     * @param  ps
     *         用作此格式化器目标的流。
     */
     ...
     * 返回构造此格式化器时设置的区域设置。
     *
     * <p> 具有区域设置参数的 {@link #format(java.util.Locale,String,Object...) format} 方法不会更改此值。
     *
    ...
    /**
     * 返回在输出目标上调用 {@code toString()} 的结果。例如，以下代码将文本格式化到一个
     * {@link StringBuilder} 中，然后检索结果字符串：
     *
     * <blockquote><pre>
     ...
     *     out().toString() </pre>
     *
     * <p> 完全相同。根据 {@code toString} 对于 {@link
    ...
    /**
     * 刷新此格式化器。如果目标实现了 {@link
     * java.io.Flushable} 接口，将调用其 {@code flush} 方法。
     *
     * <p> 刷新格式化器会将目标中的任何缓冲输出写入底层流。
     *
     * @throws  FormatterClosedException
     *          如果通过调用其 {@link
     *          #close()} 方法关闭了此格式化器
     ...
     * java.io.Closeable} 接口，则调用其 {@code close} 方法。
     *
     * <p> 关闭格式化程序允许它释放可能持有的资源（如打开的文件）。如果格式化程序已经关闭，则调用此方法不会产生任何效果。
     *
     * <p> 在格式化程序关闭后尝试调用任何方法（除了 {@link #ioException()}）将导致 {@link
     ...
     *
     * @return  由 Appendable 抛出的最后一个异常，或如果不存在这样的异常则返回 {@code null}。
     */
     ...
     *
     * @param  format
     *         一个格式字符串，如 <a href="#syntax">格式字符串语法</a> 中所述。
     *
     * @param  args
     *         由格式字符串中的格式说明符引用的参数。如果有更多的参数比格式说明符，多余的参数将被忽略。参数的最大数量由 <cite>The Java&trade; Virtual Machine Specification</cite> 定义的 Java 数组的最大维度限制。
     *
     * @throws  IllegalFormatException
     *          如果格式字符串包含非法语法、与给定参数不兼容的格式说明符、格式字符串的参数不足或其他非法条件。有关所有可能的格式错误的规范，请参见格式化程序类规范的 <a href="#detail">详细信息</a> 部分。
     *
     * @throws  FormatterClosedException
     *          如果此格式化程序已通过调用其 {@link
     *          #close()} 方法关闭。
     ...
     *
     * @param  l
     *         在格式化期间应用的 {@linkplain java.util.Locale 区域设置}。如果 {@code l} 为 {@code null}，则不应用本地化。这不会改变构造时设置的此对象的区域设置。
     *
     * @param  format
     *         一个格式字符串，如 <a href="#syntax">格式字符串语法</a> 中所述。
     *
    ...
    private FormatString[] parse(String s) {
        ...
        for (int i = 0, len = s.length(); i < len; ) {
            if (m.find(i)) {
                // 从字符串开始到格式说明符开始之间的任何内容都是固定文本或包含无效的格式字符串。
                if (m.start() != i) {
                    // 确保我们没有错过任何无效的格式说明符
    ...
    private class FormatSpecifier implements FormatString {
            ...
            throws IOException
        {
            if (c == Conversion.SCIENTIFIC) {
                // 创建一个具有所需精度的新 FormattedFloatingDecimal。
        ...
        private String hexDouble(double d, int prec) {
            ...
            if(!Double.isFinite(d) || d == 0.0 || prec == 0 || prec >= 13)
                ...
                // 为了决定如何舍入，查看工作有效数的最低位、最高位被舍弃的位（舍入位）以及较低位被舍弃的位是否非零（粘位）。
            ...
            throws IOException
        {
            if (c == Conversion.SCIENTIFIC) {
                ...
                // 如果需要，添加小数点。如果标度为零（内部表示没有小数部分）或原始精度为一，则尾数可能不包含小数点。如果设置了 '#' 或需要零填充以达到所需的精度，则追加小数点。
                ...
                // 如果需要，添加小数点。如果标度为零（内部表示没有小数部分），则尾数可能不包含小数点。如果设置了 '#' 或需要零填充以达到所需的精度，则追加小数点。
        ...
        // Add a '.' to the mantissa if required
            ...
            throws IOException
        {
            ...
            case DateTime.MONTH:                { // 'm' (01 - 12)
                ...
                break;
                        ...
                        // 组合
            case DateTime.TIME:         // 'T' (24小时制 hh:mm:ss - %tH:%tM:%tS)
        ...
        private Appendable print(StringBuilder sb, TemporalAccessor t, char c,
            ...
            try {
                ...
                case DateTime.HOUR_OF_DAY: {   // 'k' (0 - 23) -- 与 H 类似
                    int i = t.get(ChronoField.HOUR_OF_DAY);
                ...
                case DateTime.HOUR:        { // 'l' (1 - 12) -- 与 I 类似
                    int i = t.get(ChronoField.CLOCK_HOUR_OF_AMPM);
                    sb.append(localizedMagnitude(null, i, Flags.NONE, 2, l));
                    break;
                }
                ...
                case DateTime.AM_PM:       { // 'p' (am 或 pm)
                    // Calendar.AM = 0, Calendar.PM = 1, LocaleElements 定义了大写
                    String[] ampm = { "AM", "PM" };
                    if (l != null && l != Locale.US) {
                        DateFormatSymbols dfs = DateFormatSymbols.getInstance(l);
                        ampm = dfs.getAmPmStrings();
                ...
                case DateTime.ZONE_NUMERIC: { // 'z' ({-|+}####) - 负号？
                ...
                case DateTime.DAY_OF_MONTH_0:         // 'd' (01 - 31)
                case DateTime.DAY_OF_MONTH:         { // 'e' (1 - 31) -- 与 d 类似
...
// *    static final char WEEK_OF_YEAR_MONDAY   = 'W'; // (00 - 53) 星期一
        ...
        static final char TIME_24_HOUR  = 'R'; // (hh:mm 同 %H:%M)
