...
/*
 * 版权所有 (c) 2000, 2015，Oracle 及/或其附属公司。保留所有权利。
...
/**
 * 表示一种货币。货币通过其 ISO 4217 货币代码来识别。访问 <a href="http://www.iso.org/iso/home/standards/currency_codes.htm">
 * ISO 网站</a> 以获取更多信息。
 * <p>
 ...
 * 该文件的内容是 ISO 3166 国家代码和 ISO 4217 货币数据的键值对。值部分由三个 ISO 4217 值组成，即字母代码、数字代码和小单位。
 * 这三个 ISO 4217 值由逗号分隔。以 '#' 开头的行被视为注释行。如果用户需要指定一个切换日期，指示新数据何时生效，可以为每个货币条目指定一个可选的 UTC 时间戳。
 ...
 * <p>
 * 遇到语法错误的条目时，该条目将被忽略，并处理文件中的其余条目。对于存在重复国家代码条目的情况，该 {@code Currency} 的货币信息行为是未定义的，并处理文件中的其余条目。
 *
 * @since 1.4
 */
public final class Currency implements Serializable {
    ...
    //
    // sc = 特殊情况；dfd = 默认小数位数
    // 简单国家是指国家代码是货币代码的前缀，并且没有已知的计划更改货币的国家。
    //
    // 表格格式：
    // - mainTable:
    //   - 将国家代码映射到 32 位整数
    ...
    //   - 位 5-8：简单国家的小数位数，特殊情况为 0
    //   - 位 0-4：简单国家的货币代码的最后一个字符，或特殊情况的 ID
    // - 特殊情况 ID：
    //   - 0：国家没有货币
    //   - 其他：sc* 数组的索引 + 1
    // - scCutOverTimes：特殊情况国家更改货币的切换时间（以毫秒为单位），由
    //   System.currentTimeMillis 返回；对于不更改货币的国家，为 Long.MAX_VALUE
    // - scOldCurrencies：特殊情况国家的旧货币
    // - scNewCurrencies：特殊情况国家的新货币（如果正在更改货币）；对于其他情况为 null
    // - scOldCurrenciesDFD：旧货币的默认小数位数
    // - scNewCurrenciesDFD：新货币的默认小数位数，对于不更改货币的国家为 0
    // - otherCurrencies：所有不是简单国家主要货币的货币代码的连接，以 "-" 分隔
    ...
    /**
     * 返回给定区域设置国家的 <code>Currency</code> 实例。忽略区域设置的语言和变体组件。结果可能随时间变化，因为国家会更改其货币。例如，对于欧洲货币联盟的原始成员国，该方法在 2001 年 12 月 31 日之前返回旧的国家货币，从 2002 年 1 月 1 日起返回欧元，以各自国家的当地时间为准。
     * <p>
     * 对于没有货币的领土，如南极洲，该方法返回 <code>null</code>。
     *
    ...
    /**
     * 获取可用货币的集合。返回的货币集合包含所有可用的货币，可能包括代表已废弃的 ISO 4217 代码的货币。该集合可以修改而不影响运行时中的可用货币。
     *
     ...
     * 例如，如果默认语言环境是美国，则美元的符号是 "$"，而在其他语言环境中可能是 "US$"。如果无法确定符号，则返回 ISO 4217 货币代码。
     * <p>
     * 这相当于调用
    ...
    /**
     * 获取此货币在指定语言环境中的符号。
     * 例如，如果指定的语言环境是美国，则美元的符号是 "$"，而在其他语言环境中可能是 "US$"。如果无法确定符号，则返回 ISO 4217 货币代码。
     *
    ...
    /**
     * 获取与此货币一起使用的默认小数位数。
     * 例如，欧元的默认小数位数为 2，而日元为 0。
     * 在伪货币的情况下，例如国际货币基金组织特别提款权，返回 -1。
     *
     ...
     * 如果在默认语言环境中找不到合适的显示名称，则返回 ISO 4217 货币代码。
     * <p>
     * 这相当于调用
     * {@link #getDisplayName(Locale)
     *     getDisplayName(Locale.getDefault(Locale.Category.DISPLAY))}。
     *
     * @return 此货币在默认 {@link Locale.Category#DISPLAY DISPLAY} 语言环境中的显示名称
     * @since 1.7
     */
    ...
    /**
     * 获取适合在指定语言环境中显示此货币的名称。如果在指定语言环境中找不到合适的显示名称，则返回 ISO 4217 货币代码。
     *
     * @param locale 需要此货币显示名称的语言环境
     * @return 此货币在指定语言环境中的显示名称
     * @exception NullPointerException 如果 <code>locale</code> 为 null
     * @since 1.7
     */
    ...
    /**
     * 获取国家代码由 char1 和 char2 组成的国家的主要表项。
     */
    ...
    /**
     * 设置国家代码由 char1 和 char2 组成的国家的主要表项。
     */
    ...
    /**
     * 替换 currencydata.properties 文件中的货币数据
     *
     * @param pattern 属性的正则表达式模式
     * @param ctry 国家代码
     * @param curdata 货币数据。这是一个逗号分隔的字符串，包含 "三位字母代码"、"三位数字代码" 和 "一位（0-9）默认分数位数"。
     *    例如，"JPZ,392,0"。
     *    可以在字符串末尾附加一个 UTC 日期（逗号分隔），以便在指定日期后使货币变更生效。
    ...
    private static void replaceCurrencyData(Pattern pattern, String ctry, String curdata) {
        ...
        if (!m.find() || (m.group(4) == null && countOccurrences(curdata, ',') >= 3)) {
            // 格式未被识别。忽略数据
            // 如果 group(4) 日期字符串为 null 且有 4 个值，则日期值无效
            info("currency.properties entry for " + ctry +
