...
/*
 * 版权所有 (c) 2003, 2007, Oracle 及/或其附属公司。保留所有权利。
...
/*
 * 部分版权归 IBM Corporation，1997，2001。保留所有权利。
 */
...
/**
 * 不可变对象，封装描述某些数值运算规则的上下文设置，如 {@link BigDecimal} 类实现的规则。
 *
 * <p>与基数无关的设置包括：
 * <ol>
 * <li>{@code precision}:
 * 用于运算的数字位数；结果将被舍入到此精度
 *
...
public final class MathContext implements Serializable {
    ...
    /**
     * 一个 {@code MathContext} 对象，其精度设置与 IEEE 754R Decimal32 格式匹配，7 位数字，舍入模式为 {@link RoundingMode#HALF_EVEN HALF_EVEN}，即 IEEE 754R 默认值。
     */
    public static final MathContext DECIMAL32 =
        new MathContext(7, RoundingMode.HALF_EVEN);

    /**
     * 一个 {@code MathContext} 对象，其精度设置与 IEEE 754R Decimal64 格式匹配，16 位数字，舍入模式为 {@link RoundingMode#HALF_EVEN HALF_EVEN}，即 IEEE 754R 默认值。
     */
    public static final MathContext DECIMAL64 =
        new MathContext(16, RoundingMode.HALF_EVEN);

    /**
     * 一个 {@code MathContext} 对象，其精度设置与 IEEE 754R Decimal128 格式匹配，34 位数字，舍入模式为 {@link RoundingMode#HALF_EVEN HALF_EVEN}，即 IEEE 754R 默认值。
     */
    ...
    /* ----- 共享属性 ----- */
    /**
     * 用于运算的数字位数。值为 0 表示将使用无限精度（尽可能多的位数）。注意，数字的系数中的前导零永远不被视为有效数字。
     *
    ...
    /**
     * 使用指定的精度和 {@link RoundingMode#HALF_UP HALF_UP} 舍入模式构造新的 {@code MathContext}。
     *
    ...
    /**
     * 使用指定的精度和舍入模式构造新的 {@code MathContext}。
     *
     * @param setPrecision 非负的 {@code int} 精度设置。
    ...
    /**
     * 从字符串构造新的 {@code MathContext}。
     *
     * 字符串必须与 {@link #toString} 方法生成的格式相同。
     *
     * <p>如果字符串中的精度部分超出范围（{@code < 0}）或字符串不是由 {@link #toString} 方法生成的格式，则抛出 {@code IllegalArgumentException}。
     *
     * @param val 要解析的字符串
    ...
    public MathContext(String val) {
        ...
        try { // 任何错误都是字符串格式问题
            ...
            throw new IllegalArgumentException("bad string format");
                    ...
                    if (setPrecision < MIN_DIGITS)
            throw new IllegalArgumentException("位数 < 0");
        // 如果我们到达这里，其他参数不能无效
        precision = setPrecision;
    }
     ...
     * @return 如果且仅如果指定的 {@code Object} 是一个具有与该对象完全相同设置的 {@code MathContext} 对象，则返回 {@code true}
     */
     ...
     * 返回的 {@code String} 表示 {@code MathContext} 对象的设置，作为两个由空格分隔的单词（由单个空格字符 <tt>'&#92;u0020'</tt> 分隔，且没有前导或尾随的空白），如下所示：
     * <ol>
     * <li>
     * 字符串 {@code "precision="}，紧随其后的是精度设置的值，作为数字字符串，就像由 {@link Integer#toString(int) Integer.toString} 方法生成的一样。
     *
     * <li>
     * 字符串 {@code "roundingMode="}，紧随其后的是 {@code roundingMode} 设置的值，作为单词。此单词将与 {@link RoundingMode} 枚举中相应公共常量的名称相同。
     * </ol>
     * <p>
     * 例如：
     * <pre>
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        if (precision < MIN_DIGITS) {
            String message = "MathContext: 流中的位数无效";
            throw new java.io.StreamCorruptedException(message);
        }
        if (roundingMode == null) {
            String message = "MathContext: 流中的 roundingMode 为 null";
            throw new java.io.StreamCorruptedException(message);
        }
    }
