...
/**
 * 一个用于表示多精度整数的类，通过允许数字仅占用数组的一部分来高效地利用分配的空间，从而避免频繁重新分配数组。当执行多次迭代的操作时，用于保存数字的数组仅在必要时重新分配，且不必与所表示的数字大小相同。可变数字允许在同一个数字上进行计算，而无需在计算的每一步都创建新数字，如使用 BigInteger 时的情况。
 *
 * @see     BigInteger
...
class MutableBigInteger {
    /**
     * 以大端顺序存储此 MutableBigInteger 的值。值可以始于 value 数组中的偏移位置，并且可以早于 value 数组的长度结束。
     */
    int[] value;

    /**
     * 当前用于存储此 MutableBigInteger 的值的 value 数组中的 int 数量。值始于偏移位置，且 offset + intLen 可能小于 value.length。
     */
    int intLen;

    /**
     * 此 MutableBigInteger 的值在 value 数组中开始的位置。
     */
    int offset = 0;

    // 常量
    /**
     * 一个包含单个元素值数组且值为 1 的 MutableBigInteger。由 BigDecimal divideAndRound 用于递增商。仅当方法不会修改此对象时使用此常量。
     */
    static final MutableBigInteger ONE = new MutableBigInteger(1);

    /**
     * 在除法前取消二的幂的最小 {@code intLen} 阈值。如果数字的 int 数量小于此阈值，则 {@code divideKnuth} 不会从被除数和除数中消除共同的二的幂。
     */
    static final int KNUTH_POW2_THRESH_LEN = 6;

    /**
     * 在除法前取消二的幂的最小尾随零 int 数量。如果被除数和除数在末尾不共享至少这么多的零 int，则 {@code divideKnuth} 不会从被除数和除数中消除共同的二的幂。
     */
    ...
    /**
     * 默认构造函数。创建一个具有一个字容量的空 MutableBigInteger。
     */
    MutableBigInteger() {
        value = new int[1];
        intLen = 0;
    ...
    /**
     * 根据指定的 int 值构造一个新的 MutableBigInteger。
     */
    ...
    /**
     * 根据提供的数组构造一个新的 MutableBigInteger，直到数组的长度。
     */
    MutableBigInteger(int[] val) {
        value = val;
        intLen = val.length;
    ...
    /**
     * 根据指定的 BigInteger 构造一个新的 MutableBigInteger。
     */
    ...
    /**
     * 根据指定的 MutableBigInteger 构造一个新的 MutableBigInteger。
     */
    ...
    /**
     * 将此数字设置为一个所有位都为 1 的 n 个 int 的数字。用于 Burnikel-Ziegler 除法。
     * @param n value 数组中的 int 数量
     * @return 一个等于 {@code ((1<<(32*n)))-1} 的数字
     */
     ...
     */
    private long toLong() {
        assert (intLen <= 2) : "此 MutableBigInteger 超出了 long 的范围";
        if (intLen == 0)
            return 0;
    ...
    /**
     * 用于将 MutableBigInteger 对象转换为 long 值的内部方法，给定指定的符号。
     * 如果值不能转换为 long，则返回 INFLATED。
     */
    ...
    /**
     * 将 MutableBigInteger 设置为零，移除其偏移。
     */
    void reset() {
        offset = intLen = 0;
    }
    ...
    /**
     * 比较两个 MutableBigInteger 的值。返回 -1、0 或 1，表示此 MutableBigInteger 数值上小于、等于或大于 <tt>b</tt>。
     */
    final int compare(MutableBigInteger b) {
        ...
        // 添加 Integer.MIN_VALUE 以使比较作为无符号整数比较。
        int[] bval = b.value;
    ...
    /**
     * 返回一个值，等于 {@code b.leftShift(32*ints); return compare(b);} 会返回的值，但不会改变 {@code b} 的值。
     */
    ...
    /**
     * 将此数字与一个 MutableBigInteger 对象的一半进行比较（用于余数测试）。
    ...
    /**
     * 返回此 MutableBigInteger 中最低设置位的索引。如果此 MutableBigInteger 的值为零，则返回 -1。
     */
    ...
    /**
     * 返回此 MutableBigInteger 在指定索引处使用的 int 值。此方法未在所有平台上内联，因此不经常使用。
     */
    ...
    /**
     * 返回一个等于此 MutableBigInteger 在指定索引处使用的 int 的无符号值的 long。此方法未在所有平台上内联，因此不经常使用。
     */
    ...
    /**
     * 确保 MutableBigInteger 处于正常形式，特别是确保没有前导零，并且如果值为零，则 intLen 为零。
     */
    ...
    /**
     * 如果此 MutableBigInteger 不能容纳 len 个字，则将 value 数组的大小增加到 len 个字。
     */
    ...
    /**
     * 将此 MutableBigInteger 在索引 + 偏移处的 int 设置为 val。此方法未在所有平台上内联，因此不经常使用。
     */
    ...
    /**
     * 将此 MutableBigInteger 的 value 数组设置为指定的数组。intLen 设置为指定的长度。
     */
    ...
    /**
     * 将此 MutableBigInteger 的 value 数组设置为指定数组的副本。intLen 设置为新数组的长度。
     */
    ...
    /**
     * 将此 MutableBigInteger 的 value 数组设置为指定数组的副本。intLen 设置为指定数组的长度。
     */
    ...
    boolean isZero() {
        return (intLen == 0);
                ...
                /**
     * 如果这个 MutableBigInteger 是偶数，则返回 true。
     */
    boolean isEven() {
    ...
    /**
     * 如果这个 MutableBigInteger 是奇数，则返回 true。
     */
    boolean isOdd() {
    ...
    /**
     * 如果这个 MutableBigInteger 处于正常形式，则返回 true。一个
     * MutableBigInteger 处于正常形式，如果它在偏移量之后没有前导零，
     * 并且 intLen + 偏移量 <= value.length。
     */
    boolean isNormal() {
    ...
    /**
     * 返回这个 MutableBigInteger 的 10 进制字符串表示。
     */
    ...
    /**
     * 右移这个 MutableBigInteger n 位。MutableBigInteger 保持正常形式。
     */
    ...
    /**
     * 左移这个 MutableBigInteger n 位。
     */
    void leftShift(int n) {
        /*
         * 如果这个 MutableBigInteger 已经有足够的存储空间，
         * 则使用可用的空间。值数组中未使用的 int 之后的空间使用更快，
         * 因此如果可能的话，额外的空间将从右侧获取。
         */
        if (intLen == 0)
           return;
        int nInts = n >>> 5;
    ...
    /**
     * 用于除法的原语。此方法在指定偏移处将一个除数 a 的一个倍数加回到被除数 result。当 qhat 被估计得太大，必须进行调整时，使用此方法。
     */
    ...
    /**
     * 用于除法。它将一个 n 个单词的输入 a 乘以一个单词的输入 x，并从 q 中减去 n 个单词的乘积。当从被除数中减去 qhat*除数时，需要此操作。
     */
    ...
    /**
     * 与 mulsun 方法相同，只是 q 数组不更新，方法的唯一结果是借位标志。
     */
    ...
    /**
     * 右移这个 MutableBigInteger n 位，其中 n 小于 32。
     * 假设 intLen > 0，n > 0 以提高速度
     */
    ...
    /**
     * 左移这个 MutableBigInteger n 位，其中 n 小于 32。
     * 假设 intLen > 0，n > 0 以提高速度
     */
    ...
    /**
     * 返回一个等于这个数字的低 {@code n} 个 int 的 {@code BigInteger}。
     */
    ...
    /**
     * 将两个 MutableBigInteger 对象的内容相加。结果放置在当前的 MutableBigInteger 中。
     * 被加数的内容不会改变。
     */
    ...
    /**
     * 将 {@code addend} 的值左移 {@code n} 个 int 后相加。
    ...
    /**
     * 类似于 {@link #addShifted(MutableBigInteger, int)}，但 {@code this.intLen} 必须
     * 不大于 {@code n}。换句话说，将 {@code this} 和 {@code addend} 连接起来。
     */
    ...
    /**
     * 加上 {@code addend} 的低 {@code n} 个 int。
     */
    ...
    /**
     * 从较大数中减去较小数，并将结果放入当前的 MutableBigInteger 中。
     */
    ...
    /**
     * 从较大数中减去较小数，并将结果放入较大数中。返回 1 表示结果在 a 中，-1 表示结果在 b 中，0 表示未执行任何操作。
     */
    ...
    /**
     * 乘以两个 MutableBigInteger 对象的内容。结果放置在 MutableBigInteger z 中。y 的内容不会改变。
     */
    ...
    /**
     * 乘以这个 MutableBigInteger 的内容与 y。结果放置在 z 中。
     */
    void mul(int y, MutableBigInteger z) {
     ...
     /**
     * 此方法用于将 n 个字的被除数除以一个字的除数。商放置在 quotient 中。一个字的除数由 divisor 指定。
     *
     * @return 返回除法的余数。
     *
     */
     ...
     * 计算 this 除以 b 的商，并将商放置在提供的 MutableBigInteger 对象中，返回余数对象。
     *
     * 使用 Knuth 第 4.3.1 节中的算法 D。该算法的许多优化来自 Colin Plumb C 库。
     * 它对一个字的除数进行了特殊处理以提高速度。b 的内容不会改变。
     *
     */
    MutableBigInteger divideKnuth(MutableBigInteger b, MutableBigInteger quotient, boolean needRemainder) {
        ...
        // 如果我们超过 KNUTH_POW2_* 阈值，取消共同的 2 的幂
     ...
     * 此方法实现了 Burnikel-Ziegler 论文第 9 页的算法 3。
     * 参数 beta 被选择为 2<sup>32</sup>，因此几乎所有的移位都是 32 位的倍数。<br/>
     * {@code this} 和 {@code b} 必须是非负数。
     * @param b 除数
     * @param quotient 输出参数 {@code this/b}
     * @return 余数
     */
    MutableBigInteger divideAndRemainderBurnikelZiegler(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        if (r < s) {
            ...
            // 与 Knuth 除法不同，我们在这里不检查共同的 2 的幂，因为 BZ 在两个数都包含 2 的幂时已经运行得更快，取消它们没有额外的好处。
            ...
            bShifted.safeLeftShift(sigma);   // 步骤 4a: 将 b 移位，使其长度为 n 的倍数
            MutableBigInteger aShifted = new MutableBigInteger(this);
            aShifted.safeLeftShift(sigma);     // 步骤 4b: 将 a 移位相同数量
            ...
            for (int i = t - 2; i > 0; i--) {
                ...
                z.addDisjoint(ri, n);
                quotient.addShifted(qi, i * n);   // 更新 q（部分步骤 9）
            }
    ...
    /**
     * 此方法实现了 Burnikel-Ziegler 论文第 4 页的算法 1。
     * 它将 2n 位数除以 n 位数。<br/>
     * 参数 beta 为 2<sup>32</sup>，因此所有移位都是 32 位的倍数。
     * <br/>
     * {@code this} 必须是非负数，且 {@code this.bitLength() <= 2*b.bitLength()}
     * @param b 一个正数，且 {@code b.bitLength()} 是偶数
     * @param quotient 输出参数 {@code this/b}
     * @return {@code this%b}
     */
    ...
    /**
     * 此方法实现了 Burnikel-Ziegler 论文第 5 页的算法 2。
     * 它将 3n 位数除以 2n 位数。<br/>
     * 参数 beta 为 2<sup>32</sup>，因此所有移位都是 32 位的倍数。<br/>
     * <br/>
    ...
    private MutableBigInteger divide3n2n(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        // 步骤 5: r = r * beta^n + a3 - d（论文中说 a4）
        // 然而，不要在 while 循环之前减去 d，以防止 r 变为负数
        r.leftShift(32 * n);
        r.addLower(this, n);
    ...
    /**
     * 返回一个包含 {@code this} 数字中从 {@code index*blockLength} 开始的 {@code blockLength} 个整数的 {@code MutableBigInteger}。<br/>
     * 用于 Burnikel-Ziegler 除法。
     * @param index 块索引
     * @param numBlocks {@code this} 数字中的总块数
    ...
    /**
     * 内部用于计算 this 除以 v 的商，并将商放置在提供的 MutableBigInteger 对象中，返回余数。
     *
     * @return 返回除法的余数。
     */
    ...
    private MutableBigInteger divideMagnitude(MutableBigInteger div,
        ...
        int[] divisor;
        MutableBigInteger rem; // 余数开始为被除数，带有一个前导零的空间
        if (shift > 0) {
    ...
    /**
     * 用正 long 值表示的除数除以这个 MutableBigInteger。
     * 商将被放置在提供的商对象中，余数对象将被返回。
     */
    ...
    /**
     * 该方法将一个 long 量除以一个 int 以估计两个多精度数的 qhat。
     * 当 n 的有符号值小于零时使用。
     * 返回 long 值，其中高 32 位包含余数值，低 32 位包含商值。
     */
    static long divWord(long n, int d) {
        ...
        // n - q*dlong == r && 0 <= r <dLong，因此完成。
        return (r << 32) | (q & LONG_MASK);
    }


                /**
     * 计算此对象和 b 的最大公约数。此对象和 b 在计算过程中会被改变。
     */
    ...
    static int binaryGcd(int a, int b) {
        ...
        while (a != b) {
            if ((a+0x80000000) > (b+0x80000000)) {  // a > b 作为无符号数
                a -= b;
    ...
    /**
     * 返回此对象模 p 的模逆。此对象和 p 不会被此操作改变。
     */
    MutableBigInteger mutableModInverse(MutableBigInteger p) {
        // 模数为奇数，使用 Schroeppel 的算法
        if (p.isOdd())
            return modInverse(p);
    ...
    MutableBigInteger modInverseMP2(int k) {
        ...
        tLong = tLong * (2 - pLong * tLong);  // 1 次更多的牛顿迭代步骤
    ...
    /**
     * 计算此对象模 mod 的乘法逆元，其中 mod 参数是奇数。此对象和 mod 在计算过程中不会被改变。
     *
     * 此方法实现了一种由 Richard Schroeppel 提出的算法，该算法使用与 Montgomery Reduction
    ...
    private MutableBigInteger modInverse(MutableBigInteger mod) {
        ...
        if (c.compare(p) >= 0) { // c 的绝对值大于 p
            ...
            // 前一行忽略了符号，因此我们将数据复制回 c 以恢复所需的符号（并将其转换回 SignedMutableBigInteger）
    ...
    /**
     * 使用扩展欧几里得算法计算基数的模逆元，模数是 2 的幂。模数是 2^k。
     */
