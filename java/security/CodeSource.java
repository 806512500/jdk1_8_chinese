...
/**
 *
 * <p>此类扩展了代码库的概念，不仅封装了位置（URL），还封装了用于验证从该位置来源的已签名代码的证书链。
 *
 * @author Li Gong
...
public class CodeSource implements java.io.Serializable {
    ...
    // 用于 matchLocation 的缓存 SocketPermission
    ...
    /**
     * 构造一个 CodeSource 并将其与指定的位置和证书集关联。
     *
     * @param url 位置（URL）。
     *
     * @param certs 证书。它可以为 null。数组的内容被复制以防止后续修改。
     */
    ...
    /**
     * 构造一个 CodeSource 并将其与指定的位置和代码签名者集关联。
     *
     * @param url 位置（URL）。
     * @param signers 代码签名者。它可以为 null。数组的内容被复制以防止后续修改。
     *
     * @since 1.5
     */
    ...
    /**
     * 测试指定对象与该对象是否相等。如果两个 CodeSource 对象的位置值相同，并且它们的签名者证书链值相同，则认为它们相等。证书链不必按相同的顺序排列。
     *
    ...
    public final URL getLocation() {
        /* 由于 URL 实际上是不可变的，因此返回其本身不会造成安全问题 */
     ...
     * 构造函数创建的，则其证书链将被提取并用于创建一个 Certificate 对象数组。每个签名者的证书后面跟着其支持的证书链（可能为空）。每个签名者的证书及其支持的证书链按从下到上的顺序排列（即，签名者的证书在前，（根）证书机构在后）。
     *
     ...
     * 构造函数创建的，则其证书链将被提取并用于创建一个 CodeSigner 对象数组。请注意，只有 X.509 证书会被检查——所有其他类型的证书将被忽略。
     *
     ...
     * <p>
     * 更具体地说，此方法执行以下检查。
     * 如果任何检查失败，则返回 false。如果所有检查都成功，则返回 true。
     * <ul>
     * <li> <i>codesource</i> 必须不为 null。
     * <li> 如果此对象的证书不为 null，则此对象的所有证书必须存在于 <i>codesource</i> 的证书中。
     * <li> 如果此对象的位置 (getLocation()) 不为 null，则将对此对象的位置和 <i>codesource</i> 的位置进行以下检查：
     *   <ul>
     *     <li>  <i>codesource</i> 的位置必须不为 null。
     *
     *     <li>  如果此对象的位置
     ...
     *
     *     <li>  如果此对象的主机 (getLocation().getHost()) 不为 null，
     *           则使用此对象的主机构造的 SocketPermission 必须隐含使用 <i>codesource</i> 的主机构造的 SocketPermission。
     *
     *     <li>  如果此对象的端口 (getLocation().getPort()) 不等于 -1（即指定了端口），则必须等于
     ...
     *           如果此对象的文件以 "/*" 结尾，
     *           则 <i>codesource</i> 的文件必须以此对象的文件开头，并且不能有任何进一步的 "/" 分隔符。
     *           如果此对象的文件不以 "/" 结尾，
     *           则 <i>codesource</i> 的文件必须与此对象的文件加上一个 '/' 匹配。
     *
     *     <li>  如果此对象的引用 (getLocation().getRef()) 不为 null，则必须等于 <i>codesource</i> 的引用。
     *
     *   </ul>
     * </ul>
     * <p>
     * 例如，具有以下位置和 null 证书的 codesource 对象都隐含
     * 位置为 "http://java.sun.com/classes/foo.jar" 且证书为 null 的 codesource：
     * <pre>
     *     http:
     ...
     * </pre>
     *
     * 注意，如果此 CodeSource 位置为 null 且证书链为 null，则它隐含每个其他 CodeSource。
     *
    ...
    /**
     * 如果此 CodeSource 中的所有证书也在 <i>that</i> 中，则返回 true。
     *
     * @param that 要比较的 CodeSource。
     * @param strict 如果为 true，则执行严格的等值匹配。
     *               否则执行子集匹配。
     */
    ...
    /**
     * 如果两个 CodeSource 的位置“相同”，则返回 true。
     *
     * @param that 要比较的 CodeSource
     */
    private boolean matchLocation(CodeSource that) {
        ...
        if (thisPort != -1) {
            ...
            // 匹配目录及其包含的所有文件。
            // 例如，"/a/b/*" 意味着任何以 "/a/b/" 开头但没有更多斜杠的内容。
     ...
     * 将此对象写入流（即序列化）。
     *
     * @serialData 初始的 {@code URL} 后跟一个 {@code int}，表示要跟随的证书数量
     * （值为“零”表示此对象没有关联的证书）。
     * 每个证书从一个 {@code String} 开始，表示证书类型，后跟一个
     * {@code int}，指定证书编码的长度，然后是证书编码本身，以字节数组的形式写入。
     * 最后，如果有任何代码签名者，则将代码签名者数组序列化并写入。
     */
    ...
    /*
     * 将证书数组转换为代码签名者数组。
     * 证书数组是证书链的连接，其中每个链的初始证书是终端实体证书。
     *
     * @return 生成的代码签名者数组，如果没有生成则返回 null。
     */
        ...
        try {
            ...
            while (i < certs.length) {
                List<java.security.cert.Certificate> certChain =
                        new ArrayList<>();
                certChain.add(certs[i++]); // 第一个证书是终端实体证书
                int j = i;
            ...
            return null; //TODO - 可能更好的做法是在这里抛出异常
        }
    }
}
