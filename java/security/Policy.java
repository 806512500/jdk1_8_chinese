...
/**
 * Policy 对象负责确定在 Java 运行时环境中执行的代码是否有权限执行安全敏感操作。
 *
 * <p> 任何时候运行时中只有一个 Policy 对象。可以通过调用 {@code setPolicy} 方法来安装 Policy 对象。通过调用 {@code getPolicy} 方法可以获取已安装的 Policy 对象。
 *
 * <p> 如果运行时中没有安装 Policy 对象，调用 {@code getPolicy} 会安装默认的 Policy 实现（默认的子类实现）。可以通过将 {@code policy.provider} 安全属性设置为所需的 Policy 子类实现的完全限定名称来更改默认的 Policy 实现。
 *
 * <p> 应用程序代码可以直接继承 Policy 以提供自定义实现。此外，可以通过调用带有标准类型的 {@code getInstance} 工厂方法之一来构造 Policy 对象的实例。默认的策略类型是 "JavaPolicy"。
 *
 * <p> 一旦 Policy 实例被安装（无论是默认安装还是通过调用 {@code setPolicy} 安装），Java 运行时会在需要确定执行代码（封装在 ProtectionDomain 中）是否可以执行 SecurityManager 保护的操作时调用其 {@code implies} 方法。Policy 对象如何检索其策略数据取决于 Policy 实现本身。策略数据可能存储在平面 ASCII 文件、Policy 类的序列化二进制文件或数据库中。
 *
 * <p> {@code refresh} 方法导致策略对象刷新/重新加载其数据。此操作取决于实现。例如，如果策略对象在配置文件中存储其数据，调用 {@code refresh} 将导致其重新读取配置策略文件。如果不支持刷新操作，此方法将不执行任何操作。注意，刷新的策略可能不会对特定 ProtectionDomain 中的类产生影响。这取决于 Policy 提供商的 {@code implies} 方法实现及其 PermissionCollection 缓存策略。
 *
...
public abstract class Policy {
    ...
    /**
     * 返回已安装的 Policy 对象。此值不应被缓存，因为它可能通过调用 {@code setPolicy} 被更改。此方法首先调用
     * {@code SecurityManager.checkPermission}，并使用
    ...
    /**
     * 返回已安装的 Policy 对象，跳过安全检查。由 ProtectionDomain 和 getPolicy 使用。
     *
     * @return 已安装的 Policy。
     */
    ...
    private static void initPolicy (final Policy p) {
        /*
         * 一个不在引导类路径上的策略提供者可能会触发
         * Policy.implies 或 Policy.getPermissions 调用的安全检查。如果确实发生这种情况，提供者
         * 必须能够回答其自身的 ProtectionDomain，而不会触发额外的安全检查，否则
         * 策略实现将陷入无限递归。
         *
         * 为了缓解这种情况，提供者可以在安装过程中收集其自身的
         * ProtectionDomain 并关联一个 PermissionCollection。当前安装的策略
         * 提供者（如果有的话）将在此过程中处理对
         * Policy.implies 或 Policy.getPermissions 的调用。
         *
         * 该 Policy 超类缓存 ProtectionDomain 并静态绑定权限，以便遗留 Policy
         * 实现将继续正常工作。
         */
        ...
        /*
         * 收集授予此保护域的权限
         * 以便在处理 Policy.implies 或 Policy.getPermissions 调用时对提供者进行安全检查。
         */
     ...
     * 返回指定类型的 Policy 对象。
     *
     * <p> 该方法遍历注册的安全提供者列表，从最优先的 Provider 开始。
     * 返回一个新的 Policy 对象，封装了
     * 第一个支持指定类型的 Provider 的 PolicySpi 实现。
     *
     * <p> 注册的提供者列表可以通过
     * {@link Security#getProviders() Security.getProviders()} 方法获取。
     *
     * @param type 指定的 Policy 类型。有关标准 Policy 类型的列表，请参阅
     *    <a href=
     *    "{@docRoot}/../technotes/guides/security/StandardNames.html#Policy">
     *    Java 密码架构标准算法名称文档</a> 中的 Policy 部分。
     *
     * @param params Policy 的参数，可以为 null。
     *
     * @return 新的 Policy 对象。
     *
     * @exception SecurityException 如果调用者没有权限为指定类型获取 Policy 实例。
     *
     * @exception NullPointerException 如果指定的类型为 null。
     *
     * @exception IllegalArgumentException 如果指定的参数不被选定 Provider 的 PolicySpi 实现理解。
     *
     ...
     * 返回指定类型的 Policy 对象。
     *
     * <p> 返回一个新的 Policy 对象，封装了指定提供者的 PolicySpi 实现。
     * 指定的提供者必须在提供者列表中注册。
     *
     ...
     * @exception NullPointerException 如果指定的类型为 null。
     *
     * @exception IllegalArgumentException 如果指定的提供者为 null 或空，
     ...
     * 返回指定类型的 Policy 对象。
     *
     * <p> 返回一个新的 Policy 对象，封装了指定 Provider 对象的 PolicySpi 实现。
     * 注意，指定的 Provider 对象不必在提供者列表中注册。
     *
     * @param type 指定的 Policy 类型。请参阅 Policy 部分的
     *    <a href=
     *    "{@docRoot}/../technotes/guides/security/StandardNames.html#Policy">
     ...
     *
     * <p> 仅当此 Policy 实例是通过调用 {@code Policy.getInstance} 获得时，它才会有 Provider。
     * 否则，此方法返回 null。
     *
     * @return 此 Policy 的 Provider，或 null。
     *
     * @since 1.6
     */
     ...
     *
     * <p> 不建议应用程序调用此方法，因为此操作可能不被所有策略实现支持。
     ...
     *
     * <p> 此方法的默认实现首先通过 {@code getPermissions(CodeSource)}
     * （CodeSource 从指定的 ProtectionDomain 获取）返回的权限，以及指定 ProtectionDomain 内部的权限，
     * 将所有这些权限组合并返回在一个新的 PermissionCollection 对象中。如果
     * {@code getPermissions(CodeSource)} 返回 Policy.UNSUPPORTED_EMPTY_COLLECTION，
     * 则此方法返回一个包含指定 ProtectionDomain 内部权限的新 PermissionCollection 对象。
     *
    ...
    /**
     * 评估授予 ProtectionDomain 的全局策略，并测试是否授予了权限。
     *
     ...
     * 将导致文件被重新读取。
     *
     * <p> 此方法的默认实现不执行任何操作。如果策略实现支持刷新操作，应重写此方法。
     */
    public void refresh() { }
    ...
    /**
     * 此类表示一个只读的空 PermissionCollection 对象，当 Policy 类中的
     * {@code getPermissions(CodeSource)} 和 {@code getPermissions(ProtectionDomain)}
     * 方法不被 Policy 实现支持时，从这些方法返回。
     */
    private static class UnsupportedEmptyCollection
        ...
        /**
         * 检查指定的权限是否被此 PermissionCollection 中持有的权限集合隐含。
         *
         * @param permission 要比较的权限对象。
         *
         * @return 如果权限集合中的权限隐含 "permission"，则返回 true，否则返回 false。
         */
