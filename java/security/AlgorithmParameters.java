 ...
 * 本类用于作为密码参数的不透明表示。
 *
 * <p>通过调用其中一个 {@code getInstance} 工厂方法（静态方法，返回给定类的实例），
 * 可以获取用于管理特定算法参数的 {@code AlgorithmParameters} 对象。
 *
 * <p>一旦获取了 {@code AlgorithmParameters} 对象，就必须通过调用 {@code init}，
 * 使用适当的参数规范或参数编码进行初始化。
 *
 * <p>通过调用 {@code getParameterSpec} 从 {@code AlgorithmParameters} 对象中获取透明参数规范，
 * 通过调用 {@code getEncoded} 获取参数的字节编码。
 *
 ...
 * Java 密码架构标准算法名称文档的 AlgorithmParameters 部分</a> 中描述。
...
public class AlgorithmParameters {
     ...
     * 返回一个封装了第一个支持指定算法的提供者的 AlgorithmParametersSpi 实现的新 AlgorithmParameters 对象。
     *
     * <p>可以通过 {@link Security#getProviders() Security.getProviders()} 方法检索注册提供者的列表。
     *
     * <p>返回的参数对象必须通过调用 {@code init}，使用适当的参数规范或参数编码进行初始化。
     *
     * @param algorithm 请求的算法名称。
     * 有关标准算法名称的信息，请参阅 <a href=
     * "{@docRoot}/../technotes/guides/security/StandardNames.html#AlgorithmParameters">
     * Java 密码架构标准算法名称文档的 AlgorithmParameters 部分</a>。
     *
     * @return 新的参数对象。
     *
     ...
     * 返回指定算法的参数对象。
     *
     * <p>返回一个封装了指定提供者的 AlgorithmParametersSpi 实现的新 AlgorithmParameters 对象。
     * 指定的提供者必须在安全提供者列表中注册。
     *
     * <p>可以通过 {@link Security#getProviders() Security.getProviders()} 方法检索注册提供者的列表。
     *
     ...
     * 返回指定算法的参数对象。
     *
     * <p> 返回一个新的 AlgorithmParameters 对象，封装了来自指定 Provider 对象的
     * AlgorithmParametersSpi 实现。请注意，指定的 Provider 对象
     * 不必注册在提供者列表中。
     *
     * <p>返回的参数对象必须通过调用
     * {@code init}，使用适当的参数规范或参数编码进行初始化。
     *
     * @param algorithm 请求的算法名称。
     * 有关标准算法名称的信息，请参阅 <a href=
     * "{@docRoot}/../technotes/guides/security/StandardNames.html#AlgorithmParameters">
     * Java Cryptography Architecture Standard Algorithm Name Documentation</a>
     * 中的 AlgorithmParameters 部分。
     *
     * @param provider 提供者的名称。
     *
     * @return 新的参数对象。
    ...
    /**
     * 导入指定的参数并根据参数的主要解码格式进行解码。参数的主要解码格式是 ASN.1，
     * 如果存在此类参数的 ASN.1 规范。
     *
     * @param params 编码的参数。
     *
     * @exception IOException 在解码错误时，或此参数对象已被初始化时。
     */
     ...
     * 如果 {@code format} 为 null，则使用参数的主要解码格式。主要解码格式是 ASN.1，
     * 如果存在此类参数的 ASN.1 规范。
     *
     * @param params 编码的参数。
     *
    ...
    /**
     * 返回此参数对象的（透明）规范。
     * {@code paramSpec} 识别参数应返回的规范类。例如，可以是
     * {@code DSAParameterSpec.class}，以指示参数应返回为
     * {@code DSAParameterSpec} 类的实例。
     *
     * @param <T> 要返回的参数规范的类型
     ...
     *
     * @exception IOException 在编码错误时，或此参数对象未被初始化时。
     */
    ...
    /**
     * 以指定的方案编码参数。
     * 如果 {@code format} 为 null，则使用参数的主要编码格式。主要编码格式是 ASN.1，
     * 如果存在此类参数的 ASN.1 规范。
     *
     ...
     * 返回描述参数的格式化字符串。
     *
     * @return 如果此参数对象已初始化，则返回描述参数的格式化字符串，否则返回 null。
     */
