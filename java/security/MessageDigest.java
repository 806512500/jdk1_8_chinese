...
/**
 * 该 MessageDigest 类为应用程序提供了消息摘要算法的功能，例如 SHA-1 或 SHA-256。
 * 消息摘要是安全的单向哈希函数，可以处理任意大小的数据并输出固定长度的哈希值。
 *
 * <p>MessageDigest 对象初始化后开始。数据通过 {@link #update(byte) update}
 * 方法进行处理。任何时候都可以调用 {@link #reset() reset} 重置摘要。一旦所有要更新的数据
 * 都已更新，应调用其中一个 {@link #digest() digest} 方法来完成哈希计算。
 *
 * <p>{@code digest} 方法可以为给定数量的更新调用一次。调用 {@code digest} 后，MessageDigest
 * 对象将重置为其初始化状态。
 *
 * <p>实现可以自由地实现 Cloneable 接口。客户端应用程序可以通过尝试克隆并捕获
 ...
 * }</pre>
 *
 * <p>注意，如果给定实现不可克隆，仍然可以通过实例化多个实例来计算中间摘要，前提是摘要的数量已知。
 *
 * <p>注意，此类是抽象的，并且出于历史原因扩展自
 ...
 * Java 加密体系结构标准算法名称文档的消息摘要部分</a>中有描述。
...
public abstract class MessageDigest extends MessageDigestSpi {
     ...
     * 返回实现指定摘要算法的 MessageDigest 对象。
     *
     * <p> 返回一个新的 MessageDigest 对象，封装了来自指定提供者的
     * MessageDigestSpi 实现。指定的提供者必须在安全提供者列表中注册。
     *
     ...
     *
     * @exception NoSuchAlgorithmException 如果指定提供者中没有可用的指定算法的 MessageDigestSpi 实现。
     *
     ...
     * 返回实现指定摘要算法的 MessageDigest 对象。
     *
     * <p> 返回一个新的 MessageDigest 对象，封装了来自指定 Provider 对象的
     * MessageDigestSpi 实现。注意，指定的 Provider 对象不必在提供者列表中注册。
     *
     * @param algorithm 请求的算法名称。
     * 有关标准算法名称的信息，请参阅 <a href=
     * "{@docRoot}/../technotes/guides/security/StandardNames.html#MessageDigest">
     ...
     * @return 实现指定算法的 MessageDigest 对象。
     *
     * @exception NoSuchAlgorithmException 如果指定 Provider 对象中没有可用的指定算法的 MessageDigestSpi 实现。
     *
    ...
    public final void update(ByteBuffer input) {
        ...
        state = IN_PROGRESS;
                ...
                /**
     * 完成哈希计算，执行最终操作，如填充。调用此方法后，摘要会被重置。
     *
     * @return 结果哈希值的字节数组。
     */
    public byte[] digest() {
     ...
     * 完成哈希计算，执行最终操作，如填充。调用此方法后，摘要会被重置。
     *
     * @param buf 计算摘要的输出缓冲区
     *
     * @param offset 输出缓冲区中开始存储摘要的偏移量
     *
    ...
    public int digest(byte[] buf, int offset, int len) throws DigestException {
        if (buf == null) {
            throw new IllegalArgumentException("未给出输出缓冲区");
        }
     ...
     * {@link #update(byte[]) update(input)}，将 <i>input</i> 数组传递给 {@code update} 方法，
     * 然后调用 {@link #digest() digest()}。
     *
    ...
    public static boolean isEqual(byte[] digesta, byte[] digestb) {
        /* 检查 digesta 中的所有字节以确定相等性。
         * 计算时间仅取决于 digesta 的长度
         * 与 digestb 的长度或 digesta 和 digestb 的内容无关。
         */
    ...
    /**
     * 返回一个标识算法的字符串，独立于实现细节。名称应该是标准的
     * Java 安全名称（如 "SHA-256"）。
     * 有关标准算法名称的信息，请参阅 <a href=
     * "{@docRoot}/../technotes/guides/security/StandardNames.html#MessageDigest">
     * Java 密码体系结构标准算法名称文档</a> 中的消息摘要部分。
     *
     * @return 算法的名称
     */
     ...
     *
     * @exception CloneNotSupportedException 如果此方法在不支持 {@code Cloneable} 的实现上调用。
     */
    ...
    /*
     * 以下类允许提供者扩展自 MessageDigestSpi 而不是从 MessageDigest。它表示一个带有封装的、
     * 提供者提供的 SPI 对象（类型为 MessageDigestSpi）的消息摘要。如果提供者实现是 MessageDigestSpi 的实例，
     * 上面的 getInstance() 方法将返回此类的一个实例，其中封装了 SPI 对象。
     *
     * 注意：原始 MessageDigest 类中的所有 SPI 方法都已移至层次结构中的新类（MessageDigestSpi），
     ...
     */
         ...
         * 如果代理支持克隆，则返回一个克隆。
         *
         * @return 如果代理支持克隆，则返回一个克隆。
         *
         * @exception CloneNotSupportedException 如果调用的是不支持 {@code Cloneable} 的代理。
         */
        public Object clone() throws CloneNotSupportedException {
