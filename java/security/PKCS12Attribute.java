...
public final class PKCS12Attribute implements KeyStore.Entry.Attribute {
    ...
    /**
     * 从名称和值构造 PKCS12 属性。
     * 名称是表示为点分隔整数列表的 ASN.1 对象标识符。
     * 字符串值表示为字符串本身。
     * 二进制值表示为由冒号分隔的十六进制数字对的字符串。
     ...
     * <ul>
     * <li> 具有自然字符串表示的基本 ASN.1 类型的 DER 编码以字符串本身的形式返回。
     *      这些类型目前限于 BOOLEAN, INTEGER, OBJECT IDENTIFIER, UTCTime, GeneralizedTime 以及以下六种 ASN.1 字符串类型：UTF8String,
     *      PrintableString, T61String, IA5String, BMPString 和 GeneralString。
     * <li> 任何其他 ASN.1 类型的 DER 编码不会被解码，而是以冒号分隔的十六进制数字对的二进制字符串形式返回。
     * </ul>
     * 多值属性表示为用方括号括起来的以逗号分隔的值列表。参见
     * {@link Arrays#toString(java.lang.Object[])}。
     *
     ...
     * @param obj 比较对象
     *
     * @return 如果 {@code obj} 是 {@code PKCS12Attribute} 且它们的 DER 编码相等，则返回 true。
     */
    @Override
    ...
    private void parse(byte[] encoded) throws IOException {
        ...
        if (attrSeq.length != 2) {
            throw new IOException("PKCS12Attribute 长度无效");
        }
