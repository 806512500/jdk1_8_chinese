...
/**
 * AccessControlContext 用于基于其封装的上下文做出系统资源访问决策。
 *
 * <p>更具体地说，它封装了一个上下文，并且有一个 {@code checkPermission} 方法，
 * 该方法与 AccessController 类中的 {@code checkPermission} 方法等效，但有一个不同点：AccessControlContext
 * 的 {@code checkPermission} 方法基于其封装的上下文做出访问决策，
 * 而不是基于当前执行线程的上下文。
 *
 * <p>因此，AccessControlContext 的目的是在需要在给定上下文中进行安全检查的情况下，
 * 实际上需要从 <i>不同</i> 的上下文中进行（例如，从工作线程中）。
 *
 * <p> 通过调用 {@code AccessController.getContext} 方法创建 AccessControlContext。
 * {@code getContext} 方法会“快照”当前调用上下文，并将其放入 AccessControlContext 对象中，然后返回。一个示例调用如下：
 *
 * <pre>
 ...
 *
 * <p>
 * 不同上下文中的代码随后可以调用之前保存的 AccessControlContext 对象的
...
public final class AccessControlContext {
    ...
    // isPrivileged 和 isAuthorized 被 VM 引用 - 不要删除或更改它们的名称
    private boolean isPrivileged;
    private boolean isAuthorized = false;

    // 注意：此字段被虚拟机的本地代码直接使用。不要修改它。
     ...
     * 上下文不能为空。重复的域将从上下文中移除。
     *
     * @param context 与此上下文关联的 ProtectionDomains。
     * 非重复域将从数组中复制。后续对数组的更改不会影响此 AccessControlContext。
    ...
    /**
     * 使用给定的 {@code AccessControlContext} 和 {@code DomainCombiner} 创建新的 {@code AccessControlContext}。
     * 此构造函数将提供的 {@code DomainCombiner} 与提供的 {@code AccessControlContext} 关联。
     *
     * <p>
     *
     * @param acc 与提供的 {@code DomainCombiner} 关联的 {@code AccessControlContext}。
     *
     * @param combiner 要与提供的 {@code AccessControlContext} 关联的 {@code DomainCombiner}。
     *
     * @exception NullPointerException 如果提供的 {@code context} 为 {@code null}。
     *
    ...
    /**
     * 包私有，允许 ProtectionDomain 的调用而不执行 {@linkplain SecurityConstants.CREATE_ACC_PERMISSION}
     * 权限的安全检查
     */
    AccessControlContext(AccessControlContext acc,
        ...
        // 我们不需要在提供的 ACC 上运行 combine 方法。当上下文最初被检索时，它已经被“组合”了。
        //
        // 在此时，我们只需丢弃旧的 combiner 并使用新提供的 combiner。
     ...
     *
     * 这个“参数包装器”上下文将在实现中用于内部 doPrivileged() 调用的实际上下文参数。
     */
    ...
    {
        /*
         * 如果需要，将 doPrivileged() 上下文中的域组合到我们的包装器上下文中。
         */
        ...
        if (perms != null) {
            ...
            for (int i=0; i < perms.length; i++) {
                ...
                /*
                 * AllPermission 参数等同于调用 doPrivileged() 时不带任何限制权限。
                 */
         ...
         * 它包含所有可能被检查的域，前提是没有任何限制权限隐含请求的权限。
         */
        if (parent != null) {
    ...
    /**
     * 为 JavaSecurityAccess.doIntersectionPrivilege() 提供的构造函数
     */
    AccessControlContext(ProtectionDomain[] context,
     ...
     * <p>
     *
     * @return 与此 AccessControlContext 关联的 DomainCombiner，或如果不存在则返回 null。
     *
     * @exception SecurityException 如果安装了安全经理且调用者没有 "getDomainCombiner" SecurityPermission
     * @since 1.3
     */
    ...
    /**
     * 根据当前生效的安全策略和此对象中的上下文，确定指定权限的访问请求是否应被允许或拒绝。只有当上下文中的每个 ProtectionDomain 都隐含该权限时，请求才会被允许。否则，请求将被拒绝。
     *
     * <p>
     * 如果访问请求被允许，此方法将静默返回；否则，将抛出适当的 AccessControlException。
     *
     * @param perm 请求的权限。
     *
     * @exception AccessControlException 如果根据当前安全策略和此对象封装的上下文，指定的权限未被允许。
        ...
        throws AccessControlException
    {
        ...
        if (getDebug() != null) {
            ...
            if (!dumpDebug) {
                // 如果指定了 "codebase"，仅在指定的代码值在堆栈中时转储。
         ...
         * 在第一个不允许请求权限的域处停止（抛出异常）。
         *
         */

        /* 如果 ctxt 为 null，那么堆栈上只有系统域，
           或者第一个域是一个特权系统域。这是为了使系统代码的常见情况非常快 */
    ...
    private void checkPermission2(Permission perm) {
        ...
        /*
         * 忽略包装上下文的有限权限和父字段，因为它们已经传递到未包装的上下文中。
         */
        if (isWrapped) {
            return;
        }
        ...
        /*
         * 检查调用堆栈上的有限特权范围或此 ACC 的继承父线程调用堆栈。
         */
        if (parent != null) {
            /*
             * 作为一种优化，如果父上下文是从父线程继承的调用堆栈上下文，
             * 那么检查父上下文的保护域是多余的，因为它们已经被合并到子线程的上下文中。
    ...
    /**
     * 获取基于堆栈的上下文（this）并根据需要将其与特权或继承的上下文结合。
     * 无论分配的上下文是否来自立即封闭的有限 doPrivileged()，任何有限的特权范围都会被标记。
     * 有限的特权范围可以间接地从继承的父线程或通过 getContext() 之前捕获的分配上下文流动。
     */
    AccessControlContext optimize() {
        ...
        if (isPrivileged) {
            ...
            if (acc != null) {
                /*
                 * 如果上下文来自有限范围的 doPrivileged()，则将权限和父字段从创建它们的包装上下文中复制出来。
                 */
                if (acc.isWrapped) {
            ...
            if (acc != null) {
                /*
                 * 如果继承的上下文受到有限范围的 doPrivileged() 的限制，则将其设置为我们的父级，
                 * 以便处理与域无关的状态。
                 */
                if (acc.isLimited) {
                    parent = acc;
        ...
        // 如果堆栈上只有系统代码，则上下文可能为 null；
        // 在这种情况下，忽略堆栈上下文
        boolean skipStack = (context == null);


                    // 如果仅涉及系统代码，acc.context 可能为 null；
        // 在这种情况下，忽略分配的上下文
    ...
    private static ProtectionDomain[] combine(ProtectionDomain[] current,
        ...
        // 如果堆栈上只有系统代码，current 可能为 null；
        // 在这种情况下，忽略堆栈上下文
        boolean skipStack = (current == null);

        // 如果仅涉及系统代码，assigned 可能为 null；
        // 在这种情况下，忽略分配的上下文
        boolean skipAssigned = (assigned == null);

        int slen = (skipStack) ? 0 : current.length;

        // 优化：如果没有任何分配的上下文且堆栈长度
        // 小于或等于两个；没有必要压缩堆栈上下文，它已经是
    ...
    /*
     * 计算通过有限特权范围可能达到的额外域。除非这些可到达的域（如果有）
     * 已经包含在当前域上下文中（在这种情况下，任何有限特权范围检查都是多余的），
     * 否则将上下文标记为受有限特权范围影响。
     */
     ...
     * 检查 <i>obj</i> 是否为
     * AccessControlContext，并且具有与当前上下文相同的 ProtectionDomain 集合。
     * <P>
     * @param obj 要与当前对象进行相等性测试的对象。
     * @return 如果 <i>obj</i> 是 AccessControlContext，并且具有与当前上下文相同的 ProtectionDomain 集合，则返回 true，否则返回 false。
     */
    ...
    /*
     * 基于在受限权限范围内调用 AccessController.getContext() 时捕获的状态进行相等性比较。
     */
    private boolean equalLimitedContext(AccessControlContext that) {
        ...
        /*
         * 如果两个实例都没有受限权限范围，则比较结束。
         */
        if (!this.isLimited && !that.isLimited)
            return true;

        /*
         * 如果只有一个实例有受限权限范围，则比较结束。
         */
         if (!(this.isLimited && that.isLimited))
             return false;

        /*
         * 包装实例不应在实现类和 AccessController 之外逃逸，因此这种情况可能永远不会发生，
         * 但只有在它们具有相同的 isWrapped 状态时才有意义进行比较。
         */
        ...
        /*
         * 由于特权上下文的保护域和组合器已经通过 optimize() 方法包含在当前实例的上下文中，
         * 因此它们是无关的，我们只关心它们可能具有的任何受限权限状态。
         */
    ...
    private static boolean containsAllPDs(ProtectionDomain[] thisContext,
        ...
        //
        // 当前，ACC 中的 ProtectionDomains 不能为 null，这是由构造函数和各种优化方法强制执行的。
        // 然而，历史上这种逻辑尝试支持 null PD 的概念，因此这种逻辑继续支持该概念。
    ...
    /**
     * 返回此上下文的哈希码值。哈希码是通过将上下文中的所有保护域的哈希码进行异或运算来计算的。
     *
     * @return 此上下文的哈希码值。
     */
