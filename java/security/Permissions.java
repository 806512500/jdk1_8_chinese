...
/**
 * 该类表示一个异构的权限集合。也就是说，它包含不同类型的 Permission 对象，这些对象被组织成
 * PermissionCollections。例如，如果任何
 * {@code java.io.FilePermission} 对象被添加到此类的实例中，它们将全部存储在一个
 * PermissionCollection 中。这是通过调用 FilePermission 类中的
 * {@code newPermissionCollection} 方法返回的 PermissionCollection。同样，任何
 * {@code java.lang.RuntimePermission} 对象都被存储在通过调用
 * RuntimePermission 类中的 {@code newPermissionCollection} 方法返回的
 * PermissionCollection 中。因此，此类表示一个 PermissionCollections 的集合。
 *
 * <p>当调用 {@code add} 方法添加权限时，权限将被存储在适当的 PermissionCollection 中。如果还没有这样的
 * 集合，将确定权限对象的类，并对该类调用
 * {@code newPermissionCollection} 方法以创建 PermissionCollection 并将其添加到 Permissions 对象中。如果
 * {@code newPermissionCollection} 返回 null，则将创建并使用一个默认的
 * PermissionCollection，该集合使用哈希表。每个哈希表条目都存储一个 Permission 对象作为键和值。
 *
 * <p>通过 {@code elements} 方法返回的枚举不是 <em>快速失败</em> 的。在枚举集合时不应
 * 对集合进行修改。
 *
 * @see Permission
...
{
    /**
     * 键是权限类，值是该类的 PermissionCollection。
     * 不序列化；请参阅类末尾的序列化部分。
     */
    ...
    /**
     * 将权限对象添加到该权限所属类的 PermissionCollection 中。例如，如果 <i>permission</i> 是一个
     * FilePermission，它将被添加到此 Permissions 对象中存储的 FilePermissionCollection 中。
     *
     * 如果还没有适当的集合，此方法将创建一个新的 PermissionCollection 对象（并添加权限）。<p>
     *
    ...
    /**
     * 检查此对象的指定权限类的 PermissionCollection 是否隐含了
     * <i>permission</i> 对象中表达的权限。如果适当 PermissionCollection 中的权限组合
     * （例如，对于 FilePermission 的 FilePermissionCollection）一起隐含了指定的权限，则返回 true。
     *
     * <p>例如，假设此 Permissions 对象中有一个 FilePermissionCollection，其中包含一个指定
     * “读取”访问权限的 FilePermission，适用于 /tmp 目录及其所有子目录中的所有文件，以及另一个指定
     * “写入”访问权限的 FilePermission，适用于 /tmp/scratch/foo 目录中的所有文件。
     * 那么，如果 {@code implies} 方法
     * 被调用时指定对 /tmp/scratch/foo 目录中的文件同时具有“读取”和“写入”访问权限，则返回 {@code true}。
     *
     * <p>此外，如果此 PermissionCollection 包含 AllPermission，此方法将始终返回 true。
     * <p>
     * @param permission 要检查的权限对象。
     *
    ...
    /**
     * 获取此 Permissions 对象中与 <i>p</i> 类型相同的权限的 PermissionCollection。
     * 例如，如果 <i>p</i> 是 FilePermission，
     * 则将返回存储在此 Permissions 对象中的 FilePermissionCollection。
     *
     * 如果 createEmpty 为 true，
     * 则此方法会在尚未存在指定类型的权限对象时创建一个新的 PermissionCollection 对象。
     * 为此，它首先调用 <i>p</i> 上的 {@code newPermissionCollection} 方法。
     * Permission 类的子类如果需要将其权限存储在特定的 PermissionCollection 对象中以在调用
     * {@code PermissionCollection.implies} 方法时提供正确的语义，则会覆盖该方法。
     * 如果调用返回 PermissionCollection，则该集合将存储在此 Permissions 对象中。如果调用返回 null 且 createEmpty 为 true，则
     * 此方法将实例化并存储一个使用哈希表存储其权限对象的默认 PermissionCollection。
     *
     * 在为未解析的权限创建空的 PermissionCollection 时忽略 createEmpty，因为确定要使用的 PermissionCollection 会带来开销。
     *
     * 当此方法从 implies() 调用时，createEmpty 应设置为 false，因为这会带来创建和添加一个只会返回 false 的空 PermissionCollection 的额外开销。
     * 当从 add() 调用时，应将其设置为 true。
     */
     ...
     *
     * @return 包含未解析权限的 PermissionCollection，
     *  或者如果没有类型为 p 的未解析权限，则返回 null。
     *
     */
    private PermissionCollection getUnresolvedPermissions(Permission p)
    {
        // 在同步方法中调用，因此 permsMap 不需要锁

        UnresolvedPermissionCollection uc =
        (UnresolvedPermissionCollection) permsMap.get(UnresolvedPermission.class);

        // 如果 uc 为 null，则没有未解析的权限
        if (uc == null)
            return null;
        ...
        // 如果 unresolvedPerms 为 null，则没有此类型的未解析权限
     ...
     */
    /*
     * 将 permsMap 字段的内容作为 Hashtable 写出，以与早期版本的序列化兼容。allPermission 保持不变。
     */
    private void writeObject(ObjectOutputStream out) throws IOException {
        ...
        Hashtable<Class<?>, PermissionCollection> perms =
            new Hashtable<>(permsMap.size()*2); // 不同步；估计
        synchronized (this) {
        ...
        pfields.put("allPermission", allPermission); // 不同步；陈旧性可接受
        pfields.put("perms", perms);
        out.writeFields();
    }

    /*
     * 读取 Class/PermissionCollections 的 Hashtable 并将其保存在
     * permsMap 字段中。读取 allPermission。
     */
    ...
    ClassNotFoundException {
        ...
        // 获取权限
        // writeObject 为 perms 键写入一个 Hashtable<Class<?>, PermissionCollection>，
        // 因此此转换是安全的，除非数据损坏。
...
final class PermissionsEnumerator implements Enumeration<Permission> {
    ...
    public Permission nextElement() {
        ...
        // hasMoreElements 会将 permset 更新为下一个有内容的 permset...
...
implements Serializable
{
     ...
     */
    /*
     * 为了与早期版本的序列化兼容，将 permsMap 字段的内容作为 Hashtable 写出。
     */
    private void writeObject(ObjectOutputStream out) throws IOException {
        // 不调用 out.defaultWriteObject()


                    // 将权限复制到一个 Hashtable 中
    ...
    /*
     * 读取一个 Permission/Permission 的 Hashtable，并将其保存在
     * permsMap 字段中。
     */
    private void readObject(ObjectInputStream in) throws IOException,
    ClassNotFoundException {
