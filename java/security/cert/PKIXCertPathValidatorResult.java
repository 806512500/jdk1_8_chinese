 ...
 *
 * <p>所有 {@code PKIXCertPathValidatorResult} 对象都包含验证算法产生的有效策略树和主体公钥，
 * 以及描述作为证书路径信任锚点的证书颁发机构 (CA) 的 {@code TrustAnchor}。
 * <p>
 * <b>并发访问</b>
 * <p>
 * 除非另有说明，否则本类中定义的方法不是线程安全的。需要并发访问单个
 * 对象的多个线程应同步并提供必要的锁定。每个操作
 * 分别处理不同对象的多个线程不需要同步。
 *
...
public class PKIXCertPathValidatorResult implements CertPathValidatorResult {
    ...
    /**
     * 返回 PKIX 证书路径验证算法产生的有效策略树的根节点。返回的
     * {@code PolicyNode} 对象及其通过公共方法返回的任何对象都是不可变的。
     *
     * <p>大多数应用程序不需要检查有效策略树。它们可以通过设置
     * {@code PKIXParameters} 中的策略相关参数来实现其策略处理目标。然而，更
     * 高级的应用程序，特别是那些处理策略限定符的应用程序，可能需要使用
    ...
    /**
     * 返回证书路径的主体（目标）的公钥，包括适用的任何继承的公钥参数。
     *
