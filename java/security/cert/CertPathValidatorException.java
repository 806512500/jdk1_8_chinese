 ...
 * <p>
 * {@code CertPathValidatorException} 还可能包括在抛出异常时正在验证的证书路径、导致异常被抛出的证书路径中的证书索引以及导致失败的原因。使用 {@link #getCertPath getCertPath}、{@link #getIndex getIndex} 和 {@link #getReason getReason} 方法检索此信息。
 *
 * <p>
 * <b>并发访问</b>
 * <p>
...
public class CertPathValidatorException extends GeneralSecurityException {
    ...
    /**
     * @serial 导致异常被抛出的证书路径中的证书索引
     */
    private int index = -1;

    /**
     * @serial 抛出异常时正在验证的 {@code CertPath}
     */
    ...
    /**
     * 创建一个包装指定可抛出对象的 {@code CertPathValidatorException}。这允许将任何异常转换为 {@code CertPathValidatorException}，同时保留有关包装异常的信息，这些信息可能对调试有用。详细消息设置为 ({@code cause==null ? null : cause.toString()})
     * （通常包含 cause 的类和详细消息）。
     *
     ...
     * @param index 证书路径中导致错误的证书索引（或 -1 如果不适用）。注意，证书路径中的证书列表是以零为基础的。
     ...
     * 返回抛出异常时正在验证的证书路径。
     *
     * @return 抛出异常时正在验证的 {@code CertPath}（如果未指定，则返回 {@code null}）
     */
    ...
    /**
     * 返回导致异常抛出的证书在证书路径中的索引。请注意，{@code CertPath} 中的证书列表是以零为基础的。如果没有设置索引，则返回 -1。
     *
     * @return 已设置的索引，如果没有设置则返回 -1
     */
    public int getIndex() {
        return this.index;
    }
    ...
    /**
     * 返回验证失败的原因。该原因与 {@link #getIndex} 返回的证书索引相关联。
     *
     * @return 验证失败的原因，如果未指定原因，则返回 {@code BasicReason.UNSPECIFIED}
     *
     * @since 1.7
     */
    ...
    /**
     * BasicReason 枚举了任何类型的证书路径可能无效的潜在原因。
     *
     * @since 1.7
     */
