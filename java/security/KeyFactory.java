 ...
 * 或者以适当的格式检索密钥对象的底层密钥材料。
 *
 * <P> 同一个密钥可能存在多个兼容的密钥规范。
 * 例如，DSA 公钥可以使用 {@code DSAPublicKeySpec} 或
 * {@code X509EncodedKeySpec} 来指定。密钥工厂可以用于在兼容的密钥规范之间进行转换。
 *
 * <P> 以下是如何使用密钥工厂实例化 DSA 公钥的示例。
 * 假设 Alice 收到了 Bob 发送的数字签名。
 * Bob 还发送了他自己的公钥（以编码格式）来验证他的签名。Alice 然后执行以下操作：
 *
 * <pre>
...
public class KeyFactory {
     ...
     * @return 新的 KeyFactory 对象。
     *
     * @exception NoSuchAlgorithmException 如果指定算法的 KeyFactorySpi
     *          实现不可用。
     *
    ...
    public static KeyFactory getInstance(String algorithm, String provider)
        ...
        return new KeyFactory((KeyFactorySpi)instance.impl,
            instance.provider, algorithm);
                ...
                /**
     * 返回一个 KeyFactory 对象，该对象可以转换指定算法的公钥/私钥。
     *
     * <p> 返回一个新的 KeyFactory 对象，该对象封装了从指定 Provider 对象中获取的
     * KeyFactorySpi 实现。请注意，指定的 Provider 对象不必在提供程序列表中注册。
     *
     * @param algorithm 请求的密钥算法的名称。
     * 参见 <a href=
     * "{@docRoot}/../technotes/guides/security/StandardNames.html#KeyFactory">
     ...
     * @return 新的 KeyFactory 对象。
     *
     * @exception NoSuchAlgorithmException 如果从指定的 Provider 对象中无法获得指定算法的 KeyFactorySpi
     *          实现。
     *
     * @exception IllegalArgumentException 如果指定的提供程序为 null。
     *
     * @see Provider
     *
     * @since 1.4
    ...
    /**
     * 更新此类的活动 KeyFactorySpi 并返回下一个用于故障转移的实现。如果没有更多的实现可用，
    ...
    /**
     * 返回给定密钥对象的规范（密钥材料）。{@code keySpec} 指定了应返回的规范类。
     * 例如，可以是 {@code DSAPublicKeySpec.class}，表示密钥材料应返回为
    ...
    public final <T extends KeySpec> T getKeySpec(Key key, Class<T> keySpec)
        ...
        throw new InvalidKeySpecException
                ...
                /**
     * 将一个提供者可能未知或不可信的密钥对象转换为与此密钥工厂对应的密钥对象。
     *
