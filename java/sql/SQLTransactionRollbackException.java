...
/**
 * 当 SQLState 类值为 '<i>40</i>' 或在供应商指定的条件下抛出的 {@link SQLException} 子类。这表示由于死锁或其他事务序列化失败，当前语句被数据库自动回滚。
 * <p>
...
public class SQLTransactionRollbackException extends SQLTransientException {
     ...
     * <p>
     * @param cause 该 <code>SQLException</code> 的根本原因（通过 <code>getCause()</code> 方法保存以供稍后检索）；可能为 null，表示原因不存在或未知。
     * @since 1.6
     */
