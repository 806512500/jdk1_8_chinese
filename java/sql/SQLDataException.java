...
/**
 * {@link SQLException} 的子类，在 SQLState 类值为 '<i>22</i>' 或在供应商指定的条件下抛出。这表示各种数据错误，包括但不限于数据转换错误、除以 0 和函数的无效参数。
 * <p>
 * 请参阅您的驱动程序供应商文档，以了解可能抛出此 <code>Exception</code> 的供应商指定条件。
 * @since 1.6
 */
public class SQLDataException extends SQLNonTransientException {
        ...
        /**
         * 构造一个带有给定 <code>reason</code> 的 <code>SQLDataException</code> 对象。
        ...
        /**
         * 构造一个带有给定 <code>reason</code> 和 <code>SQLState</code> 的 <code>SQLDataException</code> 对象。供应商代码初始化为 0。
         *
         * <code>cause</code> 未初始化，可以后续通过调用
         * {@link Throwable#initCause(java.lang.Throwable)} 方法进行初始化。
         * <p>
        ...
        /**
         * 构造一个带有给定 <code>reason</code>、<code>SQLState</code> 和 <code>vendorCode</code> 的 <code>SQLDataException</code> 对象。
         *
         * <code>cause</code> 未初始化，可以后续通过调用
         * {@link Throwable#initCause(java.lang.Throwable)} 方法进行初始化。
         * <p>
    ...
    /**
     * 构造一个带有给定 <code>cause</code> 的 <code>SQLDataException</code> 对象。
     * <code>SQLState</code> 初始化为 <code>null</code>，供应商代码初始化为 0。
    ...
    /**
     * 构造一个带有给定 <code>reason</code> 和 <code>cause</code> 的 <code>SQLDataException</code> 对象。
     * <code>SQLState</code> 初始化为 <code>null</code>，供应商代码初始化为 0。
     * <p>
     * @param reason 异常的描述。
     * @param cause 该 <code>SQLException</code> 的根本原因（稍后通过 <code>getCause()</code> 方法检索）；可以为 null，表示原因不存在或未知。
    ...
    /**
     * 构造一个带有给定 <code>reason</code>、<code>SQLState</code> 和 <code>cause</code> 的 <code>SQLDataException</code> 对象。
     * 供应商代码初始化为 0。
     * <p>
     * @param reason 异常的描述。
     * @param SQLState 识别异常的 XOPEN 或 SQL:2003 代码
    ...
    /**
     * 构造一个带有给定 <code>reason</code>、<code>SQLState</code>、<code>vendorCode</code> 和 <code>cause</code> 的 <code>SQLDataException</code> 对象。
     * <p>
     * @param reason 异常的描述
     * @param SQLState 识别异常的 XOPEN 或 SQL:2003 代码
     * @param vendorCode 数据库供应商特定的异常代码
