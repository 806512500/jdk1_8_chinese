...
/**
 * 当 SQLState 类值为 '<i>23</i>' 或在供应商指定的条件下抛出的 {@link SQLException} 的子类。
 * 这表明违反了完整性约束（外键、主键或唯一键）。
 * <p>
...
public class SQLIntegrityConstraintViolationException extends SQLNonTransientException {
        ...
        /**
         * 使用给定的 <code>reason</code> 和 <code>SQLState</code> 构造一个 <code>SQLIntegrityConstraintViolationException</code> 对象。
         *
         * <code>cause</code> 未初始化，可以通过调用
        ...
        /**
         * 使用给定的 <code>reason</code>、<code>SQLState</code> 和 <code>vendorCode</code> 构造一个 <code>SQLIntegrityConstraintViolationException</code> 对象。
         *
         * <code>cause</code> 未初始化，可以通过调用
         * {@link Throwable#initCause(java.lang.Throwable)} 方法随后初始化。
         * <p>
        ...
        /**
         * 使用给定的 <code>cause</code> 构造一个 <code>SQLIntegrityConstraintViolationException</code> 对象。
         * <code>SQLState</code> 初始化为 <code>null</code>，供应商代码初始化为 0。
    ...
    /**
     * 使用给定的 <code>reason</code> 和 <code>cause</code> 构造一个 <code>SQLIntegrityConstraintViolationException</code> 对象。
     * <code>SQLState</code> 初始化为 <code>null</code>，供应商代码初始化为 0。
     * <p>
     * @param reason 异常的描述。
     * @param cause 该 <code>SQLException</code> 的根本原因（通过 <code>getCause()</code> 方法稍后检索）；可以为 null，表示原因不存在或未知。
    ...
    /**
     * 使用给定的 <code>reason</code>、<code>SQLState</code> 和 <code>cause</code> 构造一个 <code>SQLIntegrityConstraintViolationException</code> 对象。
     * 供应商代码初始化为 0。
     * <p>
     * @param reason 异常的描述。
     * @param SQLState 识别异常的 XOPEN 或 SQL:2003 代码
    ...
    /**
     * 使用给定的 <code>reason</code>、<code>SQLState</code>、<code>vendorCode</code> 和 <code>cause</code> 构造一个 <code>SQLIntegrityConstraintViolationException</code> 对象。
     * <p>
     * @param reason 异常的描述
     * @param SQLState 识别异常的 XOPEN 或 SQL:2003 代码
     * @param vendorCode 数据库供应商特定的异常代码
