...
/**
 * <code>DateFormatSymbols</code> 是一个公共类，用于封装日期时间格式化数据，例如月份名称、星期几的名称和时区数据。
 * <code>SimpleDateFormat</code> 使用 <code>DateFormatSymbols</code> 来封装这些信息。
 *
 * <p>
 * 通常你不应该直接使用 <code>DateFormatSymbols</code>。而是建议使用 <code>DateFormat</code> 类的工厂方法来创建日期时间格式化器：
 * <code>getTimeInstance</code>、<code>getDateInstance</code> 或 <code>getDateTimeInstance</code>。
 * 这些方法会自动为格式化器创建一个 <code>DateFormatSymbols</code>，因此你不需要手动创建。格式化器创建后，你可以使用 <code>setPattern</code> 方法修改其格式模式。
 * 有关使用 <code>DateFormat</code> 的工厂方法创建格式化器的更多信息，请参见 {@link DateFormat}。
 *
 * <p>
 * 如果你决定为特定的区域设置使用特定的格式模式创建日期时间格式化器，可以这样做：
 * <blockquote>
 * <pre>
 ...
 *
 * <p>
 * <code>DateFormatSymbols</code> 对象是可克隆的。当你获得一个 <code>DateFormatSymbols</code> 对象时，可以自由修改日期时间格式化数据。
 * 例如，你可以将本地化的日期时间格式模式字符替换为你容易记住的字符，或者将代表城市更改为你喜爱的城市。
 *
 * <p>
 * 可以添加新的 <code>DateFormatSymbols</code> 子类以支持 <code>SimpleDateFormat</code> 为更多区域设置提供日期时间格式化。
 *
 * @see          DateFormat
...
public class DateFormatSymbols implements Serializable, Cloneable {
    ...
    /**
     * 通过从默认 {@link java.util.Locale.Category#FORMAT FORMAT} 区域设置的资源加载格式数据来构造 <code>DateFormatSymbols</code> 对象。
     * 此构造函数只能构造由 Java 运行时环境支持的区域设置的实例，而不是由已安装的
     * {@link java.text.spi.DateFormatSymbolsProvider DateFormatSymbolsProvider} 实现支持的区域设置。为了实现完整的区域设置覆盖，请使用
    ...
    /**
     * 通过从给定区域设置的资源加载格式数据来构造 <code>DateFormatSymbols</code> 对象。
     * 此构造函数只能构造由 Java 运行时环境支持的区域设置的实例，而不是由已安装的
     * {@link java.text.spi.DateFormatSymbolsProvider DateFormatSymbolsProvider} 实现支持的区域设置。为了实现完整的区域设置覆盖，请使用
     * {@link #getInstance(Locale) getInstance} 方法。
     *
    ...
    /**
     * 星期字符串。例如："Sunday"，"Monday" 等。一个包含 8 个字符串的数组，通过 <code>Calendar.SUNDAY</code>，
     * <code>Calendar.MONDAY</code> 等索引。元素 <code>weekdays[0]</code> 被忽略。
     * @serial
     */
    String weekdays[] = null;

    /**
     * 短星期字符串。例如："Sun"，"Mon" 等。一个包含 8 个字符串的数组，通过 <code>Calendar.SUNDAY</code>，
     * <code>Calendar.MONDAY</code> 等索引。元素 <code>shortWeekdays[0]</code> 被忽略。
     * @serial
     */
    String shortWeekdays[] = null;

    /**
     * AM 和 PM 字符串。例如："AM" 和 "PM"。一个包含 2 个字符串的数组，通过 <code>Calendar.AM</code> 和
     * <code>Calendar.PM</code> 索引。
     * @serial
     */
    String ampms[] = null;

    /**
     * 该地区的时区本地化名称。这是一个大小为 <em>n</em> x <em>m</em> 的二维字符串数组，其中 <em>m</em> 至少为 5。
     * 每个 <em>n</em> 行是一个包含单个 <code>TimeZone</code> 的本地化名称的条目。
     * 每个这样的行包含（<code>i</code> 从 0..<em>n</em>-1 变化）：
     * <ul>
    ...
    /**
     * 未本地化的日期时间模式字符。例如：'y'，'d' 等。所有地区都使用相同的这些未本地化模式字符。
     */
    ...
    /**
     * 本地化的日期时间模式字符。例如，某个地区可能希望在日期格式模式字符串中使用 'u' 而不是 'y' 来表示年份。
     * 这个字符串必须恰好 18 个字符长，字符的索引由 <code>DateFormat.ERA_FIELD</code>，
     * <code>DateFormat.YEAR_FIELD</code> 等描述。因此，如果字符串为 "Xz..."，则本地化模式将使用 'X' 表示纪元，'z' 表示年份。
     * @serial
     */
    String  localPatternChars = null;

    /**
     * 用于初始化此 DateFormatSymbols 对象的地区。
     *
     * @since 1.6
     * @serial
     */
    ...
    /**
     * 返回此类的 <code>getInstance</code> 方法可以返回本地化实例的所有地区数组。
     * 返回的数组表示 Java 运行时支持的地区和已安装的
     * {@link java.text.spi.DateFormatSymbolsProvider DateFormatSymbolsProvider}
     * 实现支持的地区的并集。它必须至少包含一个等于 {@link java.util.Locale#US Locale.US} 的 <code>Locale</code> 实例。
     *
     * @return 可以获得本地化 <code>DateFormatSymbols</code> 实例的地区数组。
     * @since 1.6
     */
    ...
    /**
     * 获取默认地区的 <code>DateFormatSymbols</code> 实例。此方法提供了访问 <code>DateFormatSymbols</code>
     * 实例的途径，这些实例既支持 Java 运行时本身支持的地区，也支持已安装的
     * {@link java.text.spi.DateFormatSymbolsProvider DateFormatSymbolsProvider}
     * 实现支持的地区。
     * <p>这等同于调用 {@link #getInstance(Locale)
    ...
    public static final DateFormatSymbols getInstance() {
        return getInstance(Locale.getDefault(Locale.Category.FORMAT));
                ...
                /**
     * 获取指定区域设置的 <code>DateFormatSymbols</code> 实例。此方法提供了对 Java 运行时本身支持的区域设置以及已安装的
     * {@link java.text.spi.DateFormatSymbolsProvider DateFormatSymbolsProvider}
     * 实现支持的区域设置的 <code>DateFormatSymbols</code> 实例的访问。
    ...
    /**
     * 返回由提供者提供的或在缓存中找到的 DateFormatSymbols。注意，此方法返回的是缓存的实例，
     * 而不是其克隆。因此，该实例不应直接提供给应用程序。
     */
     ...
     *
     * <p>如果语言要求格式化和独立使用时有不同的形式，此方法返回格式化形式的月份名称。例如，捷克语中 January 的首选名称是 <em>ledna</em>（格式化形式），而独立形式是 <em>leden</em>。在这种情况下，此方法返回 {@code "ledna"}。有关更多详细信息，请参阅 <a href="http://unicode.org/reports/tr35/#Calendar_Elements">
    ...
    /**
     * 获取简短的月份字符串。例如："Jan"，"Feb" 等。
     *
     * <p>如果语言要求格式化和独立使用时有不同的形式，此方法返回格式化形式的简短月份名称。例如，加泰罗尼亚语中 January 的首选缩写是 <em>de gen.</em>（格式化形式），而独立形式是 <em>gen.</em>。在这种情况下，此方法返回 {@code "de gen."}。有关更多详细信息，请参阅 <a href="http://unicode.org/reports/tr35/#Calendar_Elements">
     * Unicode Locale Data Markup Language (LDML) 规范中的日历元素</a>。
     *
     * @return 简短的月份字符串。
     */
    ...
    /**
     * 获取简短的星期字符串。例如："Sun"，"Mon" 等。
     * @return 简短的星期字符串。使用 <code>Calendar.SUNDAY</code>，
     * <code>Calendar.MONDAY</code> 等来索引结果数组。
     */
    ...
    public String[] getAmPmStrings() {
        return Arrays.copyOf(ampms, ampms.length);
                ...
                /**
     * 设置上午/下午字符串。例如："AM" 和 "PM"。
     * @param newAmpms 新的上午/下午字符串。
     */
    ...
    /**
     * 获取时区字符串。不建议使用此方法；请改用
     * {@link java.util.TimeZone#getDisplayName() TimeZone.getDisplayName()}
     * 代替。
     * <p>
     * 返回的值是一个
     * 大小为 <em>n</em> 乘以 <em>m</em> 的二维字符串数组，其中 <em>m</em> 至少为 5。每个 <em>n</em> 行
     * 是一个包含单个 <code>TimeZone</code> 的本地化名称的条目。
     * 每个这样的行包含（<code>i</code> 从 0..<em>n</em>-1 变化）：
     * <ul>
     * <li><code>zoneStrings[i][0]</code> - 时区 ID</li>
     * <li><code>zoneStrings[i][1]</code> - 标准时间的时区长名称</li>
     ...
     * 所有其他条目都是本地化名称。如果时区不实施夏令时，则不应使用夏令时名称。
     * <p>
     * 如果已对这个 <code>DateFormatSymbols</code> 实例调用了
     * {@link #setZoneStrings(String[][]) setZoneStrings}，则返回该调用提供的字符串。否则，返回的数组
     * 包含 Java 运行时和已安装的
    ...
    /**
     * 使用区域设置数据初始化此 DateFormatSymbols。如果可用，此方法使用给定区域设置的缓存 DateFormatSymbols 实例。如果没有缓存的实例，
     * 此方法创建一个未初始化的实例，并从该区域设置的资源包中填充其字段，并缓存该实例。注意：zoneStrings 在此方法中未初始化。
     */
    private void initializeData(Locale locale) {
        ...
        if (ref == null || (dfs = ref.get()) == null) {
            ...
            // 从 locale 的 ResourceBundle 初始化字段。
            ...
            // 如果包的区域设置不是目标区域设置，则为包的区域设置添加另一个缓存条目。
     ...
     * 获取给定时区 ID 的索引以获取用于格式化的时区字符串。时区 ID 仅用于编程查找。未本地化！！！
     * @param ID 给定的时区 ID。
    ...
    final int getZoneIndex(String ID) {
        ...
        /*
         * 为了性能原因，getZoneIndex 已重写。不是每次遍历 zoneStrings 数组，而是缓存上次使用的时区索引
         */
    ...
    /**
     * getZoneStrings() 的包装方法，该方法从 java.text 包内部调用，不修改返回的数组，因此不需要创建防御性副本。
     */
    ...
    /**
     * 克隆源 DateFormatSymbols 中的所有数据成员到目标 DateFormatSymbols。
     *
    ...
    /**
     * 在确保 <code>zoneStrings</code> 字段已初始化以保证向后兼容性后，写入默认的可序列化数据。
     *
     * @since 1.6
     */
