...
/**
 * 该类包含 RuleBasedCollator 的静态状态：用于排序例程的各种表。多个 RuleBasedCollators
 * 可以共享一个 RBCollationTables 对象，从而减少内存需求并提高性能。
 */
final class RBCollationTables {
    //===========================================================================================
    //  下图显示了 RBCollationTables 对象的数据结构。
    //  假设我们有这样一个规则，其中 'o-umlaut' 是 Unicode 字符 0x00F6。
    //  "a, A < b, B < c, C, ch, cH, Ch, CH < d, D ... < o, O; 'o-umlaut'/E, 'O-umlaut'/E ...".
    //  该规则表示，'ch' 连字和 'c' 仅在三级差异上排序，并且 'o-umlaut' 始终扩展为 'e' 进行排序。
    //
    // 映射表                     收缩列表           扩展列表
    // (包含所有 Unicode 字符
    ...
    final class BuildAPI {
        ...
        /**
         * 该函数由 RBTableBuilder 使用来填充此对象的所有成员。(实际上，构建器类充当此
         * 类的“朋友”，但为了避免改变太多逻辑，它携带所有这些变量的“影子”副本，直到构建过程结束，
         * 然后将它们一次性复制到实际的表对象中。此函数执行“一次性复制”。
         * @param f2ary 法国二级标志的值
         * @param swap 东南亚交换规则的值
         * @param map 排序器的字符映射表
         * @param cTbl 排序器的收缩字符表
         * @param eTbl 排序器的扩展字符表
         * @param cFlgs 参与收缩字符序列的字符的哈希表
         * @param mso 最大二级顺序的值
         * @param mto 最大三级顺序的值
         */
    ...
    // ==============================================================
    // 内部使用（由 CollationElementIterator 使用）
    // ==============================================================
    ...
    /**
     * 如果此字符出现在任何收缩字符序列中，则返回 true。 (由 CollationElementIterator.setOffset() 使用。)
     */
    ...
    int getMaxExpansion(int order) {
        ...
        if (expandTable != null) {
            // 目前，这会线性搜索整个扩展表。如果排序器有许多扩展，
            // 这可能会导致性能问题，但在实践中很少发生
