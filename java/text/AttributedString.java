...
/**
 * 一个 AttributedString 持有文本和相关的属性信息。它可以在某些情况下用作实际的数据存储，
 * 例如文本读取器希望通过 AttributedCharacterIterator 接口访问带属性的文本。
 *
 * <p>
 * 属性是一个键值对，由键标识。给定字符上的两个属性不能具有相同的键。
 *
 * <p>属性的值是不可变的，或者客户端或存储不应对其进行修改。它们总是按引用传递，而不是克隆。
 *
...
public class AttributedString {
    ...
    int runCount;                   // 实际的运行数，<= runArraySize
    ...
    public AttributedString(AttributedCharacterIterator text) {
        // 如果性能至关重要，此构造函数应在此处实现，而不是调用子范围的构造函数。
        // 我们可以避免循环中的某些范围检查。
        this(text, text.getBeginIndex(), text.getEndIndex(), null);
    }

    /**
     * 使用由 AttributedCharacterIterator 表示的带属性文本的子范围构造 AttributedString 实例。
     * 如果给定范围产生空文本，所有属性将被丢弃。注意，任何由 Annotation 对象包装的属性
     * 都将被丢弃，因为它们不适用于原始属性范围的子范围。
     *
     * @param text 该带属性字符串的文本。
     * @param beginIndex 范围中第一个字符的索引。
     * @param endIndex 范围中最后一个字符之后的字符索引。
     * @exception NullPointerException 如果 <code>text</code> 为 null。
     * @exception IllegalArgumentException 如果由 beginIndex 和 endIndex 给定的子范围超出文本范围。
    ...
    /**
     * 使用由 AttributedCharacterIterator 表示的带属性文本的子范围构造 AttributedString 实例。
     * 只有与给定属性匹配的属性将被纳入实例。如果给定范围产生空文本，所有属性将被丢弃。
     * 注意，任何由 Annotation 对象包装的属性将被丢弃，因为它们不适用于原始属性范围的子范围。
     *
     * @param text 该带属性字符串的文本。
     * @param beginIndex 范围中第一个字符的索引。
     * @param endIndex 范围中最后一个字符之后的字符索引。
     * @param attributes 指定要从文本中提取的属性。如果指定 null，将使用所有可用的属性。
     * @exception NullPointerException 如果 <code>text</code> 为 null。
     * @exception IllegalArgumentException 如果由 beginIndex 和 endIndex 给定的子范围超出文本范围。
     * @see java.text.Annotation
     */
    public AttributedString(AttributedCharacterIterator text,
        ...
        // 获取并设置每个属性名称的属性运行。需要从文本的顶部扫描，以便我们可以丢弃不再应用于子文本段的任何 Annotation。
     ...
     * @exception IllegalArgumentException 如果 AttributedString 的长度为 0
     * （属性不能应用于 0 长度的范围）。
     */
     ...
     * @param endIndex 范围中最后一个字符之后的字符索引。
     * @exception NullPointerException 如果 <code>attribute</code> 为 null。
     * @exception IllegalArgumentException 如果 beginIndex 小于 0，endIndex 大于字符串的长度，
     * 或 beginIndex 和 endIndex 一起不定义字符串的非空子范围。
     */
    public void addAttribute(Attribute attribute, Object value,
     ...
     * @exception IllegalArgumentException 如果 beginIndex 小于 0，endIndex 大于字符串的长度，
     * 或 beginIndex 和 endIndex 一起不定义字符串的非空子范围，且 attributes 参数不是空 Map。
     */
                              ...
                              int beginIndex, int endIndex)
    {
        ...
        if (beginIndex == endIndex) {
            if (attributes.isEmpty())
                return;
            throw new IllegalArgumentException("不能向0长度的文本添加属性");
        }
    ...
    /**
     * 确保在偏移处有运行中断，返回运行的索引。如果这导致拆分运行，可能会发生两件事：
     * <ul>
     * <li>如果copyAttrs为true，现有运行的属性将被放置在新创建的两个运行中。
     * <li>如果copyAttrs为false，现有运行的属性将不会被复制到断点右侧（>= 偏移）的运行中，
     * 但会存在于断点左侧（< 偏移）的运行中。
     * </ul>
     */
    ...
    /**
     * 创建一个AttributedCharacterIterator实例，提供对此字符串全部内容的访问。
     *
    ...
    /**
     * 创建一个AttributedCharacterIterator实例，提供对此字符串选定内容的访问。
     * 实现者可能不需要通过迭代器提供未在attributes中列出的属性信息。
     * 如果列表为null，则应提供所有可用的属性信息。
     *
     ...
     * @return 提供对文本及其属性访问的迭代器
     * @exception IllegalArgumentException 如果beginIndex小于0，endIndex大于字符串长度，或beginIndex大于endIndex。
     */
    ...
    // 所有（除了length）读取操作都是私有的，
    // 因为AttributedString实例是通过迭代器访问的。

    // length是包私有的，以便CharacterIteratorFieldDelegate可以
    // 在不创建AttributedCharacterIterator的情况下访问它。
    int length() {
        return text.length();
    }
    ...
    // 获取属性值，但只有当其范围不超出beginIndex..endIndex范围时才返回注解
    ...
    /**
     * 将从offset到下一个运行中断（通常是文本的末尾）的范围的属性设置为attrs中指定的属性。
     * 这只应在构造函数中调用！
     */
    ...
    final private class AttributedStringIterator implements AttributedCharacterIterator {
        ...
        // 但是，我们同步对AttributedString的访问，因为它更可能在多个线程之间共享。
