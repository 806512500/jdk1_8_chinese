 ...
 * 材料是根据 Taligent 和 Sun 之间的许可协议提供的。这项技术受多项美国和国际专利保护。此通知和对 Taligent 的归属不得删除。
...
/**
 * <code>ChoiceFormat</code> 允许您将格式附加到数字范围。
 * 通常在 <code>MessageFormat</code> 中用于处理复数。
 * 选择是通过一个递增的双精度浮点数列表指定的，其中每个项目
 * 指定到下一个项目的半开区间：
 * <blockquote>
 * <pre>
 ...
 * </pre>
 * </blockquote>
 * 如果没有匹配项，那么将使用第一个或最后一个索引，具体取决于数字 (X) 是否太低或太高。如果限制数组不是
 * 递增顺序，格式化结果将不正确。ChoiceFormat 还接受 <code>&#92;u221E</code> 作为无穷大(INF)的等效。
 *
 * <p>
 * <strong>注意：</strong>
 * <code>ChoiceFormat</code> 与其他 <code>Format</code>
 * 类不同，您需要使用构造函数（而不是 <code>getInstance</code> 风格的工厂
 * 方法）创建 <code>ChoiceFormat</code> 对象。工厂方法不是必需的，因为 <code>ChoiceFormat</code>
 * 不需要为特定区域设置进行任何复杂的设置。实际上，
 * <code>ChoiceFormat</code> 不实现任何特定于区域设置的行为。
 *
 * <p>
 * 创建 <code>ChoiceFormat</code> 时，必须指定格式数组和限制数组。这些数组的长度必须相同。
 * 例如，
 * <ul>
 * <li>
 ...
 *     (<code>nextDouble</code> 可用于获取下一个更高的双精度浮点数，以
 *     形成半开区间。)
 * </ul>
 *
 * <p>
...
public class ChoiceFormat extends NumberFormat {
    ...
    /**
     * 设置用于格式化的选项。
     * @param limits 包含你希望解析的最大值，应按升序排列。当格式化 X 时，选择将是 i，其中
     * limit[i] &le; X {@literal <} limit[i+1]。
     * 如果限制数组不是按升序排列的，格式化的结果将不正确。
     * @param formats 是你希望为每个限制使用的格式。
     * 它们可以是 Format 对象或字符串。
     * 当使用对象 Y 进行格式化时，
     * 如果对象是 NumberFormat，则调用 ((NumberFormat) Y).format(X)。
     * 否则调用 Y.toString()。
     */
     ...
     * @param toAppendTo 文本追加的位置。
     * @param status 忽略，没有有用的返回状态。
     */
   public StringBuffer format(double number, StringBuffer toAppendTo,
     ...
     * 从输入文本解析一个数字。
     * @param text 源文本。
     * @param status 输入-输出参数。输入时，status.index 字段指示应解析的源文本的第一个字符。
     * 退出时，如果没有发生错误，status.index 被设置为源文本中第一个未解析的字符。
     * 退出时，如果发生错误，status.index 保持不变，status.errorIndex 被设置为导致解析失败的第一个字符的索引。
     * @return 一个表示解析的数字值的 Number。
     */
    public Number parse(String text, ParsePosition status) {
        ...
        return new Double(bestNumber);
                ...
                /**
     * 查找大于 {@code d} 的最小 double 值。
     * 如果是 {@code NaN}，则返回相同的值。
     * <p>用于创建半开区间。
     *
     * @param d 参考值
    ...
    /**
     * 选择的下限列表。如果要格式化的数字大于或等于 <code>choiceLimits[i]</code> 且小于 <code>choiceLimits[i+1]</code>，格式化器将返回 <code>choiceFormats[i]</code>。
     * @serial
     */
    private double[] choiceLimits;

    /**
     * 选择字符串列表。如果要格式化的数字大于或等于 <code>choiceLimits[i]</code> 且小于 <code>choiceLimits[i+1]</code>，格式化器将返回 <code>choiceFormats[i]</code>。
     * @serial
     */
    ...
    */
    ...
    /**
     * 查找大于 {@code d} 的最小 double 值（如果 {@code positive} 为 {@code true}），或小于 {@code d} 的最大 double 值（如果 {@code positive} 为 {@code false}）。
     * 如果是 {@code NaN}，则返回相同的值。
     *
     * 不影响浮点标志，前提是以下成员函数也不影响：
     *          Double.longBitsToDouble(long)
     *          Double.doubleToLongBits(double)
     *          Double.isNaN(double)
     *
     * @param d        参考值
     * @param positive 如果需要最小的 double 值，则为 {@code true}；否则为 {@code false}
     * @return 最小或最大的 double 值
     */
    public static double nextDouble (double d, boolean positive) {
        ...
        /* 将所有位保存在一个 long 中以供后续使用 */
        long bits = Double.doubleToLongBits(d);

        /* 去掉符号位 */
        long magnitude = bits & ~SIGN;

        /* 如果下一个 double 值远离零，增加幅度 */
        if ((bits > 0) == positive) {
