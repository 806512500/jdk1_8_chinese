...
/**
 * <code>MessageFormat</code> 提供了一种以语言中立的方式生成连接消息的手段。使用此方法来构建显示给最终用户的消息。
 *
 * <p>
 * <code>MessageFormat</code> 接收一组对象，对它们进行格式化，然后将格式化的字符串插入到模式的适当位置。
 *
 * <p>
 * <strong>注意：</strong>
 * <code>MessageFormat</code> 与其他 <code>Format</code> 类不同，您需要使用其构造函数之一创建一个 <code>MessageFormat</code> 对象（而不是使用 <code>getInstance</code> 风格的工厂方法）。工厂方法不是必需的，因为 <code>MessageFormat</code> 本身不实现特定于区域设置的行为。任何特定于区域设置的行为都是由您提供的模式以及用于插入参数的子格式定义的。
 *
 ...
 * </pre></blockquote>
 *
 * <p>在 <i>String</i> 中，一对单引号可以用来引用任何除单引号外的任意字符。例如，模式字符串 <code>"'{0}'"</code> 表示字符串 <code>"{0}"</code>，而不是一个 <i>FormatElement</i>。单引号本身必须在 <i>String</i> 中用双单引号 {@code ''} 表示。例如，模式字符串 <code>"'{''}'"</code> 被解释为 <code>'{</code>（引用开始和左大括号），<code>''</code>（单引号），和 <code>}'</code>（右大括号和引用结束），<em>而不是</em> <code>'{'</code> 和 <code>'}'</code>（引用的左大括号和右大括号）：表示字符串 <code>"{'}"</code>，<em>而不是</em> <code>"{}"</code>。
 *
 * <p><i>SubformatPattern</i> 由其对应的子格式解释，并应用子格式依赖的模式规则。例如，模式字符串 <code>"{1,number,<u>$'#',##</u>}"</code>（带下划线的 <i>SubformatPattern</i>）将生成一个带有磅号引用的数字格式，结果如：{@code
 * "$#31,45"}. 请参阅每个 {@code Format} 子类的文档以了解详细信息。
 *
 * <p>任何未匹配的引号被视为在给定模式的末尾关闭。例如，模式字符串 {@code "'{0}"} 被视为模式 {@code "'{0}'"}。
 *
 * <p>任何未引用模式中的大括号必须是平衡的。例如，<code>"ab {0} de"</code> 和 <code>"ab '}' de"</code> 是有效的模式，但 <code>"ab {0'}' de"</code>，<code>"ab } de"</code> 和 <code>"''{''"</code> 不是。
 *
 * <dl><dt><b>警告：</b><dd>在消息格式模式中使用引号的规则不幸地显示为有些令人困惑。特别是，本地化人员并不总是清楚单引号是否需要加倍。确保通知本地化人员有关规则，并告诉他们（例如，通过在资源束源文件中使用注释）哪些字符串将由 {@code MessageFormat} 处理。注意，本地化人员可能需要在翻译的字符串中使用单引号，而原始版本中没有这些单引号。
 * </dl>
 * <p>
 * <i>ArgumentIndex</i> 值是非负整数，使用 {@code '0'} 到 {@code '9'} 的数字编写，表示传递给 {@code format} 方法或由 {@code parse} 方法返回的结果数组中的索引。
 * <p>
 * <i>FormatType</i> 和 <i>FormatStyle</i> 值用于为格式元素创建一个 {@code Format} 实例。下表显示了这些值如何映射到 {@code Format} 实例。表中未显示的组合是非法的。<i>SubformatPattern</i> 必须是用于的 {@code Format} 子类的有效模式字符串。
 *
 ...
 * <p>
 * 第一个示例使用静态方法 <code>MessageFormat.format</code>，该方法内部创建一个用于一次性使用的 <code>MessageFormat</code>：
 * <blockquote><pre>
 * int planet = 7;
 ...
 *
 * <p>
 * 对于更复杂的模式，您可以使用 <code>ChoiceFormat</code> 生成单数和复数的正确形式：
 * <blockquote><pre>
 ...
 *
 * <p>
 * 您可以像上面的示例那样编程创建 <code>ChoiceFormat</code>，也可以使用模式创建。有关更多信息，请参阅 {@link ChoiceFormat}。
 * <blockquote><pre>{@code
 * form.applyPattern(
 ...
 *
 * <p>
 * <strong>注意：</strong> 如上所述，<code>MessageFormat</code> 中由 <code>ChoiceFormat</code> 生成的字符串被视为特殊；'{' 的出现用于指示子格式，并导致递归。如果您编程创建了 <code>MessageFormat</code> 和 <code>ChoiceFormat</code>（而不是使用字符串模式），则要小心不要生成递归自身的形式，这将导致无限循环。
 * <p>
...
public class MessageFormat extends Format {
    ...
    /**
     * 构造一个用于默认 {@link java.util.Locale.Category#FORMAT FORMAT} 语言环境和指定模式的 MessageFormat。
     * 构造函数首先设置语言环境，然后解析模式并为其中包含的格式元素创建子格式列表。
     * 模式及其解释在 <a href="#patterns">类描述</a> 中指定。
     *
    ...
    /**
     * 构造一个用于指定语言环境和模式的 MessageFormat。
     * 构造函数首先设置语言环境，然后解析模式并为其中包含的格式元素创建子格式列表。
     * 模式及其解释在 <a href="#patterns">类描述</a> 中指定。
     *
     * @param pattern 此消息格式的模式
     ...
     * 这会影响后续调用
     * <ul>
     * <li>到 {@link #applyPattern applyPattern} 和 {@link #toPattern toPattern} 方法，如果格式元素指定了格式类型，则在 <code>applyPattern</code> 方法中创建子格式，以及
     * <li>到 <code>format</code> 和 {@link #formatToCharacterIterator formatToCharacterIterator} 方法，如果格式元素未指定格式类型，则在格式化方法中创建子格式。
     * </ul>
     * 已经创建的子格式不受影响。
     *
    ...
    public void applyPattern(String pattern) {
            ...
            // 仅在此处分配 segments[SEG_RAW]。其余部分按需分配。
    ...
    /**
     * 设置用于传递给 <code>format</code> 方法或从 <code>parse</code>
     * 方法返回的值的格式。 <code>newFormats</code> 中元素的索引
     * 对应于先前设置的模式字符串中使用的参数索引。
     * <code>newFormats</code> 中格式的顺序因此对应于
     * 传递给 <code>format</code> 方法的 <code>arguments</code> 数组
     * 或 <code>parse</code> 方法返回的结果数组中的元素顺序。
     * <p>
     * 如果模式字符串中的格式元素使用了多个参数索引，
     * 则相应的新的格式将用于所有这些格式元素。如果模式字符串中的任何格式元素未使用参数索引，
     * 则相应的新的格式将被忽略。如果提供的格式少于所需，
     ...
     * <p>
     * 如果提供的格式多于模式字符串所需，
     * 则多余的格式将被忽略。如果提供的格式少于所需，
     * 则仅替换前 <code>newFormats.length</code> 个格式。
     * <p>
     * 由于模式字符串中格式元素的顺序在本地化过程中经常发生变化，
     ...
     * 参数索引是格式元素定义的一部分，表示
     * 传递给 <code>format</code> 方法的 <code>arguments</code> 数组
     * 或 <code>parse</code> 方法返回的结果数组中的索引。
     * <p>
     * 如果模式字符串中的多个格式元素使用了相同的参数索引，
     * 则新的格式将用于所有这些格式元素。如果模式字符串中的任何格式元素未使用参数索引，
     * 则新的格式将被忽略。
     *
     * @param argumentIndex 要使用新格式的参数索引
     * @param newFormat 要使用的新的格式
     * @since 1.4
     */
    public void setFormatByArgumentIndex(int argumentIndex, Format newFormat) {
        for (int j = 0; j <= maxOffset; j++) {
            if (argumentNumbers[j] == argumentIndex) {
                ...
                /**
     * 设置给定格式元素索引的格式元素的格式。
     * 格式元素索引是从模式字符串开头开始计数的格式元素的零基础编号。
     * <p>
     * 由于模式字符串中格式元素的顺序在本地化过程中经常发生变化，通常最好使用
     * {@link #setFormatByArgumentIndex setFormatByArgumentIndex}
     * 方法，该方法根据指定的参数索引访问格式元素。
     *
     * @param formatElementIndex 模式中的格式元素索引
     * @param newFormat 要用于指定格式元素的格式
     * @exception ArrayIndexOutOfBoundsException 如果 {@code formatElementIndex} 等于或大于模式字符串中的格式元素数量
     */
    ...
    /**
     * 获取传递给 <code>format</code> 方法或从 <code>parse</code>
     * 方法返回的值所使用的格式。返回数组中的元素索引对应于先前设置的
     * 模式字符串中使用的参数索引。
     * 返回数组中的格式顺序对应于传递给 <code>format</code> 方法的
     * <code>arguments</code> 数组或由 <code>parse</code> 方法返回的结果数组中的元素顺序。
     * <p>
     * 如果模式字符串中的多个格式元素使用相同的参数索引，则返回数组中将返回最后一个这样的格式元素所使用的格式。如果模式字符串中的任何格式元素未使用某个参数索引，则返回数组中将返回 null。
     *
     * @return 模式中用于参数的格式
     * @since 1.4
     */
    ...
    /**
     * 获取先前设置的模式字符串中格式元素所使用的格式。
     * 返回数组中的格式顺序对应于模式字符串中格式元素的顺序。
     * <p>
     * 由于模式字符串中格式元素的顺序在本地化过程中经常发生变化，通常最好使用
     * {@link #getFormatsByArgumentIndex getFormatsByArgumentIndex}
     * 方法，该方法假设格式的顺序对应于传递给 <code>format</code> 方法的
     * <code>arguments</code> 数组或由 <code>parse</code> 方法返回的结果数组中的元素顺序。
     *
     * @return 模式中格式元素所使用的格式
     */
    ...
    /**
     * 格式化对象数组，并将 <code>MessageFormat</code> 的模式（格式元素被格式化的对象替换）追加到提供的 <code>StringBuffer</code>。
     * <p>
     * 用于单个格式元素的替换文本是从格式元素的当前子格式和
     * <code>arguments</code> 数组中格式元素的参数索引处的元素派生的，如下表中第一个匹配的行所示。如果 <code>arguments</code> 为
     * <code>null</code> 或者元素数量少于 argumentIndex+1，则参数 <i>不可用</i>。
     *
     * <table border=1 summary="子格式、参数和格式化文本的示例">
     *    <tr>
     *       <th>子格式
     *       <th>参数
     *       <th>格式化文本
     ...
     *            输出：对齐字段的偏移量。
     * @return 作为 {@code result} 传递的字符串缓冲区，附加了格式化的文本
     * @exception IllegalArgumentException 如果 <code>arguments</code> 数组中的参数不是格式元素使用的预期类型。
     */
                                     ...
                                     FieldPosition pos)
    {
        return subformat(arguments, result, pos, null);
                ...
                /**
     * 使用给定的模式创建一个 MessageFormat 并使用它来格式化给定的参数。这相当于
     * <blockquote>
     ...
     * @return 格式化后的字符串
     * @exception IllegalArgumentException 如果模式无效，或者 <code>arguments</code> 数组中的某个参数
     *            不是格式元素所期望的类型。
     */
    ...
    // Overrides
    /**
     * 格式化一个对象数组，并将 <code>MessageFormat</code> 的模式（格式元素被格式化后的对象替换）追加到提供的 <code>StringBuffer</code> 中。
     * 这相当于
     * <blockquote>
     ...
     * @param arguments 要格式化和替换的对象数组。
     * @param result 追加文本的位置。
     * @param pos 输入：对齐字段（如果需要）。
     *            输出：对齐字段的偏移量。
     * @exception IllegalArgumentException 如果 <code>arguments</code> 数组中的某个参数
     *            不是格式元素所期望的类型。
     */
    public final StringBuffer format(Object arguments, StringBuffer result,
                                     FieldPosition pos)
    ...
    /**
     * 格式化一个对象数组，并将它们插入到 <code>MessageFormat</code> 的模式中，生成一个
     ...
     * 返回的文本相同。
     * <p>
     * 此外，<code>AttributedCharacterIterator</code> 包含至少指示从 <code>arguments</code> 数组中的某个参数生成文本的位置的属性。这些属性的键是
     * 类型为 <code>MessageFormat.Field</code>，它们的值是
     * <code>Integer</code> 对象，指示从哪个 <code>arguments</code>
     * 数组中的参数生成了文本。
     * <p>
     * <code>MessageFormat</code> 使用的基础 <code>Format</code> 实例的属性/值也将被
     * 放入结果的 <code>AttributedCharacterIterator</code> 中。这不仅允许你找到参数在结果字符串中的位置，还可以找到它包含的字段。
     *
     * @param arguments 要格式化和替换的对象数组。
    ...
    public AttributedCharacterIterator formatToCharacterIterator(Object arguments) {
        ...
        if (arguments == null) {
            throw new NullPointerException(
                   "formatToCharacterIterator 必须传递非 null 对象");
        }
     ...
     * <li>尚未处理递归（即
     *     替换的字符串包含 {n} 引用。）
     * <li>如果解析的某部分是模糊的，则可能无法找到匹配项（或正确的匹配项）。
     *     例如，如果使用模式 "{1},{2}" 和字符串参数 {"a,b", "c"}，它将格式化为 "a,b,c"。
     *     当结果被解析时，它将返回 {"a", "b,c"}。
     * <li>如果字符串中某个参数被解析多次，
     *     则后面的解析结果会覆盖前面的。
     * </ul>
     * 当解析失败时，使用 ParsePosition.getErrorIndex() 来找出字符串中解析失败的位置。返回的错误
     * 索引是字符串正在比较的子模式的起始偏移量。例如，如果解析字符串 "AAA {0} BBB"
     * 与模式 "AAD {0} BBB" 进行比较，错误索引为 0。当发生错误时，此方法将返回 null。
     * 如果源为 null，则返回一个空数组。
     *
     ...
     *
     * @param source 一个 <code>String</code>，其开头部分应被解析。
     * @return 从字符串解析出的 <code>Object</code> 数组。
     * @exception ParseException 如果指定字符串的开头部分无法解析。
     */
     ...
     * 如果解析成功，则 <code>pos</code> 的索引将更新为最后一个使用字符之后的索引（解析不一定使用到字符串的末尾），并返回解析出的对象数组。更新后的 <code>pos</code> 可用于指示下一次调用此方法的起始点。
     * 如果发生错误，则 <code>pos</code> 的索引不会更改，<code>pos</code> 的错误索引将设置为发生错误的字符索引，并返回 null。
     * <p>
     * 有关消息解析的更多信息，请参阅 {@link #parse(String, ParsePosition)} 方法。
     *
     * @param source 一个 <code>String</code>，其部分应被解析。
     * @param pos 一个包含上述索引和错误索引信息的 <code>ParsePosition</code> 对象。
     * @return 从字符串解析出的 <code>Object</code> 数组。发生错误时，返回 null。
    ...
    public Object clone() {
        ...
        // 克隆数组。不能使用工具类，因为 Cloneable 中存在 bug
     ...
     */
    public int hashCode() {
        return pattern.hashCode(); // 足够合理的分布
    }
    ...
    public static class Field extends Format.Field {
         ...
         * 与键关联的值将是一个 <code>Integer</code>，表示 <code>arguments</code> 数组中
         * 生成文本的参数的索引。
         */
    ...
    /**
     * 要插入格式化值的字符串。换句话说，这是构造时提供的模式，其中所有的 {} 表达式都被移除。
     * @serial
     */
    ...
    /**
     * 与每个格式化程序对应的参数编号。格式化程序按其在模式中出现的顺序存储，而不是按参数指定的顺序。
     * @serial
     */
    private int[] argumentNumbers = new int[INITIAL_FORMATS];

    /**
     * <code>offsets</code> 中条目数量减一。也可以认为是 <code>offsets</code> 中使用的最高编号元素的索引。
     * 这些数组中使用的元素数量应与 <code>offsets</code> 中使用的数量相同，因此该变量足以告诉我们所有数组中有多少条目。
     * @serial
     */
    private int maxOffset = -1;

    /**
     * 由 format 使用的内部例程。如果 <code>characterIterators</code> 不为 null，则将根据需要从子格式创建 AttributedCharacterIterator。
     * 如果 <code>characterIterators</code> 为 null 且 <code>fp</code> 不为 null 并标识 <code>Field.MESSAGE_ARGUMENT</code>，
     * 则将设置第一个替换参数的位置。
     *
     * @exception IllegalArgumentException 如果 <code>arguments</code> 数组中的参数不是格式元素使用的预期类型。
     */
    private StringBuffer subformat(Object[] arguments, StringBuffer result,
        ...
        for (int i = 0; i <= maxOffset; ++i) {
            ...
            if (false) { // if (argRecursion == 3){
                ...
                // 此时我们处于两种状态之一：subFormatter
                // 非空，表示我们应该使用它来格式化 obj，
                // 或者 arg 非空，我们应该将其用作值。

                if (characterIterators != null) {
                    // 如果 characterIterators 非空，表示我们需要
                    // 从子格式化器中获取 CharacterIterator。
    ...
    /**
     * 便捷方法，将 <code>iterator</code> 中的所有字符追加到 StringBuffer <code>result</code>。
     */
    ...
    private static final int SEG_MODIFIER = 3; // 修饰符或子格式
