...
abstract class FileSystem {
    ...
    /**
     * 返回当两个参数的 File 构造函数中的父目录参数为空路径名时使用的父路径名字符串。
     */
    public abstract String getDefaultParent();

    /**
     * 如果必要，后处理给定的 URI 路径字符串。这在 win32 上使用，例如，将 "/c:/foo" 转换为 "c:/foo"。路径字符串仍然使用斜杠分隔符；File 类中的代码将在此方法返回后进行转换。
     */
    ...
    /**
     * 返回由给定抽象路径名表示的文件或目录的简单布尔属性，如果它不存在或发生其他 I/O 错误，则返回零。
     */
    ...
    /**
     * 返回由给定抽象路径名表示的文件或目录的最后修改时间，如果它不存在或发生其他 I/O 错误，则返回零。
     */
    public abstract long getLastModifiedTime(File f);

    /**
     * 返回由给定抽象路径名表示的文件的长度（以字节为单位），如果它不存在、是目录或发生其他 I/O 错误，则返回零。
     */
    ...
    /**
     * 使用给定的路径名创建一个新空文件。如果文件被创建，则返回 <code>true</code>；如果具有给定路径名的文件或目录已经存在，则返回 <code>false</code>。如果发生 I/O 错误，则抛出 IOException。
     */
    ...
    /**
     * 列出由给定抽象路径名表示的目录的元素。如果成功，则返回一个包含目录元素名称的字符串数组；否则，返回 <code>null</code>。
     */
    ...
    /**
     * 将由第一个抽象路径名表示的文件或目录重命名为由第二个抽象路径名表示的文件或目录，仅当操作成功时返回 <code>true</code>。
     */
