...
/**
 * InputStreamReader 是从字节流到字符流的桥梁：它读取字节并使用指定的 {@link
 * java.nio.charset.Charset 字符集} 将其解码为字符。它可以按名称指定使用的字符集，也可以显式给出，或者可以接受平台的默认字符集。
 *
 * <p> 每次调用 InputStreamReader 的 read() 方法之一都可能导致从底层字节输入流读取一个或多个字节。为了高效地将字节转换为字符，从底层流中读取的字节数可能比当前读取操作所需的字节数多。
 *
 * <p> 为了提高效率，可以考虑将 InputStreamReader 包装在 BufferedReader 中。例如：
 *
 * <pre>
 * BufferedReader in
...
public class InputStreamReader extends Reader {
     ...
     * @param  charsetName
     *         支持的
     *         {@link java.nio.charset.Charset 字符集} 的名称
     *
     ...
     * </p>
     * @return 此编码的历史名称，或者如果流已关闭则返回 <code>null</code>
     *
     * @see java.nio.charset.Charset
     *
     * @revised 1.4
    ...
    /**
     * 告知此流是否准备好读取。如果 InputStreamReader 的输入缓冲区不为空，或者可以从底层字节流读取字节，则 InputStreamReader 已准备好读取。
     *
     * @exception  IOException  如果发生 I/O 错误
     */
