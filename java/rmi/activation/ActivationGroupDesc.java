...
public final class ActivationGroupDesc implements Serializable {
     ...
     * 环境覆盖（将覆盖组实现 VM 中的系统属性）。命令
     * 环境可以控制启动子 VM 时使用的精确命令/选项，或者可以为 <code>null</code> 以接受
     * rmid 的默认值。
     *
     * <p>此构造函数将创建一个 <code>ActivationGroupDesc</code>
     * 其组类名为 <code>null</code>，表示系统的默认 <code>ActivationGroup</code> 实现。
     *
     * @param overrides 当组重新创建时要设置的属性集。
     ...
     * 环境。
     *
     * @param className 组的包限定类名或 <code>null</code>。组类名为
     * <code>null</code> 表示系统的默认 <code>ActivationGroup</code> 实现。
     * @param location 从何处加载组的类
     * @param data 组的初始化数据，以序列化形式包含（例如，可以包含属性）
     * @param overrides 一个属性映射，将覆盖子进程中默认设置的属性（将转换为
     * <code>-D</code> 选项），或 <code>null</code>。
     * @param cmd 用于在另一个进程中执行 VM 的控制选项（或 <code>null</code>）。
     * @since 1.2
     */
    ...
    /**
     * 返回组的类名（可能是 <code>null</code>）。组类名为
     * <code>null</code> 表示系统的默认 <code>ActivationGroup</code> 实现。
     * @return 组的类名
     * @since 1.2
     */
    ...
    public static class CommandEnvironment implements Serializable {
        ...
        /**
         * 创建一个包含所有必要信息的 CommandEnvironment。
         *
         * @param cmdpath java 可执行文件的名称，包括完整路径，或 <code>null</code>，表示“使用 rmid 的默认值”。
         * 命名的程序 <em>必须</em> 能够接受多个 <code>-Dpropname=value</code> 选项（如“java”工具文档中所述）
         *
         * @param argv 用于创建 ActivationGroup 的额外选项。Null 与空列表具有相同效果。
         * @since 1.2
         */
         ...
         * 获取配置的路径限定 java 命令名称。
         *
         * @return 配置的名称，或 <code>null</code> 表示接受默认值
         * @since 1.2
         */
         ...
         * 注意 rmid 可能在这些选项之前或之后添加其他选项，或两者都有。
         * 永不返回 <code>null</code>。
         * @since 1.2
         */
        ...
        /**
         * 为相似的 <code>CommandEnvironment</code> 返回相同的值。
         * @return 一个整数
         * @see java.util.Hashtable
         */
        public int hashCode()
    ...
    /**
     * 为相似的 <code>ActivationGroupDesc</code> 生成相同的数字。
     * @return 一个整数
     * @see java.util.Hashtable
     */
    public int hashCode() {
