...
/**
 * <code>RMISocketFactory</code> 实例由 RMI 运行时使用，以获取 RMI 调用所需的客户端和服务器套接字。应用程序可以使用
 * <code>setSocketFactory</code> 方法请求 RMI 运行时使用其套接字工厂实例，而不是默认实现。
 *
 * <p>默认的套接字工厂实现采用三层方法创建客户端套接字。首先，尝试直接连接到远程 VM 的套接字连接。如果失败（由于防火墙），运行时使用带有服务器显式端口号的 HTTP。
 ...
 *
 * <p><strong>已弃用：HTTP 隧道。</strong> <em>上述描述的 HTTP 隧道机制，特别是带有显式端口的 HTTP 和到 cgi-bin 脚本的 HTTP，已被弃用。这些 HTTP 隧道机制可能会在平台的未来版本中被移除。</em>
 *
 * <p>默认的套接字工厂实现创建绑定到通配符地址的服务器套接字，该地址接受来自所有网络接口的请求。
 *
 * @implNote
 * <p>您可以使用 {@code RMISocketFactory} 类创建绑定到特定地址的服务器套接字，以限制请求的来源。例如，以下代码实现了一个将服务器套接字绑定到 IPv4 回环地址的套接字工厂。这限制了 RMI 仅处理来自本地主机的请求。
 *
 * <pre>{@code
        ...
        implements RMIClientSocketFactory, RMIServerSocketFactory
{
    ...
    /**
     * 设置 RMI 获取套接字的全局套接字工厂（如果远程对象未与特定的客户端和/或服务器套接字工厂关联）。RMI 套接字工厂只能设置一次。注意：只有当前安全管理器允许设置套接字工厂时，才能设置 RMISocketFactory；
     * 如果不允许，将抛出 SecurityException。
     * @param fac 套接字工厂
     * @exception IOException 如果 RMI 套接字工厂已设置
     * @exception  SecurityException  如果存在安全管理器且其
     *             <code>checkSetFactory</code> 方法不允许此操作。
    ...
    /**
     * 返回本 RMI 实现使用的默认套接字工厂。当 <code>getSocketFactory</code> 返回 <code>null</code> 时，这将是 RMI 运行时使用的工厂。
    ...
    public synchronized static RMISocketFactory getDefaultSocketFactory() {
        ...
        return defaultSocketFactory;
                ...
                /**
     * 设置在 RMI 运行时如果服务器套接字创建失败时调用的失败处理程序。默认情况下，如果没有安装失败处理程序
     * 并且服务器套接字创建失败，RMI 运行时会尝试重新创建服务器套接字。
     *
     * <p>如果有安全管理器，此方法首先调用安全管理器的 <code>checkSetFactory</code> 方法
     * 以确保允许此操作。这可能会导致 <code>SecurityException</code>。
     *
     * @param fh 失败处理程序
